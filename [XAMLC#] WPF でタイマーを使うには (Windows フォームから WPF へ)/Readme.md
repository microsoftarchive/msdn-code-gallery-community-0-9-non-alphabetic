...
<p>タイマーの機能は、System.Timers 名前空間で定義されている Timer クラスで実装されています (これは、Windows フォームに配置する System.Windows.Forms.Timer コンポーネントとは違うものです)。しかし、System.Timers.Timer クラスはユーザー インターフェイスのためのスレッド (UI スレッド) とは独立したスレッドで実行されるため、イベント ハンドラーからユーザー インターフェイスを操作することができません。スレッド上のタスク管理のために
