...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 2 月 12 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、nullable な変数を書いたらセンパイに怒られました。で、必死に null を使わないコードに書き直したら別のセンパイから冗長すぎるって怒られました」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「ほほう。よくある話じゃのう」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「いったいどっちのセンパイが正しいのですか? どう書けば両方のセンパイを満足させられるのですか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「では混乱の原因から説明しよう」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">null を排除すべき理由</a> </li><li><a href="#02">null を受け入れるべき理由</a> </li><li><a href="#03">null を受け入れる - null 許容型という必殺技</a> </li><li><a href="#04">null 合体演算子 (?? 演算子)</a> </li><li><a href="#05">null 許容型を使用する上での留意点</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. null を排除すべき理由</h2>
<p>マーチン・ファウラーがその有名な著書「リファクタリング」で記載している「ヌル オブジェクトの導入」を使えば、必然的に null は排除されます。以下に、「なぜ null を排除したいのか」という観点も含め、この一例を説明しましょう。下記のプログラムに含まれる findSmallA メソッドは、渡された配列から小文字の a を含む文字列を探して返します。</p>
<p>まず、このプログラムは特に例外も発生させずに動作を終了します。</p>
...
class Program
{
    private static string findSmallA(string[] strs)
    {
    ...
    {
        string result = findSmallA(new[] { &quot;ABC&quot;, &quot;DEFG&quot; });
...
<p>しかし、結果の文字数を知りたくなって、WriteLine メソッドの行を以下のように書き換えると例外が起きます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">Console.WriteLine(result.Length);</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">Console.WriteLine(result.Length);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>理由は null にあります。上記の findSmallA では、見つからない場合は null を返すという前提で書かれていますが、利用する側 (Main) では null になるケースを見落としています。もし、発見できないと result は null になります。WriteLine では、引数は null でも受け入れますが、Length メソッドでは受け入れません。実際 null でエラーを引き起こす機能は多く、できればこうした状況は避けたいものです。</p>
<p>このような問題を解消する処方箋として考えられたのが「ヌル オブジェクトの導入」です。アイデアは簡単で、null ではなく、「無い」ことを示す代わりのオブジェクトを返します。今回の場合、見つからなかったことを示すために null ではなく &quot;&quot; (空文字) を返しておけば例外を回避できたわけです。</p>
<p>この手法では、上記の通り、「何もない」ことを示すために「例外なく呼び出しを受け入れるだけで、何もしないオブジェクト」で代用することが一般的ですが (一般に、null を処理できる専用のオブジェクトを作成します)、null の問題を回避するために使用されない値を割り当てて使う (例えば、0 ～ 3、または null の値を返すことが期待されている場合、null の代わりに -1 を返す、など) こともよく行われます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16770-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. null を受け入れるべき理由</h2>
<p>しかし、null を排除すればそれですべてが解決するかというと、現実にはそうでないケースもいくつか存在します。</p>
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ null を排除すると回りくどいコードになる場合もある</strong></dt><dd style="margin-bottom:10px">既に null を前提としたデータも珍しくありません。例えば、データベース上に NULL を許容したフィールドは多く、これらのすべてに対して上述のような対策をおこなうのは非現実的でしょう。</dd><dt style="margin-bottom:10px"><strong>・ 安全に「無い」ことを示す値を提供できないことも多い</strong></dt><dd style="margin-bottom:10px">例えば、空文字列を含めてすべての文字列が有効な結果となるメソッドで、どのような文字列を使って「無い」を示せば良いでしょうか。また、1 行を読み込むメソッドは、空文字列を含めてすべて有効です。もし、入力が無かったことを示す別の手法で運用すれば、コードはかなり冗長になるでしょう。</dd></dl>
<p>また、既存資産を上手く活用しようと思うなら、null は避けて通れません。例えば、IEnumerable の FirstOrDefault メソッドを使うと、前述した findSmallA メソッドは以下のようにループ無しで記述できますが、これは null を返します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    private static string findSmallA(string[] strs)
    {
        return strs.FirstOrDefault((x)=&gt;x.Contains(&quot;a&quot;));
    }
    static void Main(string[] args)
    {
        string result = findSmallA(new[] { &quot;ABC&quot;, &quot;DEFG&quot; });
...
<p>なぜ null を返すのかといえば、参照型のデフォルト値は null で、FirstOrDefault メソッドは発見できない場合、型のデフォルト値を返す仕様だからです。この場合、null の排除のために別のメソッドを用意するのは逆に冗長になります。</p>
<p>また、データベースに NULL を許容する整数のフィールドがあるとすれば、それをそのまま変数に&#26684;納したいと思うのが素直な心情でしょう。そうしなければ、ソースが過剰に複雑になります。しかし、値型に null 値はそもそもありません。</p>
<p>従って、null がない方が安全性が高いことは分かっていても、現実には、過剰な複雑さまで背負い込んで null を排除したくはなく、むしろ、ソースをシンプルにするために「値型にも、場合によっては null が必要だ」と思われることでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16771-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. null を受け入れる - null 許容型という必殺技</h2>
<p>null を受け入れるための方法として null 許容型という強力な型が用意されています。これは、既存の値型を拡張し、null も取り扱い可能とするものです。</p>
<p>使い方は簡単です。あらゆる値型は、名前の後ろに「?」を負荷することで null 許容型になります。以下は実際の使用例です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    private static void dump(int? a)
    {
        ...
        else Console.WriteLine(&quot;a is {0}&quot;,a);
    }
    static void Main(string[] args)
    {
        dump(null);
...
<p>この通り、int であれば整数であり null にはなりませんが、int? であれば null も入ります。それでいて、依然として整数を受け入れる能力も持っていて、123 のような値も代入できます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16772-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. null 合体演算子 (?? 演算子)</h2>
<p>前述の通り、null を許すとソースコードが複雑になるケースがあります。前述の Length プロパティのように、null を受け入れないこともあるからです。</p>
<p>例えば、以下は引数が null の場合は現在日付時刻を出力する例です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    {
        if (a == null) Console.WriteLine(DateTime.Now);
        else Console.WriteLine(a);
    }
    static void Main(string[] args)
    {
        dump(null);
        dump(new DateTime(2010, 4, 1));
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果: (この結果は実行する日付時刻で変化します)</p>
<p>2010/01/02 14:14:37<br>
2010/04/01 0:00:00</p>
</div>
<p>見てのおわかりの通り、dump メソッドには条件判断が入り、WriteLine も 2 つあって冗長です。まず、これは、3 項演算子でもっとコンパクトに記述できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">private static void dump(DateTime? a)
    {
        Console.WriteLine(a == null ? DateTime.Now : a);
    }</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">private static void dump(DateTime? a)
    {
        Console.WriteLine(a == null ? DateTime.Now : a);
    }</code></pre>
</div>
...
<p>しかし、まだ a が 2 回登場して冗長です。これを null 合体演算子を使ってもっと短くしてみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この演算子は、1 つ目が null ではない場合にそれを採用し、null の場合のみ 2 つ目を評価して採用する機能を持っています。つまり、記述する意図としては同じですが、ずっと短く記述することができます。<br>
C# には、実は、null 許容型の利用を冗長にしないために、このような機能も用意されているのでおぼえておきましょう。</p>
<p>ちなみに、この演算子は参照型でも使用できるので、応用範囲は広くなります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16773-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. null 許容型を使用する上での留意点</h2>
<p>前述の通り、null 許容型は「?」を付ける前の元の型の持つ性質を受けぎながら、null を代入できるという能力が追加されます。そのため、int? は int と同じような計算能力を持ちます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>つまり、null ではない値を処理する際に、null 許容型だからといって特別な処理は必要ではありません。しかし、null 値を含む計算をおこなった場合、結果は以下の通りになります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>つまり、null 許容型では「結果が数値になる」ことを意味せず、null を含む演算はやはり null になるので注意してください。</p>
<p>さらに、null 許容型は、.NET の実行環境が提供する自然な型ではなく、一種のシンタックス・シュガー (糖衣構文) です。そのため、むやみに使いすぎるとメモリの利用効率が悪くなったり、実行速度が遅くなる可能性があります。null を入れる必要もないのにあえて int? と書いておくということは避けてください。</p>
<p>また、いくらソースコードを綺麗に書くことができても、実行時に NullReferenceException 例外が発生するリスクが無くなるわけではありません。(この点を解決するものではありません。) しかし、ソースコードが大幅にコンパクトになれば、バグが混入する確率も下がります。</p>
<p>null 許容型は、こうした性質を理解して、効果的に使用していくと良いでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16774-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました! つまり、null を排除するのも許容するのも良いソースを書くための 1 つの手段であって目的ではないのですね!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。どちらが良いという話ではなく、ソースコードを健全にするために使われる別のアプローチでしかないのじゃ!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ありがとうございます。センパイ達が納得するような簡潔で冗長性の少ないコードを目指します!」</td>
</tr>
</tbody>
</table>
