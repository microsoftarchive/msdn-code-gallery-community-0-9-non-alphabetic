...
<p>WPF と Windows フォームの仕組みの違いにより、WPF と Windows フォームのメニューにも違いが生じています。ここでは、両者の違いについて解説します。</p>
<h2>Code [C#]</h2>
<div style="margin:0px 0px 20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>public partial class MainWindow : Window<br>
{<br>
&nbsp;&nbsp;&nbsp;public static readonly ICommand OpenCommand = new RoutedCommand(&quot;OpenCommand&quot;, typeof(MainWindow));<br>
<br>
...
&nbsp;&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;&nbsp;private void menuExit_Click(object sender, RoutedEventArgs e)<br>
&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Close();<br>
&nbsp;&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;&nbsp;private void OpenCommand_CanExecute(object sender, CanExecuteRoutedEventArgs e)<br>
&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;e.CanExecute = (textBox1.Text.Length == 0);<br>
&nbsp;&nbsp;&nbsp;}<br>
<br>
&nbsp;&nbsp;&nbsp;private void OpenCommand_Executed(object sender, ExecutedRoutedEventArgs e)<br>
&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;var dlg = new <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/Microsoft.Win32.OpenFileDialog.aspx" target="_blank" title="Auto generated link to Microsoft.Win32.OpenFileDialog">Microsoft.Win32.OpenFileDialog</a>();<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if (dlg.ShowDialog() == true)<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;using (var reader = new <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.IO.StreamReader.aspx" target="_blank" title="Auto generated link to System.IO.StreamReader">System.IO.StreamReader</a>(dlg.FileName, Encoding.GetEncoding(&quot;SJIS&quot;)))<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;xmlns:local=&quot;clr-namespace:WpfApplication1&quot;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Title=&quot;MainWindow&quot; Height=&quot;350&quot; Width=&quot;525&quot;&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;Window.InputBindings&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;KeyBinding Command=&quot;{x:Static local:MainWindow.OpenCommand}&quot;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Gesture=&quot;Ctrl&#43;O&quot; /&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;/Window.InputBindings&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;Window.CommandBindings&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;CommandBinding Command=&quot;{x:Static local:MainWindow.OpenCommand}&quot;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;CanExecute=&quot;OpenCommand_CanExecute&quot; Executed=&quot;OpenCommand_Executed&quot; /&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;/Window.CommandBindings&gt;<br>
<br>
&nbsp;&nbsp;&nbsp;&lt;DockPanel&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;Menu Name=&quot;menu1&quot; DockPanel.Dock=&quot;Top&quot;&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;MenuItem Header=&quot;_File&quot;&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;MenuItem Header=&quot;_Open&quot; Command=&quot;{x:Static local:MainWindow.OpenCommand}&quot; /&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;MenuItem Header=&quot;E_xit&quot; Click=&quot;menuExit_Click&quot; /&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;/MenuItem&gt;<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;/MenuItem&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;/Menu&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;TextBox Name=&quot;textBox1&quot; DockPanel.Dock=&quot;Top&quot; TextWrapping=&quot;Wrap&quot; /&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;/DockPanel&gt;<br>
&lt;/Window&gt;</p>
</div>
<h2 style="margin-top:30px">実行結果</h2>
<p><img src="15255-image.png" alt=""></p>
<h2 style="margin-top:30px">ポイント</h2>
<p>通常、Windows フォームでは、WPF のレイアウト パネルを使わず、フォームに直接メニューを配置します。Windows フォームのメニュー (MenuStrip) の Dock プロパティはデフォルトで Top (上辺にドッキング) に設定されているため、配置するだけでウィンドウのタイトル バーの下にメニューが表示されます。WPF では、メニュー (Menu) も他のコントロールと同じようにレイアウト パネルを使って配置されるため、パネルの種類によって設定が変わります。</p>
<p>メニューをタイトル バーの直下に配置する場合、Grid レイアウトでは RowDefinitions で行を分割した上で、Menu の Grid.Row 添付プロパティを先頭 (0) に設定します。DockPanel レイアウトでは DockPanel.Dock 添付プロパティを Top にします。メニューを配置する場合に、その他のレイアウト (Canvas や StackPanel など) を使うことは、あまりありません。</p>
<p>メニューをどのように構築するかは大きく異なります。Windows フォームでは、以下のようにメニュー バーやドロップダウン メニューの項目を直接入力します。</p>
<p><img src="15256-image.png" alt=""></p>
<p>WPF では、プロパティ ウィンドウで Menu コントロールの Items プロパティの右側にある [&hellip;] ボタンを押して以下のようなコレクション エディターを使って編集するか、XAML に直接メニューの定義を記述します (Windows フォームでも Items プロパティや DropDownItems プロパティを編集することはできます)。</p>
<p><img src="15257-image.png" alt=""></p>
<p>Windows フォームのメニュー項目 (ToolStripMenuItem) と WPF のメニュー項目 (MenuItem) の違いについて以下の表に示します。</p>
<table class="grid" border="1" cellspacing="0" cellpadding="5" width="612" style="border-collapse:collapse; margin:20px 0px">
<colgroup span="1"><col span="1" width="380"><col span="1" width="300"></colgroup>
<tbody>
<tr>
<td align="center" valign="middle" style="background-color:#4f81bd"><span style="color:#ffffff"><strong>Windows フォームの ToolStripMenuItem</strong></span></td>
<td align="center" valign="middle" style="background-color:#4f81bd"><span style="color:#ffffff"><strong>WPF の MenuItem</strong></span></td>
</tr>
<tr>
<td valign="top">BackColor</td>
<td valign="top">Background</td>
</tr>
<tr>
<td valign="top">CheckOnClick</td>
<td valign="top">IsCheckable</td>
</tr>
<tr>
<td valign="top">Checked, CheckState</td>
<td valign="top">IsChecked</td>
</tr>
<tr>
<td valign="top">DropDownItems</td>
<td valign="top">(Items)</td>
</tr>
<tr>
<td valign="top">Enabled</td>
<td valign="top">IsEnabled</td>
</tr>
<tr>
...
<tr>
<td valign="top">ToolTipText ※ToolTip を使う場合</td>
...
<p>Windows フォームの「Text」プロパティでは、英字の前に「&amp;」記号をつけてショートカットを表すのに対し、WPF の「Header」プロパティでは、英字の前に「_」記号を付けるという違いがあります。</p>
<p>両者で大きく異なるのはメニュー コマンドの扱いです。Windows フォームでは、メニュー項目の Click イベントを使ってコマンドを処理します。WPF でも Click イベントを使うことはできますが、より進んだ形として Command プロパティを使うことができます。このプロパティには、ICommand インターフェイスを実装したクラスのオブジェクトを割り当てることができます。</p>
<p>新規作成や開く、保存といったファイル操作、コピーや切り取り、貼り付けといったクリップボード操作など、一般的なコマンドは、ApplicationCommands というクラスにあらかじめコマンドが定義されており、そのまま Command プロパティに割り当てることができます。上記の例では、「Edit」のドロップダウン項目に、クリップボード用のコマンドを割り当てています。<br>
※ApplicationCommands が提供するコマンドについては「<a href="http://msdn.microsoft.com/ja-jp/library/system.windows.input.applicationcommands_properties">ApplicationCommands プロパティ</a>」を参照してください。コマンドは自分で作成することもできます。上記の例では、ファイルを開くためのコマンド (OpenCommand) をクラスの静的メンバーとして作成しています (これはテスト用に作成したもので、通常は
 ApplicationCommands.Open を使います)。</p>
<p>OpenCommand に対して特殊キー操作 ([Ctrl]&#43;[O]) を関連付けるために、XAML では Window の InputBinding を設定していることに注意してください。Windows フォームでは、ShortcutKeys プロパティで特殊キーのためのショートカット キーを定義しますが、XAML では InputBinding に KeyBinding を追加することで定義します。ここには、マウス操作を関連付けるための MouseBinding も追加できます。</p>
<p>コマンドを使うことで、呼び出す機能と、メニュー項目やボタンとの結びつきを弱め、独立したものとして実装できます。必要に応じて、特定のウィンドウではなく、アプリケーション内で自由に使えるように実装することもできます。クリップボード用コマンドのように、コマンドの中で機能を完結しておけば、ユーザー インターフェイスを設計する際には Command プロパティにコマンドを割り当てるだけで済み、コーディングは不要になります。また、CanExecute 属性を使うことで、「そのコマンドが使えるかどうか」を定義できます。上記の例では、TextBox
 のテキストがすべて空のときだけ、ファイルを開くコマンドを有効にしています (これは、テスト用の実装です)。CanExecute は必要なときに自動的に呼び出されるため、画面表示のタイミングを気にする必要はありません。</p>
<p>この他、WPF では MDI (マルチ ドキュメント インターフェイス) がないため、異なるウィンドウのメニューを併合するといった機能はありません。最初に述べたとおり、メニューの差異は、Windows フォームと WPF 自身の違いによるところが大きいので、WPF の特徴に慣れていけば、メニューの扱いにも慣れることができるでしょう。</p>
...
逆引きサンプル コード一覧へ</a> </li><li>もっと他のレシピを見る &gt;&gt; <a href="http://code.msdn.microsoft.com/ja-jp" target="_blank">
