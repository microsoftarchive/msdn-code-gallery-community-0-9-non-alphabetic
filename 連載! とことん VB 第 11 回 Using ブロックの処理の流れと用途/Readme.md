...
<pre class="hidden">Using comp As New MyComp1()    &larr;①
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">vb</span>
<pre class="hidden">Public Class MyComp1        &larr;①
    Implements IDisposable    &larr;②
 
    Public Sub New()        &larr;③
    End Sub
 
    Public Sub Dispose() Implements IDisposable.Dispose    &larr;④
    End Sub
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">vb</span>
<pre class="hidden">Dim comp As New MyComp1()    &larr;①
Try                &larr;②
    Using ブロック内の処理
Finally                &larr;③
    If comp IsNot Nothing Then    &larr;④
        comp.Dispose()        &larr;⑤
    End If
End Try</pre>
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">vb</span>
<pre class="hidden">Private Sub Button1_Click( ... ) Handles Button1.Click  &larr;Clickイベントハンドラ
    Dim flag As Boolean = True            &larr;①
 
    Using comp As New MyComp1(ListBox1.Items)    &larr;②
        ListBox1.Items.Add(&quot;処理1&quot;)        
        If flag Then Return            &larr;③
        ListBox1.Items.Add(&quot;処理2&quot;)
        ListBox1.Items.Add(&quot;処理3&quot;)
    End Using
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">vb</span>
<pre class="hidden">Public Class MyComp1
    Implements IDisposable            &larr;④
 
    Private myList As IList            &larr;⑤
 
    Public Sub New(ByVal list As IList)    &larr;⑥
        myList = list
        myList.Add(&quot;MyComp1.初期化&quot;)
    End Sub
 
    Public Sub Dispose() Implements IDisposable.Dispose    &larr;⑦
        myList.Add(&quot;MyComp1.後処理&quot;)
    End Sub
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">vb</span>
<pre class="hidden">Using scope As New TransactionScope()    &larr;①
    Dim status As Integer
 
     :(何らかのトランザクションに係る制御)
 
    If status = 1 Then Return        &larr;②
 
     :(何らかのトランザクションに係る制御)
 
    If status = 2 Then Return        &larr;③
