...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 2 月 5 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、クラスにフィールドを追加したとき、『あとでプロパティに置き換えればいいよ』と言われたんですが、置き換えてみたらコンパイル エラーになってしまいました . . .」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「なるほど」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「本当に置き換えて良いものなんでしょうか? もしかして、、、若造だからという理由で嘘を教えられたのでしょうか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「そうとも言えないのう。そういうテクニックは確かにあるのじゃ。しかし、それは万能ではないということじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">なぜプロパティを使うのか?</a> </li><li><a href="#02">プロパティのメリット</a> </li><li><a href="#03">参照渡しの問題</a> </li><li><a href="#04">回避する方法</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. なぜプロパティを使うのか?</h2>
<p>プロパティという機能は、C# が誕生する以前から使用されてきました。では、なぜプロパティという機能が生まれたのでしょうか? それは、カーソルを「待て (WAIT)」を表す砂時計などに変更する以下のコードを見れば明らかでしょう。</p>
<p>プロパティを使用した例 (C#):</p>
...
<pre class="hidden"><code class="csharp">Cursor = Cursors.WaitCursor;</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">Cursor = Cursors.WaitCursor;</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>プロパティを使用しない例 (C&#43;&#43; と Win32 API):</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">SetCursor(LoadCursor(NULL,IDC_WAIT));</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">SetCursor(LoadCursor(NULL,IDC_WAIT));</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>これらは、見ていただいておわかりの通り、以下の相違があります。</p>
<ul>
<li>後者の方が長く回りくどい </li><li>後者の方が括弧が多い </li><li>変更しているという意図が前者の方が分かりやすい </li></ul>
<p>つまり、前者には長所があります。しかし、容易には実現できません。カーソルとは機能であり、変数ではないからです。制御するためには関数を呼ぶ必要があります。そのため、実は、初期の Visual Basic では、プロパティとは「システムが提供する機能を読み書きの構文を通して呼び出す」という仕組みだけがあり、自分でこの機能を定義するには C/C&#43;&#43; で込み入ったコードを書く必要がありました。しかし、&quot;使う&quot; だけでは十分ではないため、誰でも &quot;作る&quot; ことができる機能として .NET Framework では標準で用意されることとなりました。</p>
<p>この自分で定義可能なプロパティのアイデアは簡単です。</p>
<ul>
<li>書き込みが行われると変数に書き込む代わりに指定のコード (関数) を実行する </li><li>読み出しが行われると変数から読み出す代わりに指定のコード (関数) を実行する </li></ul>
<p>例えば、C# で実際に記述してみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>123</p>
</div>
<p>この場合、A がプロパティです。A では、隠蔽されて外部からアクセスできないフィールド a を読み書きするという機能を与えられていますが、A には当然、自由なコードを書くことができます。get は return で目的の値を返し、set は暗黙的に用意された value という変数を処理すれば良いことになります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16757-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. プロパティのメリット</h2>
<p>上記では、「コードの理解性」という観点でプロパティを説明しましたが、プロパティを使用するメリットはいろいろあります。</p>
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ 読み書きする方向を強制する</strong></dt><dd>上記では set/get を双方記載しましたが、set 句と get 句は片方だけ書くこともでき、その場合は読み出し専用、書き込み専用のプロパティとなります。通常の変数 (プロパティではない変数) も readonly キーワードを使用することで読み出し専用にすることは容易ですが、書き込み専用はプロパティでのみ実現できます。</dd></dl>
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ 変数ではない実体を読み書きする</strong></dt><dd>実際には「変数」として存在していない実体を読み書きできます。例えば、システムが持っている状態を API 呼び出しで変更する手順を提供するラッパとして記述できます。</dd></dl>
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ 違う型で保存する</strong></dt><dd>以下は、実際に保存される型は string であるのに、外部からはまるで int の情報が保存できるかのように見えます。</dd></dl>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
{
    private string a;
    public int A { get { return int.Parse(a); } set { a = value.ToString(); } }
...
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ 値のチェック (妥当性検&#35388;など) を入れる</strong></dt><dd>例えば、型としては int であるが、必ず 1 から 26 までの数字しか扱わないとわかっている場合、以下のように記述して判定することができます。</dd></dl>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>(例外が発生して Console.WriteLine は呼び出されません)</p>
</div>
<p>ちなみに、この場合はビルド方法に関係なく例外が起きますが、デバッグ ビルドの場合のみ判定する場合は Dubug.Assert メソッドを使います。</p>
<p>ブレーク ポイントを設定する場合などもプロパティは有利です。ある値の書き込みが発生したときに止まるブレーク ポイントも、set 句に対して容易に指定できます。</p>
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ 仮想 (virtual) にできる</strong></dt><dd>プロパティは virtual キーワードなどを付加して仮想にすることができます。仮想プロパティ (virtual property) は当然継承によって置き換えることができ、動作を変更できます。また、純粋に仮想な抽象プロパティ (abstract property) を継承して実装することもできます。以下は、その例です。</dd></dl>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
{
    private int a;
    public override int A { get { return a; } set { a = value; } }
}
...
<dl style="padding:10px"><dt style="margin-bottom:10px"><strong>・ アクセサとして使用する</strong></dt><dd>クラスの構造に変更が発生した場合、それを使用する周辺のクラスへの影響を最小化するため、「クラスのフィールドを外部にありのまま見せるのは良くない」という考え方があります。これを回避するためにフィールドを public/internal にせずに、代わりにアクセサと呼ばれるメソッドを作成して、このアクセサを通してフィールドにアクセスするという方法が使用されることがあります。この場合、C# では、実際には「メソッド」が使用されるのではなく、機能的に&#20284;ている「プロパティ」が使用されることになるでしょう。</dd><dd style="margin-top:5px">この C# の方法では、フィールドとプロパティの相違は非常に小さくなり、これを利用する側のソース コードはほとんど同じになります。以下のソース コードで、A の利用と B の利用では、その使用方法に差がないことがわかります。</dd></dl>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>従って、アクセサを &quot;付ける/付けない&quot; で大幅に構文が変化してしまうようなプログラミング言語では、「最初から必ずアクセサを付ける習慣を身に付けておきましょう」という原則が提唱されることがあります。しかし、上記のように、利用する構文がほぼ同じ C# では、「必要とされるまでアクセサを付ける行為を延期させてもよい」と考えることができます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16758-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 参照渡しの問題</h2>
<p>以下のようなソース コードを記述したとします。これは、価&#26684; (Price) の情報が入ったあるメンバーに 5% の消費税を加算している例です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Class1
{
    private static void AddTax( ref int a )
    {
        a = a*105/100;
    }
    static void Main(string[] args)
    {
        var c = new Class2() { Price = 100 };
        AddTax(ref c.Price);
...
<p>ここで、上記で述べた「アクセサは必要になるまで付けることを延期できる」という原則により、Price をフィールドからプロパティに書き換えてみました。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">class Class2
{
    private int price;
    public int Price { get { return price; } set { price = value; } }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">class Class2
{
    private int price;
    public int Price { get { return price; } set { price = value; } }
}</code></pre>
...
<div style="margin:20px 0px; padding:10px; background-color:#efefef; border:solid 1px">
<p>コンパイル結果:</p>
<p>エラー 1 プロパティ、インデクサー、または動的メンバー アクセスを out または ref のパラメーターとして渡すことはできません。</p>
</div>
<p>すると、上記の通り、突然コンパイル エラーになります。問題点は、ref による参照渡しです。エラー メッセージの通り、プロパティは参照渡しできません。しかし、通常のフィールドは参照渡しできます。その結果が、冒頭でジョシュアが言っていた「フィールドをプロパティに変更したらコンパイル エラーが出た」の正体です。フィールドとプロパティの両者は、それを使用するソース コード上ではそっくりに見えますが、まったく同じものではないため、微妙な制約の影響を受け、上記の例以外にも結果が同じにならないケースがいくつかあります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16759-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 回避する方法</h2>
<p>この問題を回避する方法がいくつかあります。</p>
<ul>
<li>プロパティをやめ、値をセットするデリゲートと値を得るデリゲートを渡すメソッドにソースを直す。なお、かなりの大工事を必要とします . . . </li><li>最初から参照渡し (ref や out) などの非互換の機能を使わないようにする </li><li>あとでプロパティにするかも知れないフィールドは最初からプロパティにしておく </li></ul>
<p>この中で最も簡単で現実的な対策は最後の 1 つでしょう。このような場合、C# では、コンパイラが暗黙的にデータを保存する実体を用意する方法で、プロパティの定義をとても簡潔に実装できます。以下は実際に記述した例です。(コンパイルをおこなうと、内部では保存する処理の実体は用意されますが、下記の通り、ソース コード上には見えていません。リフレクションを使って実行時に見ることはできます。)</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>最初からこのような記述を心がけておけば、非互換に関する問題は最初から分かるので、「プロパティに書き直したらコンパイル エラーになった」という問題は無くなります。ちなみに、これは、前述した「純粋仮想のプロパティ」ではないので注意してください。内部的には、コンパイラーがプロパティの処理の実装の実体を自動的に用意しています。(これは、「自動実装プロパティ」と呼ばれます。)</p>
<p>現実のプログラミングでは、書き直して (プロパティに変更して) エラーになるというのは非常に希なケースです。このため、特に影響範囲が問題にならないならば、現実には「エラーが起きたらそこを直すと割り切った、ノーガード戦法」もありでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16760-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「確かに。調べたら、フィールドを参照渡し (ref) を使ったメソッドに渡してました!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。それはエラーになるケースじゃが、けっこうレアなケースじゃな」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「はい。他のフィールドはプロパティに書き換えても平気でした」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
...
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ともかく影響する部分だけソースを直して乗り切りたいと思います。ありがとうございました!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「不完全ではあっても、コードの書き換えが少ないこの方法は有効なテクニックとして使用される、ということじゃな!」</td>
</tr>
</tbody>
</table>
