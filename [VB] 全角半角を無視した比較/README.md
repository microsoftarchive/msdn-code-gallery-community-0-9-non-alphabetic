...
<hr>
<p><a href="http://msdn.microsoft.com/ja-jp/library/s1wwdcbf(VS.80).aspx">System.String</a> クラスはテキストを Unicode 文字として表現し、更に文字列の操作や比較など有用な静的メソッドが多数用意されています。例えば 2 つの文字列の比較を行う場合は
<a href="http://msdn.microsoft.com/ja-jp/library/84787k22(VS.80).aspx">String.Compare</a> メソッドを使用します。このメソッドは文字列の違いを integer 型の値で返すため、同じ文字列と判定された場合は 0 を返します。</p>
...
<pre id="codePreview" class="vb"><code class="csharp">Imports <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Globalization.aspx" target="_blank" title="Auto generated link to System.Globalization">System.Globalization</a>

Module Module1

    Sub Main()
        Dim str1 As String = &quot;MSDNライブラリ CodeRecipe&quot;
        Dim str2 As String = &quot;MSDNライブラリ CodeRecipe&quot;


        Console.WriteLine(&quot;文字列1: {0}&quot;, str1)
        Console.WriteLine(&quot;文字列2: {0}&quot;, str2)

        Dim diff As Integer = String.Compare(str1, str2)

        If diff = 0 Then
            Console.WriteLine(&quot;同一の文字列です&quot;)
        Else
...
<p>上記コードの実行結果は次の通りです。</p>
<div style="margin:20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>文字列1: MSDNライブラリ CodeRecipe<br>
文字列2: MSDNライブラリ CodeRecipe<br>
同一の文字列です</p>
</div>
<p>日本語を取り扱う際、全角英数字については半角英数字とは別文字として認識されるため、st1 を &quot;ＭＳＤＮﾗｲﾌﾞﾗﾘ ＣｏｄｅＲｅｃｉｐｅ&quot; と変更した場合は次のような結果となってしまいます。</p>
<div style="margin:20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>文字列1: ＭＳＤＮﾗｲﾌﾞﾗﾘ ＣｏｄｅＲｅｃｉｐｅ<br>
文字列2: MSDNライブラリ CodeRecipe<br>
異なる文字列です</p>
</div>
<p>日本人としては上記のような場合も同じ文字列として判断したい状況がありますが、String.Compre メソッドのオーバーロードで判定に使用するカルチャ情報並びに判定条件を指定できるものが用意されているので、こちらを使用することで全角、半角を無視した文字列の比較が可能です。</p>
<div style="margin:20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>string.Compare(str1, str2, CultureInfo.CurrentCulture, CompareOptions.IgnoreWidth);</p>
</div>
<p>判定条件の列挙体である <a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.compareoptions(VS.80).aspx">
CompareOptions</a> には大文字、小文字やひらがな、カタカナを無視する条件などが用意されています。</p>
<p>全角、半角、大文字、小文字、ひらがな、カタカナを無視し、現在のカルチャからではなく、どの環境においても日本語のカルチャ情報による比較を行うサンプル コードはこちらになります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
Module Module1
    ...
    Sub Main()
        Dim str1 As String = &quot;ＭＳＤＮﾗｲﾌﾞﾗﾘ　ＣｏｄｅＲｅｃｉｐｅ&quot;
        Dim str2 As String = &quot;MSDNらいブラリ CodeRECIPE&quot;
        ...
        Dim diff As Integer = String.Compare(str1, str2, ci, CompareOptions.IgnoreWidth Or CompareOptions.IgnoreKanaType Or CompareOptions.IgnoreCase)
...
<h3>関連リンク</h3>
<ul>
<li><a href="http://msdn.microsoft.com/ja-jp/library/s1wwdcbf(VS.80).aspx">String クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.compareoptions(VS.80).aspx">CompareOptions 列挙体</a>
</li></ul>
