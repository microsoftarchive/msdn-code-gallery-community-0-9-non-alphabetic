...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">var a = 123;</code></pre>
...
<p>しかし、これは杞憂です。なぜなら、C# は型を明示するのが原則であり、var によって宣言された変数は、決して何でも入る「魔法の箱」にはなりません。「推論」により型を決定するだけで、型が 1 つに決まることには代わりがないのです。このため、int ではなく var に書き換えても、やはり、上記の a.GetType().FullName の結果は System.Int32 のままです。</p>
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">Dictionary&lt;int, string&gt; dic = new Dictionary&lt;int, string&gt;();</code></pre>
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">var dic = new Dictionary&lt;int, string&gt;();</code></pre>
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">var array = new [] { 1, 2, 3 };</code></pre>
...
<p>C# で使用できるコレクションには、型を明示しないもの (System.Collections) と、型を明示するもの (System.Collections.Generic) の 2 種類があります。実際、C# 2.0 で後者 (ジェネリック) がサポートされたあとは、型を明示しない前者のコレクションはほとんど使用されなくなりました。以下でこの 2 つを比較してみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">// 型を明示しない場合:
using System;
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">// 型を明示する場合:
using System;
