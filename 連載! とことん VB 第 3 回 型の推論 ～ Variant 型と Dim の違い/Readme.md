...
<h2>目次</h2>
<ol>
<li><a href="#01">はじめに</a> </li><li><a href="#02">Visual Basic 2008 から導入された Dim 文での「型の推論」</a> </li><li><a href="#03">「型の推論」が有効なときと、有効でないとき</a> </li><li><a href="#04">「型の推論」の用途</a> </li></ol>
<hr>
<h2 id="01">1. はじめに</h2>
<p>.NET より以前の Visual Basic 6.0 などで変数を宣言する場合、次の例の①のように As 句を使用して型を明示できたほか (ここでは As Integer)、②のように As 句を省略することができました。そして、②の場合、変数 y は汎用的なデータ型である Variant 型であるとみなされました。</p>
<p><strong>例 1. VB 6.0: 変数の宣言</strong></p>
...
<pre class="hidden"><code class="csharp">Dim x As Integer	&larr;① Integer型として変数xを宣言
Dim y			&larr;② 暗黙的にVariant型として変数yを宣言</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Dim x As Integer	&larr;① Integer型として変数xを宣言
Dim y			&larr;② 暗黙的にVariant型として変数yを宣言</code></pre>
</div>
</div>
...
<p>同様に、.NET 版の Visual Basic でも As 句を省略することができます。しかし、その扱いは VB 6.0 とは異なりますので注意してください。特定の状況下で As 句を省略すると、「型の推論」と呼ばれる機能が作用します。ここでは、この As 句を省略した際の「型の推論」を改めて取り上げましょう。この As 句の省略ですが、一見すると構文のバリエーションの些細な 1 つにしか思えないですが、実は、れっきとした使い道もあり、今回、その点も確認していきましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16957-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. Visual Basic 2008 から導入された Dim 文での「型の推論」</h2>
<p><br>
前述の例 1 は、VB.NET でも有効な構文です。ただし、VB.NET において例 1 の②のように As 句を省略した場合では、変数 y は汎用的なデータ型である Object 型になります。つまり、暗黙的に「As Object」の意味になります。しかし、これだけなら Variant 型が Object 型へ単に置き換わっただけの話ですが、次の例 2 のように変数の値に対して明示的な初期化を伴う場合は、As 句の省略が異なる意味を持ちます。</p>
<p><strong>例 2. VB.NET: 変数の宣言における明示的な初期化</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Dim x As Integer = 3	&larr;①
Dim y = 3		&larr;②
Dim z = 10, s = &quot;ABC&quot;	&larr;③</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Dim x As Integer = 3	&larr;①
Dim y = 3		&larr;②
Dim z = 10, s = &quot;ABC&quot;	&larr;③</code></pre>
</div>
</div>
...
<p>この例 2 のような変数宣言時に特定の値で初期化する表記方法は、VB 6.0 では構文エラーになりますが、VB.NET では有効な構文です。このような初期化を伴う場合は、As 句を省略した際には、「As Object」の意味とはなりません。もっと正確に言うと、Visual Basic 2008 からは「As Object」の意味ではなくなりました。</p>
<p>実際、Visual Basic .NET 2002 から Visual Basic 2005 までは、例 2 の②や③のように、As 句を省略した場合は「As Object」の意味になりました。しかし Visual Basic 2008 からは、設定される初期値のデータ型に応じて、変数のデータ型もそれに合うように、コンパイルの時点で変数の型が決定されます。この仕組みは「型の推論」と呼ばれています。</p>
<p>たとえば、例 2 の②の場合では、「3」が初期値として設定されています。言語仕様では、小数点を持たない数値のリテラル (定数) は、Integer 型に収まる範囲の値は Integer 型とみなされ、それ以上は Long 型とみなされるので、この「3」は Integer 型となります。(よって、変数 y も Integer とみなされます。) つまり、②では「As Integer」の意味と同等になります。同様に、③の変数 z は Integer 型になり、変数 s は &quot;ABC&quot; を設定しているので String
 型になります。</p>
<div style="margin:10px auto; padding:10px; background-color:#efefef; border:solid 1px #333333; width:550px">
<p><strong>Note:</strong> Visual Basic 2008 では、新しいコンパイル オプションとして、Option Infer が導入されました。既定構成では「Option Infer On」に設定された状態であり、この状態では型の推論が作用します。このオプションを Off の状態に変更すると、As 句の省略は「As Object」とみなされるようになり、Visual Basic 2005 のときの言語仕様と同様の動作になります。</p>
</div>
<p style="margin-top:20px"><a href="#top"><img src="16958-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 「型の推論」が有効なときと、有効でないとき</h2>
<p>言語仕様上、この「型の推論」によって特定の型に決定できる仕組みが利用できるのは、ローカル変数のみである点に注意してください。次の例では様々な変数や引数が定義されていますが、実は、この仕組みが利用できるのは④の Dim 文による動的なローカル変数宣言だけであり、④の場合のみ Integer 型になります。</p>
<p><strong>例3. VB.NET: 様々な変数定義</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre class="hidden"><code class="csharp">Public Class Form1
    ...
    Private Function Func1(ByVal x As Integer, Optional ByVal y = 10)
				&larr;⑥
        Return 20		&larr;⑦
    End Function
...
<div>
<p>たとえば、Form 1 クラスの中に定義された①から③のメンバ変数では、型の推論による型の特定は出来ません。これらはすべて「As Object」とみなされます。同様に⑤のように、ローカルな変数であっても、それが静的 (Static) な場合には、「As Object」の意味になります。</p>
<p>また、⑥のようなプロシージャ定義において、オプション引数 y では既定値として 10 が設定され、As 句が省略されていますが、この引数 y も Object 型です。さらに⑦のように戻り値として 20 を返し、⑥の定義では戻り値の型が As 句で明示されていませんが、この Function プロシージャの戻り値も Object 型です。</p>
<div style="margin:10px auto; padding:10px; background-color:#efefef; border:solid 1px #333333; width:550px">
<p><strong>Note:</strong> ただし、例 3 のコードは、コンパイル オプションが既定構成であることが前提です。具体的には、Option Strict Off Option Infer On の状態です。Option Strict On に変更した場合は、型の推論が可能な④の部分を除き、 As 句の省略は不正構文として扱われコンパイル エラーになります。</p>
</div>
</div>
<p style="margin-top:20px"><a href="#top"><img src="16959-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 「型の推論」の用途</h2>
<p>それでは、このような型の推論は、どのような用途で使うのでしょうか。一見すると、型の推論を使用しなくとも、単に「As 句」を書けば済むだけの感じもします。しかし状況によっては、この型の推論を使用したほうが便利だったり、また、使う必要性が生じたりするのです。使うべきシナリオとして、主に次の 2 つの状況が考えられます。</p>
<ol style="list-style-type:upper-alpha">
<li>型の表記が複雑な場合 </li><li>匿名型を使用している場合 </li></ol>
<p>まず、[A] のケースに当てはまる例として、次のような LINQ のクエリ式を使用した例を挙げることができます。LINQ は別の機会に取り上げるので、ここでは型の推論にフォーカスして考察することにします。</p>
<p><strong>例 4. VB.NET: LINQ のクエリ式による配列の操作</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Dim data(99) As Integer	&larr;①

    : (省略)

Dim query = From d in data Where d &gt;= 30	&larr;②
For Each d in query		&larr;③

    : (省略)

Next</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Dim data(99) As Integer	&larr;①
...
<div>
<p>この例の①では、Integer 型の要素 100 個からなる配列 data が宣言されています。ここでは、配列にどんな処理を加えたかは重要でないので、その配列に対する処理の記載は割愛します。</p>
<p>ポイントは②の部分です。②では変数 query が定義されていますが、As 句は省略されています。代わりに、初期値として「From d in data Where d &gt; 30」という LINQ の「クエリ式」が設定されています。このクエリ式は、配列 data の中から値が 30 以上の要素を抽出するための、一種のコマンドです。このコマンドが変数 query に代入されています。このコマンドによる結果の型は次のようになっており、型の推論によって、変数 query も (Object 型ではなく)
 この型になります。</p>
<ol style="list-style-type:none">
<li>IEnumerable(Of Integer) </li></ol>
<p>詳しい説明は省きますが、この変数 query で表わされるクエリ式は「Integer 型の要素の集合」を返すコマンドであり、「Of Integer」の部分が要素の「型」を示していると考えてください。</p>
<p>また、クエリ式の書き方やクエリ対象のデータの種類によって、この変数 query の型は異なります。場合によっては、以下のものになることもあれば、これ以外の場合もあります。</p>
<ol style="list-style-type:none">
<li>IQueryable(Of Integer) </li></ol>
<p>つまり、 As 句を明示的に記述するためには、クエリ式の書き方や使用するデータの種類に応じて、どんな型になるのか、いちいち覚えておく必要があります。</p>
<p>典型的なクエリ式の使用方法の1つは ③の For Each ループなどでの使用であり、変数 query の型が何であろうが、その操作方法は透過的に同じ場合が多くあります。そういう意味で、変数 query の定義において、As 句で型を明示的に記述しても、それほど効能は見い出せません。</p>
<p>これらを鑑みると、As 句を省略して、型の推論を使用するほうが便利かつ簡潔であると言えるでしょう。</p>
<p>なお、この [A] のケースでは型の推論が必須なのではなく、もちろん As 句を書いてもかいません。しかし、次に述べる [B] のケースでは As 句の省略が必須 (省略する必要がある!) になります。次に [B] のケースについて考えてみましょう。</p>
<p>[B] の例としては、次のような「匿名型」を使用する例が挙げられます。</p>
<p><strong>例 5. VB.NET: LINQ のクエリ式での匿名型の使用</strong></p>
</div>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Dim rects(99) As Rectangle	&larr;①

   : (省略)

Dim query = From r In rects Where r.Width &gt;= 100 _	&larr;②
            Select r.Width, r.Height, Area = r.Width * r.Height	&larr;③</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Dim rects(99) As Rectangle	&larr;①
...
<div>
<p>①の部分では、Rectangle オブジェクト 100 個の集合を成す配列 rects を定義しました。②から③の 2 行にまたぐクエリ式では、この Rectangle オブジェクトの Width プロパティの値が、100 以上のものを抽出しています。しかし、抽出したオブジェクトは Rectangle オブジェクトのままではなく、③の部分で、抽出後の新しいオブジェクトの形式を定義しています。すなわち③では、カンマ区切りでオブジェクトのプロパティを列挙し、Width プロパティ、Height プロパティ、および
 Area プロパティからなる新しいオブジェクトを定義しています。</p>
<p>オブジェクトを定義するということは、何らかの名前が付いた新しいクラスを定義することです。実は、③の記述によって、コンパイラは任意の名前を付けた新しいクラス定義を出力するのです。このクラスを「匿名クラス」と言います。より一般的な言い方としては「匿名型」と言います。結局、②から③のクエリ式では、匿名型のオブジェクトを要素とする集合を返すことになります。<br>
このとき、②の変数 query の型はどうなるでしょうか。次の形式になるはずですが、「Of 名前」の部分の「名前」が特定できません。</p>
<ol style="list-style-type:none">
<li>IEnumerable(Of 名前) </li></ol>
<p>というのは、この匿名型の実際の名前は、コンパイルするまで確定しないので、プログラマーがコンパイル前のソース コード上でその名前を参照することが出来ないのです。よって、②から③のような匿名型のオブジェクトを返すような場合、変数 query の宣言では、As 句を省略して型の推論を &quot;必ず&quot; 利用することになるのです。</p>
<p><br>
以上、型の推論の典型的な 2 つの用途を確認しました。ここで取りあたげ 2 つのケースは、どちらも LINQ のクエリ式で使用されています。その意味で、Visual Basic 2008 で新たに LINQ が採用されたのとあわせて「型の推論」も導入されたということは、自然であると言うことが実感して頂けるでしょう。</p>
</div>
