# 連載! とことん C#: 第 8 回 複数の値の返し方 (参照渡しとパック)
## Requires
- 
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 2 月 15 日</span></h4>
</div>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「生徒のジョシュアです。ところで、メソッドが return で戻せる値は、普通なら 1 個ですよね?」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「その通りじゃ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「2 つ以上戻したい場合はどうするのですか? 名前と年齢のどちらも返すメソッドを書いているのですが」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「実はやり方は多いのじゃ...」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">参照渡し! 返すだけなら...</a> </li><li><a href="#02">パックしてから返そう</a> </li><li><a href="#03">パフォーマンスに配慮した実装</a> </li><li><a href="#04">可読性に配慮した実装 (匿名型と dynamic!)</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. 参照渡し! 返すだけなら...</h2>
<p>メソッドから複数の値を返したいとき、一般的に参照渡しがよく使われます。</p>
...
<p>実行結果:</p>
<p>Name=Bill Age=18</p>
</div>
<p>この通り、2 つの値 (名前と年齢) を返すことができます。引数を参照渡しにすれば、いくらでも値を返すことができます。そのためには、仮引数と実引数に ref キーワードを付けて参照渡しを指定するだけです。これを参照パラメーターと呼びます。それに対して、値を渡す方式を値パラメーターと呼びます。</p>
<p>しかし、このプログラムには 1 つだけ不満があります。それは、変数 Name と Age を後で上書きされる値 (上記の場合は、空文字と 0) で初期化していることです。この値は一切使われないにも関わらず、これを書かねばコンパイル エラーになります。値型における「参照渡し」は、別の場所にある変数の場所を教えることでその値を覗くことであり、その場所を確保するために、変数は常に初期化しないといけないからです。</p>
<p>こういう場合、必ずメソッド内で値を入れることをコンパイラに教えれば、変数を初期化する上記の =&quot;&quot; や =0 を免除することができます。そのためには、ref ではなく out キーワードを使うだけです。これを出力パラメーターと呼びます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>しかし、良いことばかりではありません。out キーワードを指定すると、今度は、参照された変数に値を入れることが義務化されます。「処理に応じて値を入れることもあれば入れないこともある」という使い方には馴染みません。その場合は、ref キーワードを使うことになります。つまり、どちらがよりよいという話ではなく、必要に応じて使い分けることが重要です。</p>
<p style="margin-top:20px"><a href="#top"><img src="16777-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. パックしてから返そう</h2>
<p>この参照渡しは、返す値ごとに引数が増えていくことになり、ref や out というキーワードの必要性と相まって長くなりがちです。何とか短くする方法はないでしょうか?</p>
<p>さて、メソッドの戻り値 (return 値) は 1 つですが、複数の値を 1 つにまとめた「型」があれば、これを 1 つの結果として返すことで容易に返却できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(&quot;Name={0} Age={1}&quot;, packed.Name, packed.Age);
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>この場合、返す値の数が増減しても、クラス定義を直すだけで対処できるという特徴があります。メソッドのシグネチャは変化しません。これで、まずは、シンプルなシグネチャのメソッドで複数の値を返すことができました。</p>
<p style="margin-top:20px"><a href="#top"><img src="16778-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. パフォーマンスに配慮した実装</h2>
<p>クラスは、値のようにスタック上にメモリを確保するのではなく、動的にメモリを割り当て、かつガベージ コレクション の対象となるため、こうした余計なクラスがソース コード上に増えてきた場合 (あるいは、こうした処理を多量に反復するような場合)、メモリ確保の観点でオーバーヘッドの高い処理となる点をおぼえておいてください。</p>
<p>この動的なメモリ確保の時間をカットするには、C# では構造体を使用することができます。上記の例を構造体で置き換えると以下の通りになります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">struct NameAndAge
{
    internal string Name;
    internal int Age;
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">struct NameAndAge
{
    internal string Name;
    internal int Age;
}</code></pre>
...
<p>しかし、この構造体を使用した方法も、受け渡すデータが多いと、使用していないデータも含むすべての実体 (メモリ領域) を受け渡すため、使い方によっては一概に速くならない場合があるので注意してください。(最適化の際は、実際の利用シナリオに応じてテストをおこなってみてください。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16779-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 可読性に配慮した実装 (匿名型と dynamic!)</h2>
<p>さて、クラスにせよ構造体にせよ、そもそも定義しないと使えないのでは、手間が増えて面倒です。この場合、C# 3.0 (Visual Studio 2008) で使用可能な匿名型により、定義抜きでオブジェクトを生成できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(&quot;Name={0} Age={1}&quot;, 
          packed.GetType().GetProperty(&quot;Name&quot;).GetValue(packed, null), 
          packed.GetType().GetProperty(&quot;Age&quot;).GetValue(packed, null));
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>getNameAndAge の内部に書かれているのが匿名メソッドです。名前がありませんが、複数の値を含むオブジェクトを生成できます。これで戻したい値を集めた型を宣言するという手間から解放されました。</p>
<p>しかし、この返り値は、型の名前が無いので名前を指定して返すことができず、返す場合は何でも入る object 型としておき、読み出す際は、上記の通りリフレクションを使用し、クラスのプロパティを解析して読み出すことになります。</p>
<div style="margin:10px 0; padding:10px 10px 0 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>注記:</strong> リフレクションを使用すると、実行時に (動的に) 型の情報を解析してメンバーにアクセスすることができます。「リフレクション」については、今後の連載で詳細を説明する予定です。<br>
なお、一般に、匿名型ではメンバーへのアクセスが可能ですが、「名前を持たない型」(内部的に自動生成される型) のため、今回の例では、いったん object 型に変換する必要があり、リフレクションによるメンバーへのアクセスが必要です。匿名型を受け取るためによく使用される「var」は、「var 型」ではなく、ローカル変数の型を推測するようコンパイラに指示するキーワードです。</p>
</div>
<p>Visual Studio 2010 で使用可能な C# 4.0 であれば、このリフレクションの処理は大幅に簡単になります。dynamic 型については、いずれ本連載で詳細を説明する予定ですが、まずは、 この dynamic 型を使用した以下の例を見てみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この例が示すように、dynamic 型を使用すると、型が実行時に決定されるため、複雑なリフレクションの式が消えて無くなり、シンプルに記述できるようになりました。</p>
<p>しかし、前述の構造体 (struct) の例とは対象的に、以下の点に注意する必要があります。</p>
<ul>
<li>プロパティの存在チェックが、コンパイル時ではなく実行時に遅延されます </li><li>通常のクラスの使用と比べ、厳密な意味でオーバーヘッドは大きくなります </li></ul>
<p>これらの弱点さえ心得て使えば、「ソースコードをシンプルにする」という観点では、強い武器になるでしょう。呼び出す回数が多くなく、カバレッジ率 100% の単体テストが完備していれば、試してみる価値のある方法の 1 つです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16780-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
<table border="0" cellspacing="0" cellpadding="0" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました! つまり絶対不変な正解はないけど、使用する場面に応じて方法がいろいろ使えるわけですね?」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。重さ、軽さ、使い勝手など様々じゃ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「選ぶ自由と責任は僕たちにあるわけですね。ありがとうございます!」</td>
</tr>
</tbody>
</table>
