# 連載! とことん C#: 第 13 回 名前空間とモジュールの分割 (using, extern)
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 3 月 5 日</span></h4>
</div>
...
<tbody>
<tr>
<td valign="top" style="width:16%; padding:10px">センセーション</td>
<td valign="top" style="padding:10px">「おほん、教師のセンセーションである」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、せっかく作ったクラスの名前が別の人のクラスと衝突してしまいました。でも、分かりやすさから考えて名前を変えたくありません。こういうときは名前空間を使うんでしたよね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。名前空間でも良いが、アセンブリで分けるという方法もあるのじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「モジュールのことですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「いや、本来アセンブリとモジュールは別の概念なのじゃよ!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
...
<h2>目次</h2>
<ol>
<li><a href="#01">名前空間と名前の短縮</a> </li><li><a href="#02">名前の別名 (alias)</a> </li><li><a href="#03">アセンブリを分ける</a> </li><li><a href="#04">extern エイリアス</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. 名前空間と名前の短縮</h2>
<p>名前空間は名前の衝突を回避するために使用されます。例えば、以下のプログラムはコンパイルできません。X.SayMyName メソッドが 2 つ存在し、どちらを呼び出して良いかが決まらないからです。</p>
...
<p>しかし、多数のプログラマーがさまざまな機能を開発するような場合、名前が完全に重複しないようにすることは事実上むずかしいでしょう。例えば、モデリング ツールを開発しているチームで Activity クラスを作成し、ワークフロー ツールを開発しているチームでも Activity クラスを作成するといった場合です。</p>
<p>そこで、言語が持っている「名前空間」の機能を活用してこれを回避します。上記のサンプル コードは、以下のように記述すれば回避できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>名前空間も同じになってしまった場合には、無論、エラーになります。また、ネストした名前空間 (例えば、A の中の B) と、A.B という名前空間は同じと見なされるため、下記も、class X は同じクラスと見なされて、1 つのソース コード上に共存できません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">namespace A
{
    namespace B
    {
        class X { }
...
<p>前述の例のように名前空間を常に明示すると、当然、ソース コードが長くなります。そこで、ソース コードでよく見かける using ディレクティブを使って名前を短縮する行為が行われます。よくサンプル コードに出てくる Console.WriteLine は、本来なら <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Console.WriteLine.aspx" target="_blank" title="Auto generated link to System.Console.WriteLine">System.Console.WriteLine</a> ですが、先頭にある using System のおかげで System 名前空間を省略できます。</p>
<p>using ディレクティブのこの使い方は、単に「名前を短くすることができる」という意味しかありません。そのため、「using ディレクティブで明示するから呼び出せる」、「using ディレクティブで明示しない名前空間は呼び出せない」という話ではありません。</p>
<p>また、本質的に「名前の衝突」を解決する機能は、名前空間を含めたフルネームで書くという方法しかありません。例えば、クラス名が同じ 2 つの名前空間のクラスを使用する場合には、結局、この方法では解決できません。</p>
<p style="margin-top:20px"><a href="#top"><img src="16586-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 名前の別名 (alias)</h2>
<p>using ディレクティブは、他に 2 つの使い方があります。拡張メソッドの有効性を切り替えるスイッチ (今回は、この話については割愛します) と、名前の別名 (alias) です。</p>
<p>名前の別名は、名前空間やクラスの名前に別名を付けることができる仕組みです。以下は、<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Console.aspx" target="_blank" title="Auto generated link to System.Console">System.Console</a> に C という別名を付けて使用した例です。つまり、別の短い名前に別名を付けてしまうことで、「同じ名前の衝突を防ぐ」、「短く書く」という 2 つの目的を同時に満たせます。(下記のように、名前空間だけでなく、クラス自体にも別名が適用できます。)</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">using C = <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Console.aspx" target="_blank" title="Auto generated link to System.Console">System.Console</a>;
...
<p style="margin-top:20px"><a href="#top"><img src="16587-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. アセンブリを分ける</h2>
<p>さて、実行するプログラムの内容を異なる実行ファイル (.dll ファイルや .exe ファイル) として 2 つに分けるには、以下の 2 つの手順を踏むだけです。</p>
<ul>
<li>クラス ライブラリ (DLL) のプロジェクトを作成する (同じソリューションの中でも外でも構いません) </li><li>その DLL を参照する </li></ul>
<p>これは、Windows フォーム アプリケーションでもコンソール アプリケーションでも Web アプリケーションでも同じです。ただし、このように 2 つに分けることにより、アクセス範囲の制限を受けます。例えば、以下のコードはコンパイルできません。 (class Program とstatic class Class1 は 1 つのプロジェクトから 1 つの実行ファイルにしたときはコンパイルできますが、2 つに分けるとコンパイルできません。)</p>
<p><strong>実行ファイル (.exe) 側:</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このプログラムには、以下の 2 つの問題があるためです。</p>
<ul>
<li>Class1 に public が付いていないので internal 扱いになっている </li><li>SeyHello も public ではなく internal になっている </li></ul>
<p>うっかりアクセス修飾子 (public、internal、private など) を書くことを忘れると、外部からアクセスできない internal 扱いになります。メソッドが既定で private 扱いになる点とは異なります。つまり、たった 1 つの小さなプロジェクトで試している場合はアクセスできるのに、2 つに分けた瞬間にアクセスできなくなります。</p>
<p>internal の問題は、逆に考えれば有効なアクセス制御の手段となります。何でも public を付けるのではなく、あるアセンブリの内部でしか使用しないメソッドは、積極的に public ではなく internal を付けていくべきでしょう。外部に対するサービスを提供する場合だけ public を使い、internal の使用を基本とすべきでしょう。こうすることで、本来必要なものだけが公開され、インテリセンスの候補も不必要に増やさず選びやすくなります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16588-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. extern エイリアス</h2>
<p>実際の開発では、名前空間まで衝突しているケースは滅多にありません。ほとんどは、ここまで紹介した方法で対処できます。しかし、外部ベンダーが作成したアセンブリなど、バイナリーで供給される複数のライブラリ間で競合が発生してしまう場合はお手上げです。容易に書き換えて逃げられないライブラリを 2 つ参照して名前が競合してしまうことも、あり得ないとは言えません。</p>
<p>この問題に対処するために用意されたのが「extern エイリアス」という機能です。これは、後述するようにソース コードだけで解決させる方法ではありません。<br>
例えば、まったく同じ名前空間やクラス名を持った 2 つのクラス ライブラリがあったとします。(それぞれは署名をおこなうなどして、別のアセンブリとして構築しておきます。)</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
namespace ClassLibrary
{
  public class Class
  {
    ...
    {
      Console.WriteLine(&quot;in ClassLibrary1!&quot;);	// もう1つは&quot;in ClassLibrary2!&quot;
    }
  }
}</code></pre>
</div>
...
<p>ここまでで紹介した方法では、このソース コードを持つ 2 つのライブラリを同時に使用したコードは記述できません。先ほどの例のように名前空間も含むクラスやメソッドの正式名 (長い名前) を使用しても、名前が競合してコンパイルできないためです。</p>
<p>この名前の衝突を、2 つのクラス ライブラリを修正することなく解決するには、以下の 3 つの手順を必要とします。</p>
<ol>
<li>アセンブリの参照追加をおこない、参照にエイリアス名を設定する </li><li>extern エイリアスの定義を追加する </li><li>extern エイリアスを用いてメソッドを呼び出す </li></ol>
<p>まず、参照にエイリアス名を設定するには、Visual Studio の場合はソリューション・エクスプローラーから参照しているアセンブリのプロパティを開きます。ここで、[エイリアス] に「global」という名前が入っていますが、これをアセンブリを参照するための別の名前に書き換えます (下図)。複数のアセンブリに対して、参照するアセンブリごとに異なる名前を書き込んでおきます。ここでは、「Lib1」と「Lib2」という名前を指定しています。</p>
<p><img src="16589-image.png" border="0" alt="図 1"></p>
<p><strong>図 1 アセンブリのエイリアス設定 (Visual Studio 2010)</strong></p>
<p>次に、2 の extern エイリアスの定義の追加です。これはソース コードの先頭に以下の 2 行を追加します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">extern alias Lib1;
extern alias Lib2;</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>このように、「extern alias」の後に参照プロパティに書き込んだ名前 (上図) を書くだけです。</p>
<p>後は、extern エイリアスを用いてメソッドを呼び出します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">Lib1::ClassLibrary.Class.SayHello(); // 出力：in ClassLibrary1!
Lib2::ClassLibrary.Class.SayHello(); // 出力：in ClassLibrary2!</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>このとき、クラス ライブラリのプロジェクトには何も手を触れていないことに注目してください。設定しているのは、あくまで、参照する側のソース コードと参照プロパティだけです。この方法により、バイナリーを変えることなく、同じ名前のメソッドを呼び出すことができています。</p>
<p style="margin-top:20px"><a href="#top"><img src="16590-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました! つまり、名前が衝突しても、名前を変える方法以外に回避する方法はあるわけですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
...
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、まだアセンブリとモジュールの違いがよく分かりません . . .」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「現実に見かけるほとんどのアセンブリは、モジュールとアセンブリが 1 対 1 で対応しておるから分かりにくいかもしれん。これは、拡張子が.netmodule の複数のモジュールをあわせて 1 つのアセンブリを作成できる仕組みじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、そんな名前の拡張子はほとんど見かけたことがありません。多くは 1 対 1 で、あまり意味がないのですよね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「う～ん、しかし、希に、こうした方法を利用することがある。覚えておいても損はないじゃろう」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「分かりました。とりあえず、どの方法で行けば良いか調べてみます! ありがとうございます!」</td>
</tr>
</tbody>
</table>
<p>(アセンブリとモジュールについては、本連載の<a href="http://msdn.microsoft.com/ja-jp/ff621338.aspx">第 18 回</a>で取り上げます)</p>
