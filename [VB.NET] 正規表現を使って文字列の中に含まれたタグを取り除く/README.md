# [VB.NET] 正規表現を使って文字列の中に含まれたタグを取り除く
## License
...
<hr>
<p><a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Text.RegularExpressions.Regex.aspx" target="_blank" title="Auto generated link to System.Text.RegularExpressions.Regex">System.Text.RegularExpressions.Regex</a> クラスに実装されている Replace メソッドを用いることで正規表現を用いて条件に一致した文字列を指定の文字列に置換することができます。正規表現については<a href="http://msdn.microsoft.com/ja-jp/library/hs600312.aspx">こちら</a>を参考にしてください。</p>
<p>今回は文字列から HTML タグを取り除く方法を紹介します。簡単な HTML タグは正規表現を用いると &lt;.*?&gt; と定義できます。&quot;.&quot; は任意の文字、&quot;*?&quot; は直前の文字 (ここでは任意の文字) の 0 回以上の繰り返しの中で最も短いものを意味します。</p>
...
<pre id="codePreview" class="vb"><code class="csharp">Imports <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Text.RegularExpressions.aspx" target="_blank" title="Auto generated link to System.Text.RegularExpressions">System.Text.RegularExpressions</a>
Module Module1
    Sub Main()
        Dim sampleStr As String = &quot;&lt;title&gt;逆引きサンプル コード&lt;/title&gt;&quot;
        Dim patternStr As String = &quot;&lt;.*?&gt;&quot;
        Dim newStr As String = Regex.Replace(sampleStr, patternStr, String.Empty)
...
<p>コンソール アプリケーションでこのコードを実行すると、</p>
<div style="margin:20px 0px; padding:10px; background-color:#dedfde">
<p>逆引きサンプル コード</p>
</div>
<p>とタグが取り除かれた状態で表示されます。</p>
<p>ただし、このパターン文字列ではタグの内部に改行文字が含まれている場合は正しく判定することができません。この問題に関しては、<a href="http://msdn.microsoft.com/ja-jp/library/system.text.regularexpressions.regexoptions.aspx">RegexOptions.SingleLine</a> を設定し改行文字を通常の文字として扱うことで正しく判定することが可能となります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="vb"><code class="csharp">Console.WriteLine(Regex.Replace(sampleStr, patternStr, String.Empty, RegexOptions.Singleline))</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>Regex クラスは上記のように静的メソッドを使用する他にもオブジェクトをインスタンス化して使用することも可能です。ただし、正規表現エンジンはパフォーマンスを向上させるためにコンパイル済みの正規表現のキャッシュをアプリケーション全体で保持しますが、.NET Framework 2.0 では静的メソッドで呼び出された正規表現パターンのみキャッシュされます。このキャッシュされる正規表現の最大数は
<a href="http://msdn.microsoft.com/library/system.text.regularexpressions.regex.cachesize.aspx">
Regex.CacheSize</a> にて取得/設定が可能です。</p>
<p>Regex クラス<br>
<a href="http://msdn.microsoft.com/ja-jp/library/system.text.regularexpressions.regex.aspx">http://msdn.microsoft.com/ja-jp/library/system.text.regularexpressions.regex.aspx</a><br>
単純な HTML タグであれば今回のパターンで取り除くことが可能ですが、JavaScript ブロック内部にタグ文字が表示されている場合は正しく取り除けない可能性があります。その場合は下記の正規表現要素を参考に JavaScript ブロックそのものを取り除くパターンを設定し、タグを取り除く前に実行して下さい。</p>
<p>正規表現言語要素<br>
<a href="http://msdn.microsoft.com/ja-jp/library/az24scfc(VS.80).aspx">http://msdn.microsoft.com/ja-jp/library/az24scfc(VS.80).aspx</a></p>
