# 連載! とことん C#: 第 18 回 モジュール (.netmodule) の意義
## Requires
- 
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 4 月 9 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、前にモジュールとアセンブリの話をしてくれましたよね?」(<a href="http://msdn.microsoft.com/ja-jp/ff458159.aspx">第 13 回</a>を参照)</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「その通りじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、その 2 つって、使い分ける意味があるのですか?」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">アセンブリを分けてみよう</a> </li><li><a href="#02">モジュールを使ってみよう</a> </li><li><a href="#03">なぜモジュールに分けるのか?</a> </li><li><a href="#04">結末</a> </li></ol>
<hr>
<h2 id="01">1. アセンブリを分けてみよう</h2>
<p>まず、1 つのプログラムを複数のアセンブリに分けることは簡単です。以下の 3 点を守るだけです。</p>
<ul>
<li>クラス ライブラリのプロジェクトを作成し、コードの一部をそこに置く </li><li>メイン プログラム側は、そのクラス ライブラリを参照に加える </li><li>他のアセンブリから参照されるクラス等には必ず public キーワードを付ける<br>
(あるいはフレンド アセンブリの指定などを行ってアクセス可能とする) </li></ul>
<p>この手順は、基本的に使用するテクノロジに関わらず同じです。ASP.NET だろうと WPF だろうと、Visual Basic を使おうと、作成するのはクラス ライブラリです。例えば、以下のようなプログラムがあると仮定します。</p>
...
class SampleClass
{
    internal static void SayWelcome()
    {
...
<p>実行結果:</p>
<p>Welcome to C# World!</p>
</div>
<p>これを SampleClass だけ分離して別プロジェクト (クラス ライブラリ) にする場合、実行する手順は基本的に上記で述べた 3 つです。メイン プログラムは以下のようになり、特に変更はありません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この場合、クラス ライブラリのプロジェクト側は以下のようなコードになります。public キーワードで公開することを忘れてはなりません。(太字が変更した箇所です)</p>
<div style="margin:2px 0px; padding:5px 5px 0px 5px; background-color:#dedfde; width:591px">
<p>using System;</p>
<p>public class SampleClass<br>
{<br>
<strong>public</strong> static void SayWelcome()<br>
{<br>
Console.WriteLine(&quot;Welcome to C# World!&quot;);<br>
}<br>
}</p>
</div>
<p>あとは、前述のメイン プログラムのプロジェクト側から、Visual Studio の [参照の追加] メニューを使用して、クラス ライブラリ側のプロジェクト (またはコンパイルして作成された .dll ファイル) を参照しておけば実行できます。実際に使用する際は、アセンブリごとに名前空間が変わることも多いと思うので、using などを使用して相手の名前空間を解決することも必要とされるでしょう。</p>
<p>以上のように、クラス ライブラリには、「共有され、使い回されるクラスをまとめてライブラリ化する」という効能の他に、ただ単にプログラムを分割するという機能性もあります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16617-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. モジュールを使ってみよう</h2>
<p>さて、つぎに、アセンブリではなくモジュール単位に分割とコンパイルを行うには、実は、コマンドラインからコンパイラを使う必要があります。(Visual Studio の IDE 環境上では、.netmodule をコンパイルするための選択肢は提供されていません。)</p>
<p>では、実際に作成方法をみてみましょう。以下の例では、別々のフォルダに以下の 2 つのファイルがあると仮定し、これらをそれぞれ別々にコンパイルし、最終的に 1 つにアセンブリにまとめてみます。</p>
<p>Class1.cs:</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
...
<p>これらをコンパイルするわけですが、普通にコマンド プロンプトでコンパイルするだけなら、Visual Studio をインストールしたときに同時にインストールされる Visual Studio コマンド プロンプト (Visual Studio x64 Win64 Command Prompt (2010) など) を開き、csc.exe コマンドに対象となるファイル名を指定してコンパイルできます。(Visual Studio コマンド プロンプトには、csc.exe など必要なコマンドへのパスが通っています。)</p>
<p>しかし、モジュールが欲しいときは、そこに /target:module というオプションを以下の通り追加します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">C:\MyProjects&gt; csc /target:module c\class1.cs

以下は出力結果です . . .
Microsoft (R) Visual C# 2010 Compiler バージョン 4.0.21006.1
Copyright (C) Microsoft Corporation. All rights reserved.</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">C:\MyProjects&gt; csc /target:module c\class1.cs
...
<p>これでカレント フォルダーに Class1.netmodule というファイルが得られます。</p>
<p>つぎにメイン プログラムをコンパイルしますが、こちらは最終的に実行ファイルが欲しいのであって、モジュールを増やしたいわけではありません。そのため、モジュール作成を指示するオプションは追加しません。その代わり、上でコンパイルしたモジュールをアセンブリに追加します。以下の通り、今度は、/addmodule:&lt;ファイル名&gt; というオプションを使用して、モジュールを追加しています。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">C:\MyProjects&gt; csc /addmodule:Class1.netmodule Program.cs

Microsoft (R) Visual C# 2010 Compiler バージョン 4.0.21006.1
Copyright (C) Microsoft Corporation. All rights reserved.</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">C:\MyProjects&gt; csc /addmodule:Class1.netmodule Program.cs
...
<p>これによりコンソール アプリケーション (Program.exe) が作成され、プログラムは完成です。これを実行すれば、意図した結果が得られます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">C:\MyProjects&gt; Program.exe

Welcome to C# World!</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">C:\MyProjects&gt; Program.exe
...
<p>さて、ここで注目していただきたいのは、2 つのソース ファイルは別々にコンパイルされていますが、最終的に 1 つの &quot;アセンブリ&quot; になったという点です。その&#35388;拠として、メイン プログラム側から公開されていない (public が付いていない) Class1 をちゃんと利用できています。これは、アセンブリを 2 つに分けた時には得られない効能であり、あくまで同じアセンブリだからこそできることです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16618-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. なぜモジュールに分けるのか?</h2>
<p>さて、モジュールの作成方法は理解できましたが、なぜモジュールに分ける必要があるのでしょうか?</p>
<p>ソース コードが増え過ぎ、プログラムが巨大になりすぎたとき、ソース コードを個別にグループに分けて扱いたいこともあるでしょう。このように分割をおこなうと、プログラム自身のメモリーへのロードの負荷も分割されることになります。しかし、モジュールでは、上述の通り、単なるアセンブリ分割と異なり、最終的に 1 つのアセンブリにまとめられる点が重要です。アクセス修飾子の指定が、上で見たように違ってくるからです。本来、一貫した 1 つの機能である巨大なプログラムがあった場合、それを「ただ分割したい」という理由だけで、その意味や公開範囲を変えて対処するのは、決して望ましいこととは言えません。</p>
<p>またこの方法では、別々の言語で作成したモジュールどうしをコンパイルして 1 つのアセンブリとして提供したい場合にも有効です。特に大きなプログラムを扱う場合は、通常、機能ごとに開発の役割を分担するため、開発者や開発チームのスキルに応じて、さまざまな言語を選択しなければならないケースもあるかもしれません。</p>
<p>そうなってくると、とりあえずモジュールにコンパイルした上で、最終的にモジュールを集めてアセンブリを作るということが実際の開発現場で要求される場合も (稀ではありますが) あるでしょう。特に、非常に巨大なプログラムをビルドするとき (あるいは、そうしたプログラムのビルド担当者 (ビルダー) になったとき) には、この特徴を活かすことは重要かもしれません。</p>
<p>また、既にできている同じアセンブリの一部をモジュールとして受け取ると、負担が軽減されるということもあるでしょう。</p>
<p>いずれにせよ、「分割してコントロールせよ」ということは、物事の基本です。膨大すぎて手に負えないと思うのではなく、いかにして分割して対処するかが重要です。小さく分ければ、難しそうな難題も対処可能な水準に落ちてくるかもしれません。モジュールは、そうした場合に有効な手段となります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16619-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 結末</h2>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました。食べやすく分けてしまえばどんな相手も怖くないのですね?」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。どんな凶悪な敵であろうと、分割して対処できるサイズにしてから対処するのじゃ。これは昔から使われた王道ではあるが、.NET はそれをやりやすい仕掛けが最初から入っているのじゃ!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「大きなプロジェクトでは、マシンのパワーに&#38972;りすぎない細やかな配慮も必要なのですね」</td>
</tr>
</tbody>
</table>
