...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 5 月 21 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。先輩たちと話していると、ラムダ式って、人によって随分と使用頻度が違いますよね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。それはラムダ式は最初から C# に備わっていた機能ではないからじゃ。初期の C# は実行すべき機能を扱うデリゲート型は持っておったが、まだラムダ式はなかったのじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ラムダ式を使わないと、コードもまるで違った印象になりますよね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。初期の C# に慣れ親しんだプログラマーの書くコードと、最新の C# にどっぷり浸ったプログラマーの書くコードはまったくの別物じゃな」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">デリゲート型とは何か</a> </li><li><a href="#02">イベントとは何か</a> </li><li><a href="#03">匿名メソッドとは何か</a> </li><li><a href="#04">ラムダ式とは何か</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. デリゲート型とは何か</h2>
<p>デリゲート型とは、C# を特徴づけるデータ型の 1 つです。</p>
<p>この型は、数値でも文字列でもなく、メソッドを扱う型です。といっても、これはメソッドそのものではなく、メソッドへの参照です。つまり、呼び出して欲しいメソッドを他のメソッドなどに伝達するために使用できます。</p>
<p>例を見てみましょう。</p>
...
class Program
{
    private delegate void sampleDelegateType();
    ...
    private static void b(sampleDelegateType sample)
    {
        sample();
    }
...
<p>このプログラムのポイントは、「メソッド b はメソッドの a のことを何も知らないが、引数経由で教えて貰うことで呼び出し可能になっている」という点です。ちなみに以下の部分がデリゲート型 sampleDelegateType の定義です。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden">private delegate void sampleDelegateType();</pre>
<pre id="codePreview" class="csharp">private delegate void sampleDelegateType();</pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
<p>実は、C# には、既定でこれと同じ「Action」という定義が存在しています。このため、これを使うことで上記は省略できます。つまり、このプログラムは以下のように書き直しても同じです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>実際に利用する場合は、上記の通り、戻り値がないデリゲート型は Action 型とそのジェネリック版、戻り値がある場合は Func 型のジェネリック版を使用することができ、実際に自前では定義しないことも多くあります。(デリゲートのジェネリックについては、以前、<a href="http://msdn.microsoft.com/ja-jp/ff427430.aspx">「第 11 回 共変性 (covariance) と反変性 (contravariance)」</a>でも説明しました。) しかし、独自に定義すれば、同じ引数と返り値の型でも区別して扱うことができるため、あえてこの定義をおこなう場合もあります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16656-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. イベントとは何か</h2>
<p>デリゲート型の利用例の 1 つがイベントです。イベントはあるクラス等の内部で起こった出来事を外部に伝達する機能です。しかし、イベントは単なるデリゲート型のメンバーとは違う機能であり、それとは違う機能性も提供してくれます。</p>
<ul>
<li>C# にビルドインされた機能であり、型ではありません </li><li>&#43;= 演算子でハンドラを追加し、-= 演算子でハンドラを削除できます </li></ul>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Sample
{
    internal event EventHandler MethodACalledEvent;
    internal void A()
    {
        MethodACalledEvent(this, EventArgs.Empty);
    }
}
...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        target.MethodACalledEvent &#43;= handler;
        target.A();
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>このサンプルコードで、MethodACalledEvent がイベントであり、&#43;= 演算子でハンドラを登録しています。</p>
<p>しかし、このサンプルコードで使用されている EventHandler 型は実はデリゲート型であり、こうした点からもイベントとデリゲート型は無縁ではないことがわかります。実際に、イベントは ASP.NET や Windows フォームなどさまざまな技術で多用されており、マウスのクリック イベントやフォームのロードイベントなどのイベントを伝達するために使用されています。</p>
<p>ここではデリゲートの使い勝手 (利用場面) の観点で説明しましたが、以降は、それは横に置いて、先に進みましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16657-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 匿名メソッドとは何か</h2>
<p>さて、このような機能を多用しようと思うとすぐ障害になるのは、ソースコードの文字数が多くなることです。実際に受け渡されるデータはメソッドそのものではなくその参照である以上、メソッド本体をどこかに書かなくてはならず、それには名前を付けなければなりません。それは短い文字数で済ませられるものではないのです。</p>
<p>そこで、名前を持たない「匿名メソッド」という機能が C# 2.0 より追加されました。以下に使用例を 1 つ示しますが、解説は行いません。なぜなら、実際にはこの匿名メソッドももう使用されず、もっと短い匿名メソッドが C# 3.0 より追加されているからです。ここでは、メソッドの定義をおこなわずに、デリゲート メソッドの呼び出しをおこなっているという点を理解しておいてください。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p style="margin-top:20px"><a href="#top"><img src="16658-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. ラムダ式とは何か</h2>
<p>この匿名メソッドの導入でデリゲート型の利用のハードルは下がりましたが、ラムダ式の導入でこのハードルは更に下がっています。上の例は以下のように書き換えることで、更に短くなります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>しかし、なぜラムダ式を使うのでしょうか?</p>
<p>それは以下の 2 つのプログラムを比較すると分かります。まず、下記の最初のプログラムでは、1 つのインターフェイスに異なる 2 つの挙動を実装していますが、そのために 2 つのクラスを作り出すという &quot;コード量の多い解決策&quot; を採っています。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class B : ISample
{
    void ISample.Phase1()
    {
        Console.WriteLine(&quot;in B.Phase1&quot;);
    }
    void ISample.Phase2()
    {
        Console.WriteLine(&quot;in B.Phase2&quot;);
    }
}
...
<p>ここでは、インタフェースのメソッドの実装を変えるために、A と B という 2 つのクラスを作成しています。しかし、実装の内容を変えたいだけならば、ラムダ式を引数で渡すことで以下のように書き換えられます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class X
{
    internal void Phase1(Action x)
    {
        x();
    }
    internal void Phase2(Action y)
    {
        y();
    }
}
...
class Program
{
    static void doIt1()
    {
        ...
        X x = new X();
        x.Phase1(() =&gt; { a &#43;= 1; });
        x.Phase2(() =&gt; { Console.WriteLine(a); });
    }

    static void doIt2()
    {
        X x = new X();
        x.Phase1(() =&gt; { Console.WriteLine(&quot;in B.Phase1&quot;); });
        x.Phase2(() =&gt; { Console.WriteLine(&quot;in B.Phase2&quot;); });
    }
...
<p>ここで注目すべき点は、挙動の違い (メソッド本体) を「実装するクラスの違い」から「引数で渡すラムダ式の違い」に変化させることで、余計なクラスやインタフェースがなくなっている点です。つまり、クラスを中心にして考えるだけではなく、ラムダ式を引数に渡すことで、&quot;メソッドも&quot; カスタマイズの主役になれることです。異なるメソッドを利用するたびに、毎回そのメソッドを定義したクラスを実装する必要はありません。この例では、もはや、かつての ISample インターフェイスに意味はないので、ISample インタフェースも存在していません。</p>
<p>もう 1 つは、ラムダ式の中から変数 a にアクセスできる点です。これは変数のキャプチャと呼ばれる機能であり、このようにアクセスされる変数 a の寿命はメソッドを抜けただけでは終わらず、参照されているラムダ式が生き続ける限り存続します。その結果、1 つ目の例ではクラスに属していたフィールド a は、2 つ目の例ではただのローカル変数に&#26684;下げとなり、より簡単かつ短く書くことができます。(しかし、キャプチャされるので寿命は伸びます。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16659-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました。C# の歴史上、ラムダ式は比較的新参の機能なので、慣れ親しんだ時期によってあまり使わない人と、よく使う人がいるわけですね !」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。一見、C# というとたった 1 つの言語がそこにあるかのように思えるが、実際はどんどん変化しておるのじゃ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「はい、頭を柔らかくして、新しい機能にもっと挑戦したいと思います!」</td>
</tr>
</tbody>
</table>
