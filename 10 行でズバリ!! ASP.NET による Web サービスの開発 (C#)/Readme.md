...
<h2><img src="13418-image.png" alt=""> このコンテンツのポイント</h2>
<ul>
<li>ASP.NET を利用した Web サービスの実装 </li><li>ASP.NET を利用した Web サービスのローカルでのテスト </li></ul>
<h2><img src="13419-image.png" alt=""> 今回紹介するコード</h2>
...
<pre class="hidden"><code class="csharp">1     [WebMethod (Description=&quot;サーバーの現在時刻を返します&quot;)]
2     public string ServerTime()
     {
         // サーバーの現在時刻を返します
3         return DateTime.Now.ToLongTimeString();
     }

4     [WebMethod (Description=&quot;生まれてから今日までの日数を算出します&quot;)]
5     public int LiveDate(DateTime BirthDay )
     {
         // 引数で渡された日付から今日までの日数を計算します
6         return DateTime.Now.Subtract(BirthDay).Days;
     }</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">1     [WebMethod (Description=&quot;サーバーの現在時刻を返します&quot;)]
2     public string ServerTime()
     {
         // サーバーの現在時刻を返します
3         return DateTime.Now.ToLongTimeString();
...
<h3><img src="13420-image.png" alt=""> 今回のシステム要件</h3>
<ul>
<li>Visual Studio 2005 Beta 2 </li></ul>
<p>&nbsp;</p>
<h3>目次</h3>
<p><a href="#aspnet_webservice_cs1">はじめに</a><br>
<a href="#aspnet_webservice_cs2">Web サービスの開発準備</a><br>
<a href="#aspnet_webservice_cs3">Web サービスの実装</a><br>
<a href="#aspnet_webservice_cs4">Web サービスをテストする</a><br>
<a href="#aspnet_webservice_cs5">おわりに</a></p>
<p>&nbsp;</p>
...
<tr>
<td align="center">
<div style="text-align:left; float:left; margin:0pt 9px 9px 0pt"><img src="13421-image.png" alt=""></div>
</td>
<td><a href="http://msdn.microsoft.com/ja-jp/events/dd231291.aspx">10 行でズバリ !! ASP.NET による Web サービスの開発 (VB)</a></td>
</tr>
</tbody>
</table>
<p>&nbsp;</p>
<hr>
<h2 id="aspnet_webservice_cs1">はじめに</h2>
...
<h2 id="aspnet_webservice_cs2">Web サービスの開発準備</h2>
...
<p>ここでは、言語として Visual C# を選択し、テンプレートとして 「ASP.NET Web サービス」 を選択します。 [場所] 欄では、ドロップダウン リストで 「ファイル システム」が選択されていることを確認した後、パスとして 「C:\Test\MyWebService」と入力し、[OK] をクリックします。</p>
<p>これで、Web サービスの開発準備が完了しました。</p>
<p><img src="13422-image.png" alt=""></p>
<p><strong>図 1. Web サービス開発のためのプロジェクトを新規に作成する</strong></p>
<h2 id="aspnet_webservice_cs3">Web サービスの実装</h2>
<p>既定の状態では、Web サービスの実装の雛形となる、Service.cs ファイルのコード エディタが開いています (図 2)。</p>
<p><img src="13423-image.png" alt=""></p>
<p><strong>図 2. Web サービスの雛形が作成される</strong></p>
<p>ソリューション エスクプローラを見ると、この Service.cs ファイルが、App_Code サブフォルダに置かれていることが分かります (図 3)。このフォルダは ASP.NET にとって特別なフォルダであり、Web サービスの実行時には、このフォルダ内のソース ファイルが自動的にコンパイルされます。(実行前に予めコンパイルする方法もあります。ここでは簡単にするため、既定の前者の方法を使用します。) また、ルート フォルダには、Service.asmx ファイルがあります。このファイルは、クライアントが
 Web サービスにアクセスするためのアクセス ポイントであり、このファイルを URL として指定します。</p>
<p><img src="13424-image.png" alt=""></p>
<p><strong>図 3. Web サービスに必要なフォルダ階層が構成されている</strong></p>
<p>ここでは、Web サービスとして 2 つのメソッドを Service.cs の中に実装します。サンプル用の既存のWeb サービス メソッドである HelloWorld を削除して、 以下の 2 つの Web サービス メソッドのコードを記述します。このように、一度に複数の メソッドを実装することができます。 なお、ルートフォルダにある Service.asmxを、直接編集することは余りありません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">[WebMethod (Description=&quot;サーバーの現在時刻を返します&quot;)]
...
<pre id="codePreview" class="csharp"><code class="csharp">[WebMethod (Description=&quot;サーバーの現在時刻を返します&quot;)]
public string ServerTime()
{
    // サーバーの現在時刻を返します
    return DateTime.Now.ToLongTimeString();
...
<p><img src="13425-image.png" alt=""></p>
<p><strong>図 4. 2 つのメソッドを記述する</strong></p>
<h2 id="aspnet_webservice_cs4">Web サービスをテストする</h2>
...
<p><img src="13426-image.png" alt=""></p>
...
<p><img src="13427-image.png" alt=""></p>
...
<p><img src="13428-image.png" alt=""></p>
...
<p><img src="13429-image.png" alt=""></p>
...
<p><img src="13430-image.png" alt=""></p>
<p><strong>図 9. Web サービスの実行結果</strong></p>
<h2 id="aspnet_webservice_cs5">おわりに</h2>
