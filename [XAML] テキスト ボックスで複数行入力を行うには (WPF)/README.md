...
<pre id="codePreview" class="xaml"><code class="csharp">&lt;Window x:Class=&quot;TextBox_WPF01.Window1&quot;
...
<h2>実行結果</h2>
<p><img src="15553-image.png" alt=""></p>
<h2>ポイント</h2>
<p>TextBox コントロールで複数行の入力に影響するプロパティは大きく 2 つあります。</p>
<p>1 つ目は Return (Enter) キーの入力を行えるようにする AcceptsReturn プロパティになります。</p>
<p>AcceptsReturn プロパティを True にすることによって TextBox コントロールの編集中に Return キー (Enter キー) の入力が行えるようになります。</p>
<p>2 つ目は文字の折り返し指定が行える TextWrapping プロパティになります。</p>
<p>TextWrapping プロパティは TextWrapping 列挙体を指定することが可能で、TextBox コントロールの幅を超えた文字列の折り返しに対する挙動を指定できます。</p>
<p>指定できる値は折り返しを行う Wrap、折り返しを行わない NoWrap が指定できます。</p>
<p>Wrap を指定すると基本的にはスペースなどの区切り文字を境界として折り返しを行いますが、入力された文字列の 1 単語の中に区切りとなる文字が存在しなかった場合は表示できる限界で折り返しが行われます。NoWrap を指定すると折り返しは行われません。</p>
<p>上記 2 つのプロパティで注意すべき点は、AcceptsReturn プロパティも、TextWrapping プロパティも基本的にはスクロール バーなどの表示制御は行わないということです。</p>
<p>そのため NoWrap を指定したあと、水平方向に表示領域を超えて文字入力を行ってもスクロール バーの表示は既定では行われません。</p>
<p>AcceptsReturn キーも同様で TextBox コントロールの Height を超えた行になっても垂直方向のスクロール バーの表示はおこなわれません。</p>
<p>Windows Form 等でデータ入力を行った場合、入力がコントロールの持つ高さ、幅を超えた際にスクロール バーの表示を期待する場合があります。そのためには別途スクロール バーを制御するプロパティの設定が必要になります。</p>
<p>TextBox ではスクロール バーの表示を制御するためのプロパティとして以下のプロパティがあります。</p>
<p>水平方向のスクロール バーの表示の制御は HorizontalScrollBarVisibility、<br>
垂直方向のスクロール バーの表示の制御は VerticalScrollBarVisibility</p>
<p>これらは ScrollBarVisibility 列挙体によりスクロール バーの挙動が定義できます。</p>
<p>ScrollBarVisibility 列挙体のメンバーは以下になります。</p>
...
<td valign="top">メンバー名</td>
<td valign="top">説明 (要約)</td>
</tr>
<tr>
<td valign="top">Disabled</td>
<td valign="top">表示されません。加えて入力された文字列はスクロールがおこわれません。(AcceptsReturn が True であれば複数入力は可能です)</td>
</tr>
<tr>
<td valign="top">Auto</td>
<td valign="top">指定されたテキストの行数に応じて自動的にスクロール バーの表示を制御します。</td>
</tr>
<tr>
<td valign="top">Hidden</td>
<td valign="top">表示されません。スクロール バーは表示されませんが、カーソル位置へのスクロールは行われます。</td>
</tr>
<tr>
<td valign="top">Visible</td>
<td valign="top">常に表示されます。</td>
</tr>
</tbody>
</table>
<p>いずれも既定では Hidden が指定されているため、スクロール バーの表示を行う場合は Auto や Visible などの指定を行う必要があります。</p>
<p>また、TextBox とは性質が異なるコントロールにはなりますが、フロー ドキュメントを取り扱うことができる RichTextBox コントロールでも複数行のテキストを取り扱うことが可能です。</p>
<p>フロー ドキュメントとは、その名のとおり固定したレイアウトだけではなく、コントロールのサイズに合わせて、その中に表示されるテキスト等を臨機応変にレイアウトするための仕組みを持ったドキュメントとなります。</p>
<p>フロー ドキュメントで指定できるものはワード パッドで作成するようなRTF形式のリッチ テキストになります。つまり、書式を持ったテキストやイメージなどが取り扱うことが可能です。</p>
<p>RichTextBox コントロールはこのようなリッチ テキストの編集が行えるコントロールのため、複数行表示も簡単に行える上、詳細な書式の指定も可能です。</p>
<h2>参考</h2>
<p>ScrollBarVisibility 列挙体<br>
<a href="http://msdn.microsoft.com/ja-jp/library/system.windows.controls.scrollbarvisibility(VS.95).aspx">http://msdn.microsoft.com/ja-jp/library/system.windows.controls.scrollbarvisibility(VS.95).aspx</a></p>
<p>RichTextBox の概要<br>
<a href="http://msdn.microsoft.com/ja-jp/library/aa970779.aspx">http://msdn.microsoft.com/ja-jp/library/aa970779.aspx</a></p>
