# 連載! とことん C#: 第 2 回 静的 (static) なクラスとその意義
## License
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「生徒のジョシュアです。ところで、僕があれほどみんなに『このクラスは、インスタンスはたった 1 つだよ』『2 つ作ったら整合性がとれなくなるからダメだよ』と注意したのに、みんな new するコードを書いてしまいました」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「なるほど。プログラムの中でたった 1 つという仕様を貫徹できないことが問題なのだね?」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「そうです。ドキュメントにはしっかり書いたのに、みんな守ってくれないんです。」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「そういう場合は、インスタンス化するとエラーになるよう、最初から静的なクラスをデザインすればいいのじゃ!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「静的なクラスって何ですか?」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">そもそもクラスとは?</a> </li><li><a href="#02">プライベートなコンストラクタ</a> </li><li><a href="#03">静的クラス (Static Class)</a> </li><li><a href="#04">1 つだけ実体はある</a> </li><li><a href="#05">専用の構文はなぜ必要か</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. そもそもクラスとは?</h2>
<p>クラスとは、いわば設計図です。クラスをインスタンス化することで、設計図は実体になります。</p>
<p>実体にする方法は、主に new 演算子です。(ここは、構造体と同じではありません。) new は極めて強力な機能であり、そのクラスが見えていれば設計図はいくらでも実体になります。その結果、自動車クラスはいくつでも実体にすることができ、たった 1 つの車クラスから道路を埋め尽くす大量の自動車の実体を産み出すことができます。</p>
<p>しかし、「交通整理のおじさん」クラスの実体は無制限に増やしてはなりません。「交通整理のおじさん・その 1」が「行け」といい、「交通整理のおじさん・その 2」が別の経路の車に「行け」と言ったら事故が起こるだけです。交通整理役は必ず 1 人にしなければなりません。</p>
<p>そのため、あるクラスのインスタンスをたった 1 つに限ることは、特に「クラス間の調停」という機能性を持つクラスに対してしばしば行われます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16628-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. プライベートなコンストラクタ</h2>
<p>new が使用できない、という意図を貫徹する最もシンプルな方法は、new すると例外を投げる方法です。たとえば、コンストラクタに throw 文を仕込むと言う方法です。</p>
...
<p>しかし、この方法は実行するまで結果が分かりません。コンパイル時に実体化できないか分かった方が便利です。そこで、private なコンストラクタを使うことで、new が使えない意図を示すという方法が使用できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このようなテクニックは、C# でも古いソースに残っている可能性があるので、知っておくと良いでしょう。</p>
<p>唯一のコンストラクタが private であるということは、外部からコンストラクタを呼ぶことができません。しかし、実は、この方法には以下の問題があります。</p>
<ul>
<li>呼びされないメソッドを記述するのはおかしい </li><li>new させないという意図と private なコンストラクタは直結しない (書いているのに実行させないのは直感的ではない) </li><li>private なメンバーを呼べる状況になった場合は、結局、new に成功してしまう </li></ul>
<p>そこで、こうした場合、C# では、静的なクラスという機能を使用するのが一般的です。</p>
<p style="margin-top:20px"><a href="#top"><img src="16629-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 静的クラス (Static Class)</h2>
<p>「new させない」という意図を示すために、C# では静的クラスという機能を使用します。これは、static (静的) キーワードをクラスに付加するだけです。</p>
<div style="margin:20px 0px; padding:10px; background-color:#dedfde">
<p>using System;</p>
<p><strong>static</strong> class Sample // 静的クラス<br>
{<br>
}</p>
<p>class Program<br>
{<br>
...
{<br>
new Sample(); // newはコンパイル・エラーになる<br>
}<br>
}</p>
</div>
<div style="margin:20px 0px; padding:10px; background-color:#efefef; border:solid 1px">
<p>コンパイル エラー:</p>
<p>静的クラス 'Sample' のインスタンスを作成することはできません。</p>
</div>
<p>これでもう、呼び出されることのないコンストラクタを書かねばならない、という矛盾 (意図とコードの間のギャップ) から解放されます。static というキーワードがクラスを new させないという意図を示しており、矛盾はありません。状況次第で成功することもありません。書くべき文字数も減っています。</p>
<p>つまり、このような定義を与えておけば、new しようとした人は必ずコンパイルエラーとなり、間違いに気づくわけです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16630-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 1 つだけ実体はある</h2>
<p>静的なクラスは new できません。そのため、たった 1 つの実体すら作れないように思えるかも知れません。しかし、それは誤りです。静的なクラスの実体は、基本的に常に 1 つだけ用意されるのです。</p>
<p>なぜかといえば、静的なクラスは静的なメンバーのみ記述できるという制約があり、静的なメンバーは基本的に 1 つだけ実体が用意されるからです。</p>
<p>ですから、以下のコードは有効です。new は無くとも実体は 1 つだけ用意され、代入は成功します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p style="margin-top:20px"><a href="#top"><img src="16631-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 専用の構文はなぜ必要か</h2>
<p>たった 1 つという制約を持つクラスは実際の開発ではそれほど多くはないかもしれません。「わざわざ専用の構文を用意する必要などない」、あるいは「本&#26684;的に覚えて活用するほどでもない」と思われる方も多いかもしれません。</p>
<p>しかし、プログラムの断片を自由に渡せる「ラムダ式」と「デリゲート型」を活用しようと思うと、話は変わってきます。</p>
<p>まず、この 2 つを活用しない事例から見ていきましょう。以下は、クラス A の動作をクラス B で変更している例です。クラス A の動作が変更されているので、クラス A からの Say メソッドの呼び出しはクラス B の Say メソッドを呼び出します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このように記述しているなら、上記で説明した静的なクラスはほとんどないように思えるかも知れません。しかし、継承ではなくデリゲート型とラムダ式でカスタマイズするコードに書き直すと違った世界が見えてきます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この実行結果は同じになります。ラムダ式については、いずれこの連載でも詳しく説明していきますが、このコードでは、クラス A の中で、void の引数をとって Console.WriteLine(&quot;I am A!&quot;); を実行する () =&gt; { Console.WriteLine(&quot;I am A!&quot;); } という関数 (正確には、Action デリゲート) を作成し、この関数を Say に設定しています。そして、Main のプログラムでは、この関数の内容を変更しています。</p>
<p>見ての通り、外部から Say の動作は自由に代入して入れ替えることができます。ラムダ式を書いて差し替えれば自由にカスタマイズできます。クラス A から呼び出しても、カスタマイズされた機能が呼び出されます。このコードでは、その前のコードで登場したクラス B は消えて無くなりました。</p>
<p>しかし、このプログラムは更にもっとシンプルに書き換えることができます。外部から「動作」を受け取るメソッドを記述するだけです。メソッドを受け取らなかった場合だけ、「I am A!」と叫びます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class A
{
    internal void WhoAreYou(Action action = null)
    {
        if (action != null) action(); else Console.WriteLine(&quot;I am A!&quot;);
    }
}
...
<p>以前のコードでは、WhoAreYou メソッドを使用する前にクラス A の内容を書き換えていますが、このコードでは、WhoAreYou メソッドを使用する際にクラス A への依存性はなくなっています。本来、クラス A は複数インスタンス存在する (new する) 意味はありませんが、これで、静的クラス (static class) に書き換えても良いことになります (下記)。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>つまり、最初にカスタマイズされる対象だったクラス A は単なる静的 (static) なメソッドの入れ物に立場を後退させ、代わりに WhoAreYou メソッドが「実行すべきコードの断片」を受け入れる能力を得たことで、「外部から動作をカスタマイズされる」主役になります。そのような主役も C# の構文の制約上では、何らかのクラスなどに属する必要があります。静的なクラスとは、そのようなメソッドの入れ物としてもうってつけなのです。</p>
<p>このように、クラスを主役として継承で動作をカスタマイズする場合、静的クラスの出番はそれほど多くありませんが、メソッドを主役として「ラムダ式」と「デリゲート型」で動作をカスタマイズする場合、静的クラスの出番は多くなると言えます (メソッド主体の場合、本来、クラスをいくつも生成させる必要はありません)。こうした状況では、専用の構文を用意してまで使う価値があると言えるでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16632-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="margin-bottom:10px">
<tbody>
<tr>
<td valign="top" style="border-bottom:#ffffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffffff dotted 1px; padding:10px">「分かりました。クラス単体である意図を明示したい場合に、静的なクラスが使えるということですね!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffffff dotted 1px; padding:10px">「うむ。しかし、このような方法で自由度が低くなるとも限らない。ラムダ式で動作の拡張もできるメソッドを書けば後から拡張できるぞ!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「これでプログラムが矛盾したらエラーにできます! ありがとうございました!」</td>
</tr>
</tbody>
</table>
