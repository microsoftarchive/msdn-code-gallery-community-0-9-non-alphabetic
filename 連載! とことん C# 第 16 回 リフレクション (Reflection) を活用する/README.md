...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 3 月 26 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、先輩がリフレクションという仕組みを使ったコードを書いていたのですが、そんなもの使わなくても、普通にクラスとかをそのまま使えば良いですよね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。アクセスする権限さえあればのう」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「じゃあ、どうして、リフレクションを使ってプログラムに含まれるクラスを列挙するようなコードを書くのですか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「いくつかパターンがあるのじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">すべてのアセンブリを列挙してみる</a> </li><li><a href="#02">型も列挙してみよう</a> </li><li><a href="#03">型のメンバーにアクセスせよ</a> </li><li><a href="#04">インスタンス化して呼び出す</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. すべてのアセンブリを列挙してみる</h2>
<p>C# を使ってプログラムでアセンブリを列挙するだけなら簡単です。</p>
<p>AppDomain.CurrentDomain.GetAssemblies メソッドを使用するだけです。特に準備なども必要ありません。(以下は、そのサンプル コードです。)</p>
...
class Program
{
   static void Main(string[] args)
   {
       foreach (var assem in AppDomain.CurrentDomain.GetAssemblies())
       {
           ...
           Console.WriteLine(&quot;{0}&quot;, asmttl.Title);
       }
   }
}</code></pre>
</div>
...
<p>実行結果: <br>
(アプリケーションを ConsoleApplication1 の名前で作成している場合)</p>
<p>mscorlib.dll<br>
Microsoft.VisualStudio.HostingProcess.Utilities.dll<br>
System.Windows.Forms.dll<br>
System.Drawing.dll<br>
System.dll<br>
Microsoft.VisualStudio.HostingProcess.Utilities.Sync.dll<br>
Microsoft.VisualStudio.Debugger.Runtime.dll<br>
vshost32.exe<br>
System.Core.dll<br>
...
System.Data.dll<br>
System.Xml.dll<br>
ConsoleApplication1</p>
</div>
<p>それぞれのアセンブリの名前などを知ることも容易です。.NET Framework は自己記述的であり、豊富なメタデータが付与できるからです。この場合、アセンブリの名前もメタデータの一種であり、上記のように、実行中でも知ることができます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16605-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 型も列挙してみよう</h2>
<p>こうしてアセンブリを列挙すること自体は、アセンブリのバージョンを調べるといった場合などを除けば、実際にはあまり意味がありません。しかし、そのアセンブリに属する型の一覧を調べることは、実際のプログラミングでは時折あります。ある条件を満たすすべての型について、一律に何かの処理を行う場合などには、こうした処理が役に立つからです。</p>
<p>例えば、以下では、名前に「<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.String.aspx" target="_blank" title="Auto generated link to System.String">System.String</a>」の文字列を含む型を列挙します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p><a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.String.aspx" target="_blank" title="Auto generated link to System.String">System.String</a><br>
<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.StringSplitOptions.aspx" target="_blank" title="Auto generated link to System.StringSplitOptions">System.StringSplitOptions</a><br>
<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.StringComparer.aspx" target="_blank" title="Auto generated link to System.StringComparer">System.StringComparer</a><br>
<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.StringComparison.aspx" target="_blank" title="Auto generated link to System.StringComparison">System.StringComparison</a><br>
SampleSystem.String</p>
</div>
<p>上記の GetExportedTypes メソッドは、そのアセンブリ内で外部から利用できるすべての型を列挙するメソッドです。FullName は、名前空間名を含む型の名前 (正式名) を示します。</p>
<p>この場合、読み込まれているすべてのアセンブリで、外部から利用可能なすべての型を列挙し、その中から指定条件に見合う型だけを探していますが、例えば、以下のような条件でも容易に検索できます。</p>
<ul>
<li>クラスだけを列挙する </li><li>抽象型だけを列挙する </li><li>あるクラスを継承している場合だけを列挙する </li></ul>
<p>上記で列挙されているのは、現在のアプリケーション・ドメインに読み込まれているすべてのアセンブリが外部に対して公開している型です。つまり、明示的、または暗黙的に参照されて、このアプリケーションに読み込まれたすべてのアセンブリが対象になります。</p>
<p>ここで &quot;くせ者&quot; であるのは、明示的な参照はプログラマー自身が把握しているので割と分かりやすいのですが、暗黙的に読み込まれているアセンブリも対象となっているという点です。そのため、型を探す場合の条件を間違えると、意図した型以外も検索されてしまう可能性があります。また、参照しているアセンブリで、例えば、誰かがクラス宣言を書き加え、その結果として、関係ないクラスまで見つかってしまうこともあるでしょう。</p>
<p>このように、すべてのアセンブリや型を列挙 (検索) するコードは強力ですが、同時に無用なトラブルも引き起こしやすい点に注意しましょう。また、検索対象を確定させるための条件の設定には注意を払うようにしましょう。こうした点が甘いと、関係ないクラスを追加した場合など、簡単なコードの書き換えで動作が狂ってしまうでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16606-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 型のメンバーにアクセスせよ</h2>
<p>実際の活用場面を想定し、例えば、以下のようなケースを考えてみましょう。</p>
<ul>
<li>あるプログラムに、「開く」(ロード) と「保存」(セーブ) の機能を付けたい </li><li>「保存」(セーブ) では、ある一連の情報 (データ) をファイルに保存したい </li><li>しかし、保存対象の情報 (データ) は、そのプログラム内の各所に分散しており、まとまって管理されているわけではない </li></ul>
<p>こういう場合は、上記のプログラムをさらに応用して、以下のように処理することができます。</p>
<ul>
<li>「開く」(ロード) と「保存」(セーブ) の処理で、あらゆるクラスを列挙する </li><li>その列挙されたクラスの中で、特定の属性 (Attribute) が付いているメンバーの値のみを読み書き (ロード/セーブ) の対象とする </li></ul>
<p>また、それでも遅い場合には、以下のような措置も追加しておきます。</p>
<ul>
<li>クラスに、保存すべき値があることを示す属性 (Attribute) を付けておく (クラスすべてをスキャンするのではなく、対象のクラスのみスキャンする) </li></ul>
<p>実際にこの処理を作成した例 (サンプル コード) を以下に示します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Reflection.aspx" target="_blank" title="Auto generated link to System.Reflection">System.Reflection</a>;
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Xml.aspx" target="_blank" title="Auto generated link to System.Xml">System.Xml</a>;
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Xml.Linq.aspx" target="_blank" title="Auto generated link to System.Xml.Linq">System.Xml.Linq</a>;
...
class Program
{
    // AutoFlags属性を持つクラスの中のFlag属性を持つフィールドに、
    // 引数action の関数を実行します
    // (このメソッドは、このあとの saveメソッド, loadメソッドで使用します)
    private static void walkAll(Action&lt;System.Reflection.FieldInfo&gt; action)
    {
        foreach (var assem in AppDomain.CurrentDomain.GetAssemblies())
        {
            ...
            {
                if (t.GetCustomAttributes(typeof(AutoFlagsAttribute), 
                  true).Length == 0) continue;
                foreach (var f in t.GetFields(BindingFlags.Public | 
                  BindingFlags.Static))
                {
                    var query1 = from n2 in f.GetCustomAttributes(true) 
                        where n2 is FlagAttribute select n2;
    ...
    // AutoFlags属性を持つクラスのFlag属性を持つフィールドを
    // filename のファイルに (XML 形式で) 保存する
    private static void save(string filename)
    {
        using (XmlTextWriter writer = 
            new XmlTextWriter(filename,<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Text.Encoding.UTF8.aspx" target="_blank" title="Auto generated link to System.Text.Encoding.UTF8">System.Text.Encoding.UTF8</a>))
        {
            ...
            walkAll((field) =&gt;
            {
                if (field.FieldType == typeof(int))
                {
                    writer.WriteElementString(field.Name, 
                       ((int)field.GetValue(null)).ToString());
                }
            });
        }
    }

    // 逆に、AutoFlags属性を持つクラスのFlag属性を持つフィールドのみを
    // filename のファイルからロードする
    private static void load(string filename)
    {
        ...
        walkAll((field) =&gt;
        {
            ...
            {
                field.SetValue(null, 
                  int.Parse(x.Element(&quot;flags&quot;).Element(field.Name).Value));
            }
        });
    }
    ...
    static void Main(string[] args)
    {
        string filename = &quot;file name here.xml&quot;;
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>XMLファイルの内容:</p>
<p>&lt;flags&gt;&lt;ValueA&gt;123&lt;/ValueA&gt;&lt;/flags&gt;</p>
</div>
<p>上記のように、ファイルに書き出した (保存した) 後で A.ValueA に -1 を代入しても、ファイルからデータをロードすれば、123 という元の値が復活します。ここで、A.ValueA は属性 (Flag属性) が付いているので保存の対象ですが、B.ValueB は保存されません。(B.ValueB には属性が付いていないからです。) そのため、B.ValueB に設定した -2 の値はそのまま出力されています。</p>
<p>ちなみに、AutoFlagsAttribute 属性と、FlagAttribute 属性は保存すべき値を指定するスイッチとしてしか機能しないので、特に中身 (独自の処理など) はありません。(あくまでも、「目印」として使用しているだけです。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16607-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. インスタンス化して呼び出す</h2>
<p>このようにして検索 (スキャン) された型は、単に「存在している」ことを知るだけではありません。実際に、インスタンス化して実体を作成することもできます。そのためには、Activator.CreateInstance メソッドを呼び出すだけです。また、そのインスタンスのメソッドを呼び出すこともできます。それには InvokeMember メソッドを使用します。</p>
        ...
        foreach (var assem in AppDomain.CurrentDomain.GetAssemblies())
        {
            foreach (var t in assem.GetExportedTypes())
            {
                if (t.Name == &quot;Sample&quot;)
                {
                    ...
                    instance.GetType().InvokeMember(&quot;SayHello&quot;, 
                      BindingFlags.Public|BindingFlags.Instance|BindingFlags.InvokeMethod,
                      null, instance, new [] {&quot;Hello Unknown Method!&quot;});
                }
            }
        }
    }
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>Hello Unknown Method!</p>
</div>
<p>ちなみに、この方法を使えばコンパイル時に定義が行われていないクラス (あとからロードされたクラスなど) もインスタンス化してメソッドを呼び出すことができます。</p>
<p>しかし、この記述はあまりにも面倒です。実は、CreateInstance、InvokeMember の処理をおこなっている上記の 2 行は、C# 4.0 以降であれば、以下のように書くことができます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">dynamic instance = Activator.CreateInstance(t);
instance.SayHello(&quot;Hello Unknown Method!&quot;);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>この場合のポイントは dynamic 型にあります。dynamic 型は実行時にメソッドや引数を解決するので、コンパイル時に未知の型やメソッドも、実行時に明確であれば扱うことができます。(この動的ロードについては、次回、詳述します。また、dynamic 型については、いずれ、本連載でも詳述していきます。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16608-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました。ある条件を満たすクラスを処理の対象にする場合や、動的に後からロードされるアセンブリを処理する場合に必要とされるわけですね!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。名前がすべて確定しても、いちいち書くのが面倒な時は、こうして列挙してしまうのも 1 つの手じゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ありがとうございます。これで疑問も解消です!」</td>
</tr>
</tbody>
</table>
