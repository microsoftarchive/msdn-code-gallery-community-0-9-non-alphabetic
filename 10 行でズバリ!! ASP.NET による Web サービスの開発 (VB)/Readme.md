# 10 行でズバリ!! ASP.NET による Web サービスの開発 (VB)
## Requires
- 
## License
...
<h2><img src="13402-image.png" alt=""> このコンテンツのポイント</h2>
<ul>
<li>ASP.NET を利用した Web サービスの実装 </li><li>ASP.NET を利用した Web サービスのローカルでのテスト </li></ul>
<h2><img src="13403-image.png" alt=""> 今回紹介するコード</h2>
...
<pre class="hidden"><code class="csharp">1   &lt;WebMethod(Description:=&quot;サーバの現在時刻を返します&quot;)&gt; _
2   Public Function ServerTime()As String
        'サーバーの現在時刻を返します
3       Return DateTime.Now.ToLongTimeString()
    End Function

4   &lt;WebMethod(Description:= &quot;誕生日から今日までの日数を算出します&quot;)&gt; _
5   Public Function LiveDate(ByVal BirthDay As Date) As Integer
        ' 引数で渡された日付から今日までの日数を計算します
6        LiveDate = DateTime.Now.Subtract(BirthDay).Days()
    End Function</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">1   &lt;WebMethod(Description:=&quot;サーバの現在時刻を返します&quot;)&gt; _
2   Public Function ServerTime()As String
        'サーバーの現在時刻を返します
3       Return DateTime.Now.ToLongTimeString()
    End Function
...
<h3><img src="13404-image.png" alt=""> 今回のシステム要件</h3>
<ul>
<li>Visual Studio 2005 Beta 2 </li></ul>
<h3>目次</h3>
<p><a href="#aspnet_webservice_vb1">はじめに</a><br>
<a href="#aspnet_webservice_vb2">Web サービスの開発準備</a><br>
<a href="#aspnet_webservice_vb3">Web サービスの実装</a><br>
<a href="#aspnet_webservice_vb4">Web サービスをテストする</a><br>
<a href="#aspnet_webservice_vb5">おわりに</a></p>
<p>&nbsp;</p>
...
<tr>
<td align="center"><img src="13405-image.png" alt=""></td>
<td><a href="http://msdn.microsoft.com/ja-jp/events/dd282853.aspx">10 行でズバリ!! ASP.NET による Web サービスの開発 (C#)</a></td>
</tr>
</tbody>
</table>
<p>&nbsp;</p>
<hr style="color:#b2c1e0">
<h2 id="aspnet_webservice_vb1">はじめに</h2>
<p>Web サービス とは、Web テクノロジを使って、ソフトウェアを Web上に公開したものです。.NET では、Web アプリケーションの開発および実行基盤である ASP.NET を使用して、Web サービスをメソッドとして実装し公開することができます。また、Visual Studio 2005 を使って Web サービスを構築することによって、Web サイトを使った簡単なテストを行うことができます。</p>
<p>ここでは、Visual Studio 2005 を使った Web サービスの実装方法と簡単なテストの方法を解説します。</p>
<h2 id="aspnet_webservice_vb2">Web サービスの開発準備</h2>
<p>まず、新規にWebサイトを作成します。Visual Studio 2005 の [ファイル] メニューをクリックし、[新規作成] をポイントして、[Web サイト] をクリックします。 すると、[新しい Web サイト] ダイアログボックスが表示されます (図 1)。</p>
<p>ここでは、言語として Visual Basic を選択し、テンプレートとして 「ASP.NET Web サービス」 を選択します。 [場所] 欄では、ドロップダウン リストで 「ファイル システム」が選択されていることを確認した後、パスとして 「C:\Test\MyWebService」と入力し、[OK] をクリックします。</p>
<p>これで、Web サービスの開発準備が完了しました。</p>
<p><img src="13406-image.png" alt=""></p>
<p><strong>図 1. Web サービス開発のためのプロジェクトを新規に作成する</strong></p>
<h2 id="aspnet_webservice_vb3">Web サービスの実装</h2>
<p>既定の状態では、Web サービスの実装の雛形となる、Service.vb ファイルのコード エディタが開いています (図 2)。</p>
<p><img src="13407-image.png" alt=""></p>
<p><strong>図 2. Web サービスの雛形が作成される</strong></p>
<p>ソリューション エスクプローラを見ると、この Service.vb ファイルが、App_Code サブフォルダに置かれていることが分かります (図 3)。このフォルダは ASP.NET にとって特別なフォルダであり、Web サービスの実行時には、このフォルダ内のソース ファイルが自動的にコンパイルされます。(実行前に予めコンパイルする方法もあります。ここでは簡単にするため、既定の前者の方法を使用します。) また、ルート フォルダには、Service.asmx ファイルがあります。このファイルは、クライアントが
 Web サービスにアクセスするためのアクセス ポイントであり、このファイルを URL として指定します。</p>
<p><img src="13408-image.png" alt=""></p>
<p><strong>図 3. Web サービスに必要なフォルダ階層が構成されている</strong></p>
<p>ここでは、Web サービスとして 2 つのメソッドを Service.vb の中に実装します。サンプル用の既存のWeb サービス メソッドである HelloWorld を削除して、 以下の 2 つの Web サービス メソッドのコードを記述します。このように、一度に複数の メソッドを実装することができます。 なお、ルートフォルダにある Service.asmxを、直接編集することは余りありません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">&lt;WebMethod(Description:=&quot;サーバの現在時刻を返します&quot;)&gt; _
...
End Function</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">&lt;WebMethod(Description:=&quot;サーバの現在時刻を返します&quot;)&gt; _
Public Function ServerTime()As String
    'サーバーの現在時刻を返します
    Return DateTime.Now.ToLongTimeString()
End Function
...
<p>通常のメソッドと同じように記述し、先頭に WebMethod 属性を付与します。この属性を付けることで、Web サービスとして公開することができます。WebMethod 属性に Description プロパティを指定することで、 Web サービスの簡単な説明を公開することもできます。</p>
<p>1つ目の ServerTime メソッドは引数を持たず、現在時刻を単一の文字列として返す Web サービスです。また、2 つ目の LiveDate メソッドは引数で渡された日付から、今日までの日数を計算して整数値で返す Web サービスです。</p>
<p>この2 つのメソッドの入力が済んだら、[ファイル] メニューの [すべての保存] をクリックし、記述内容を保存しておきます。</p>
<p><img src="13409-image.png" alt=""></p>
<p><strong>図 4. 2 つのメソッドを記述する</strong></p>
<h2 id="aspnet_webservice_vb4">Web サービスをテストする</h2>
<p>コードの記述が完了したら、実行してテストしてみましょう。</p>
<p>本&#26684;的なテストのためには、Web サービスにアクセスするクライアント アプリケーションを作成した上で、クライアントからアクセスする必要があります。しかし、ASP.NET の Web サービスには、簡単に動作確認を行うテスト用のWeb ページが用意されています。ここでは、テスト用ページを使用します。</p>
<p>ソリューション エスクプローラのツリー上で、Service.asmx を右クリックして、[ブラウザで表示] をクリックします。</p>
<p>すると、Visual Studio 2005 の既定の構成では、試験用の Web サーバーとして 「ASP.NET 開発サーバー」が、自動的に起動し、そのサーバーの上で、Web サービスが実行できるようになります。そして、Web ブラウザが自動的に起動して、指定したファイル Service.asmx に自動的にアクセスします。URL には、ASP.NET 開発サーバーが使用するための、ランダムに設定されたポート番号が含まれています。このポート番号はソリューション ファイルに書き込まれ、これ以降は同じポート番号で実行することになります。</p>
<p>この拡張子 .asmx のファイルに、ブラウザからアクセスすると、ASP.NET はこの Web サービスを解析し、Web サービスに関する説明を記述した Web ページを自動的に生成します (図 5)。このページには、先に作成した2 つのメソッドが、リンクとしてリストアップされています。</p>
<p><img src="13410-image.png" alt=""></p>
<p><strong>図 5. ASP.NET は、Web サービスに関する説明を自動生成する</strong></p>
<p>ここで、ServerTime リンクをクリックすると、Web メソッドである ServerTime のためのテスト ページが開きます (図 6)。このページでは、Web メソッドを呼び出す [起動] ボタンのほか、下部には実際にやり取りされるサンプル メッセージが記載されています。</p>
<p><img src="13411-image.png" alt=""></p>
<p><strong>図 6. Web メソッドを呼び出すテスト ページ</strong></p>
<p>ここで [起動] ボタンを押してみます。すると、ServerTime Web メソッドが実行され、XML 形式で現在時刻が返されます。XML タグを見ると文字列として返されていることがわかります (図 7)。</p>
<p><img src="13412-image.png" alt=""></p>
<p><strong>図 7. Web サービスの実行結果</strong></p>
<p>ここで、実行結果が表示されたウィンドウを閉じて、ブラウザの戻るボタンで Web サービスの説明が記述された最初のページ (図 5) に戻ります。今度は、 LiveDate メソッドのリンクをクリックします。LiveDate メソッドは引数を持つため、[起動] ボタンのほかに、引数を入力するためのテキストボックスが用意されています (図 8)。</p>
<p><img src="13413-image.png" alt=""></p>
<p><strong>図 8. LiveDate Web メソッドのテスト ページ</strong></p>
<p>ここで、「1980/1/1」 と入力し、[起動] ボタンを押してみます。入力した日付から今日までの日数が計算され、XML 形式で日数が返されます (図 9)。</p>
<p><img src="13414-image.png" alt=""></p>
<p><strong>図 9. Web サービスの実行結果</strong></p>
<h2 id="aspnet_webservice_vb5">おわりに</h2>
<p>このように ASP.NET と Visual Studio 2005 に代表される統合開発環境を使用することによって、より簡単に Web サービスを作成し、簡易テストも行うことができます。また、このようなWeb サービスを使い、Web サービスの内部から、既存のアプリケーションを呼び出すようにすれば、Web サービス経由で、既存アプリケーションを他の環境から利用できるようになり、再利用性が高まります。</p>
<p>関連するコンテンツなどを参考にしながら、新たな Web サービスの開発にチャレンジしてみて下さい。</p>
<hr>
<div>
<table>
<tbody>
