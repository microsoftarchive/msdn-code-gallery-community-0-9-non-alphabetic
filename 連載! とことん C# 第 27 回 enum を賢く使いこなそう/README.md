# 連載! とことん C#: 第 27 回 enum を賢く使いこなそう
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 6 月 18 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで enum ってなんですか? 数値として扱ってるプログラムもあれば名前として扱っているプログラムもあります」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「あれは数値でもあり、名前も持っている存在なのじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">列挙型入門</a> </li><li><a href="#02">列挙型を数値として扱う</a> </li><li><a href="#03">列挙型の名前を得る</a> </li><li><a href="#04">列挙型のさまざまな操作</a> </li><li><a href="#05">列挙型の数値計算</a> </li><li><a href="#06">数値指定時の注意点</a> </li><li><a href="#07">LINQ もできる</a> </li><li><a href="#08">結末</a> </li></ol>
<hr>
<h2 id="01">1. 列挙型入門</h2>
<p>世の中には数値でも文字列でもなく、日付や時刻でもない情報があります。例えば、「性別」という情報を考えてみます。ここで種類として「男」と「女」があるとしましょう。このとき、この情報は数値として扱うこともがきます。男は 0、女は 1と決めれば良いのです。しかし、その取り決めを知らない人には、性別は 0 と言われても意味が分かりません。また 2 が来たらどうするのかも分かりません。</p>
<p>文字列として扱うようにすると、少し分かりやすくなりますが、やはり「男」、「女」以外の文字列の場合にどう解釈するかが明確ではありません。例えば、「紳士」や「淑女」のような文字列まで配慮すればきりがありません。</p>
<p>このような場合は列挙型を使います。列挙型は、有限の個数の名前を並べたデータ型です。</p>
<div style="margin:20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>enum 性別 { 男, 女 }</p>
</div>
<p>この記述によって、「性別」はデータ型として扱え、そして「男」と「女」という値があり、他には何もないことが定義されます。このように、数値にも文字列にも馴染まないが、列挙型で上手く表現できるデータは現実世界に多く、頻繁に使用されます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16680-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 列挙型を数値として扱う</h2>
<p>上記の列挙型は、数値として扱うことができます。並べられた値 1 つ 1 つに、0 から始まる番号が自動的に割り当てられますが、明示的な値を指定することもできます。つまり、列挙型はソース コードを読みやすくするための一種のシンタックス シュガーであり、中身は数値です。(このため、高い処理効率を期待できます。)</p>
<p>実際に列挙型の数値の値を得る方法は簡単であり、以下のようにキャストを入れるだけです。</p>
...
class Program
{
    static void Main(string[] args)
    {
        ...
        A b = (A)a;
        int c = (int)b;
        if (b == A.X) Console.WriteLine(&quot;b is A.X&quot;);
        if (c == 0) Console.WriteLine(&quot;c is 0&quot;);
    }
}</code></pre>
</div>
</div>
...
<p style="margin-top:20px"><a href="#top"><img src="16681-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 列挙型の名前を得る</h2>
<p>列挙型は定数に名前を付けるだけではありません。例えば、以下の 2 つのプログラムは同様の結果にはなりません。(片方は名前が出力され、もう片方は、その変数に割り当てられた数値が出力されます。)</p>
<p>以下で、enum A { ByEnum=0 } は、シンボル ByEnum に 0 という値を明示的に与え、const int ByConst = 0; は、ByConst に 0 という値を明示的に与えます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この 2 つの挙動は、以下の決定的な相違があります。</p>
<ul>
<li>const を使って整数に別名を定義すると整数型ですが、列挙型は整数とは異なる「型」です </li><li>上記の通り、ToString メソッドの挙動が異なります。整数型は値としての数値を返しますが、列挙型は名前を返します (上記で、Console.WriteLine メソッドは、ToString メソッドの結果を出力します)
</li></ul>
<p style="margin-top:20px"><a href="#top"><img src="16682-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 列挙型のさまざまな操作</h2>
<p>C 言語などの列挙型は単なる定数定義でしかありませんが、上記の動作からわかるように、C# では違っています。列挙型を扱う機能は、<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Enum.aspx" target="_blank" title="Auto generated link to System.Enum">System.Enum</a> クラスに含まれます。これを使うことで、特定の列挙型に含まれる値の一覧なども容易に得ることができます (下記)。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    static void Main(string[] args)
    {
        foreach (int val in Enum.GetValues(typeof(A)))
        {
...
<p>上記で、GetValues メソッドで値の一覧を得ることができますが、このサンプルコードでは int 型で受けている点に注意してください。あえて int と指定することで整数の値が出力されていますが、ここを var で受ければ、変数 val の型は A となり、名前が出力されます。</p>
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>その場合の実行結果:<br>
X<br>
Y<br>
...
<p>しかし、名前が欲しい時は GetNames を使って列挙した方が良いでしょう。その場合は、string 型の値として列挙されます。(異なる型で値が得られることに注意が必要です。)</p>
<p>ただし、ここで言う「名前」は、いわゆる難読化ツールを使用することで変化してしまう可能性があることに注意しましょう。例えば、以下のプログラムを実行して「男」という結果が得られても、難読化ツールを使用すると結果が変化してしまう可能性があります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この場合、属性 (<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Reflection.ObfuscationAttribute.aspx" target="_blank" title="Auto generated link to System.Reflection.ObfuscationAttribute">System.Reflection.ObfuscationAttribute</a>) などを使って難読化の対象外とするか、列挙型の名前は判断のみに使用し、出力には使用しない配慮などが必要です。(「属性」については、<a href="http://msdn.microsoft.com/ja-jp/ff458236.aspx">「第 14 回 属性 (Attribute) の使用」</a>を参照してください。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16683-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 列挙型の数値計算</h2>
<p>列挙型の値は数値計算できますが、通常の計算をおこなう価値はそれほどありません。たいていの場合、意味のある値にならないからです。ただし、OR 演算はしばしば使用されます。複数の有効なビットを集めて 1 つの値として渡す場合には有効なテクニックです。</p>
<p>例えば、以下のサンプルコードを見てください。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この場合、結果である 9 という数値そのものには意味が無く、名前を得ることもできません。(列挙型には 9 に対応する名前が存在しないからです。) しかし、この結果を逆に、ビットごとに分解すれば、ONE と EIGHT が含まれることが調べられます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16684-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 数値指定時の注意点</h2>
<p>例えば、以下のようなプログラムは、Y ではなく Z を代入しているにも関わらず、実行すると Y を出力します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このプログラムは実は X に 0、Y に 1 を割り当ており、Z の 1 と Y の 1 が意図せずして重複しています。そして、列挙型は、内部的に数値でデータを保管する仕組みのため、同じ数値になると区別が付かなくなります。そのため、プログラムはソース コード上で Z を代入しているにも関わらず、実行すると Y を出力してしまいます。</p>
<p>つまり、以下の点に注意を払って使用することはプログラマーの責任となります。</p>
<ul>
<li>列挙型を使用する場合は、すべてのシンボルが異なる値を持つようにする </li><li>同じ値に別の名前を用意することは構わないが、システムに問い合わせても同じ名前が出てこない可能性がある </li></ul>
<p>もしも、この注意を守れず、別の名前に同じ値を割り当ててしまった場合、プログラムが誤動作する可能性がありますが、コンパイラは特にエラーや警告は出してくれません。数値を指定せず、フレームワークに数値の割り振りをすべて任せれば、すべてユニークな値になるので安全です。しかし、移植時などの互換性を意識して常に名前と値の関係を維持したい場合や、上述した演算を考慮した数値を割り当てる場合などは、あえて数値を指定した定義を行う場合もあります。こうした場合には、上記の点に注意して割り当てをおこなうようにしましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16685-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="07">7. LINQ もできる</h2>
<p>列挙型の値は、列挙できる以上、当然 LINQ の対象になります。ただし、そのままでは列挙できないので、Enum.GetValues の返り値にキャストなどを使用して型を明示する必要があります。</p>
<p>また、その内容をクエリ (検索) したい場合には、実は、値にひと工夫を加える価値があります。</p>
<p>以下に、A～I の文字を示す列挙型 Alpha を定義し、そこから母音の文字だけを抜き出すクエリを考えてみましょう。まず、正攻法の実装です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
enum Alpha { A, B, C, D, E, F, G, H, I }

class Program
{
    static void Main(string[] args)
    {
        Alpha[] array = (Alpha[])Enum.GetValues(typeof(Alpha));
        var query = from n in array
                    where n == Alpha.A
                    || n == Alpha.E
                    || n == Alpha.I
                    select n;
        string all = string.Join(&quot;と&quot;, array);
        string v = string.Join(&quot;と&quot;, query);
        Console.WriteLine(&quot;{0}の中で母音は{1}です。&quot;, all, v);
    }
}</code></pre>
</div>
</div>
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:<br>
A と B と C と D と E と F と G と H と I の中で母音は A と E と I です。</p>
</div>
<p>ちなみに、上記で、string 型の Join メソッドは、複数の文字列をセパレータ文字を挟んで連結してくれるメソッドです。(例えば、クエリ結果 (query) が A と E と I のとき、string.Join (&quot;と&quot;、query) は「A と E と I」という文字列を生成してくれます。)</p>
<p>さて、ここで気になるのは、where 節の条件判断が長いことです。そこで、「母音には負数を、子音には正数を割り当てる」というルールを使って列挙型のそれぞれの値に意味を与えてみます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
enum Alpha { A=-3, B=1, C=2, D=3, E=-2, 
	F=4, G=5, H=6, I=-1 }
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:<br>
B と C と D と F と G と H と A と E と I の中で母音は A と E と I です。</p>
</div>
<p>この通り、where 節の比較は整数の比較 1 つに収まって短くなりました。(整数の値になってしまえば、もはや正数と負数の判定しか必要とされません。)</p>
<p>しかし、連続しない値を与えると、列挙型を列挙したときの並び順が変わってしまいます。もし、これを維持したいと思うなら、もうちょっとトリッキーな値の付け方を行うこともできます。以下は、母音は奇数、子音は偶数というルールで値を昇順に割り当てた例です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
enum Alpha { A = 1, B = 2, C = 4, D = 6, E = 9, 
	F = 10, G = 12, H = 14, I = 17 }
...
<p>ちなみに、((int)n % 2) != 0 とは、奇数であることを判定する式です。2 で割った余りが 0 なら偶数、1 なら奇数です。ちなみに、ビットの扱いと偶数、奇数の関係が分かっている場合は、((int)n &amp; 1) != 0 と書いても同じです。</p>
<p>このようにすると、判定はやや複雑になりますが、それでも最初の例よりもずっとシンプルに完了することができます。それでいて、順番は維持されています。</p>
<p>この例から分かる通り、列挙型は LINQ で活用することもでき、その際は与える値の工夫次第でクエリをシンプルに記述できる余地が大きくあります。列挙型を正しく理解し、多様なクエリをシンプルに記述してみましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16686-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="08">8. 結末</h2>
...
<td valign="top" style="padding:10px">「分かりました。列挙型の本質は数値ですが、いろいろと使い方を考慮して工夫ができるのですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。しかし、難読化ツールを使用した場合など、状況に応じていくつかの注意が必要じゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「はい、名前はプログラミングのキーワードでもあるし、是非有効活用してみます!」</td>
</tr>
</tbody>
</table>
