...
<div class="Clear"></div>
<div class="Clear"></div>
<p>更新日: 2009 年 11 月 10 日</p>
<table border="0" cellspacing="0" cellpadding="5" bgcolor="#e5f1f8" style="margin-bottom:10px">
<tbody>
<tr>
<td style="padding:10px 10px 10px 10px">
<div style="text-align:left; float:left; margin:0pt 9px 9px 0pt"><img src="10032-image.png" alt=""></div>
Visual Basic の内容はこちらに掲載しています。 <a href="http://msdn.microsoft.com/ja-jp/netframework/ee681694.aspx">
10 行でズバリ!! サービスの利用 (VB)</a></td>
</tr>
</tbody>
</table>
<h2><img src="10033-image.png" alt=""> このコンテンツのポイント</h2>
<ul>
<li>WCF サービスに接続するクライアント (コンシューマー) のプログラミングを理解する </li></ul>
<h2><img src="10034-image.png" alt=""> 今回紹介するコード</h2>
<p>&lt;Program.cs&gt;</p>
...
{
  // プロキシの作成とオープン
  using (ServiceReference1.OrderServiceClient proxy = new
    ServiceReference1.OrderServiceClient())
  {
    // メソッドの呼び出し
    ServiceReference1.OrderItem item = proxy.GetOrderById(3);
    // コンソールへの出力
    Console.WriteLine(&quot;Id は {0}、Name は {1} です&quot;, item.Id, item.Name);
    // プロキシのクローズ
    proxy.Close();
  }
...
<h2>目次</h2>
<ol>
<li><a href="#01">はじめに</a> </li><li><a href="#02">WCF サービスを起動する</a> </li><li><a href="#03">プロジェクトを作成する</a> </li><li><a href="#04">コードを実装する</a> </li><li><a href="#05">アプリケーションを実行する</a> </li><li><a href="#06">おわりに</a> </li></ol>
<hr>
<h2 id="01">1. はじめに</h2>
<p>ここでは、WCF サービスに接続するクライアント (コンシューマー) の実装方法 (プログラミング方法) を説明します。</p>
<p>クライアント (コンシューマー) を実装するには、Visual Studio (または Svcutil.exe ツール) を使用したプロキシ作成をおこなって接続する方法と、ChannelFactory クラスを使用した実装方法の 2 通りの方法があります。ここでは、前者の方法 (プロキシを使用した方法) を使ってクライアントを作成します。</p>
<p style="margin-top:20px"><a href="#top"><img src="10035-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. WCF サービスを起動する</h2>
<p>ここでは、あらかじめ作成された WCF サービス (および WCF サービスのホスティング) を使用します。WCF サービスの作成方法については、<a href="http://msdn.microsoft.com/ja-jp/netframework/ee676491.aspx">10 行でズバリ !! サービスの作成 (WCF) (C#)</a> を参照してください。</p>
<p>以降では、<a href="http://msdn.microsoft.com/ja-jp/netframework/ee676491.aspx">10 行でズバリ !! サービスの作成 (WCF) (C#)</a> で構築した WCF サービスのサンプルを起動して、この WCF サービスに接続します。このため、あらかじめ Visual Studio を管理者権限で起動してこのサンプル プロジェクトを開き、デバッグ実行をおこなう ([F5] キーを押す) などして、WCF サービスを起動しておきます。(<a href="http://msdn.microsoft.com/ja-jp/netframework/ee676526.aspx">サービスの起動の自動化
 (WCF の IIS ホスティング) (C#)</a> で説明した方法でホスティングをおこなっている場合には、あらかじめ起動をおこなう必要はありません。)</p>
<p style="margin-top:20px"><a href="#top"><img src="10036-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. プロジェクトを作成する</h2>
<p>以下の手順で、Visual Studio を起動して、新規にプロジェクトを作成します。</p>
<p>[ファイル] メニューの [新規作成] を選択して、[プロジェクト] をクリックします。表示される [新しいプロジェクト] ダイアログ ボックスの、プロジェクトの種類ペインで [Visual C#] ノードを展開し、図 1 の通り、[テンプレート] ペインでは [コンソール アプリケーション] をクリックします。([プロジェクト名] ボックス、[場所] ボックスには任意の名称とパスを指定してください。)</p>
<p><img src="10037-image.png" alt=""></p>
<p><strong>図 1. プロジェクト テンプレートを選択してプロジェクトを作成</strong></p>
<p>プロジェクトが作成されたら、Visual Studio の [プロジェクト] メニューの [サービス参照の追加] を選択します。表示される画面 (図 2) の [アドレス] 欄に、上記であらかじめ起動しておいた WCF サービスのアドレスを入力して、[移動] ボタンをクリックします。(接続に成功すると、下図の通り、サービスのエンド ポイントが表示されます。)</p>
<p><img src="10038-image.png" alt=""></p>
<p><strong>図 2. サービス参照の追加</strong></p>
<p>図 2 で [OK] ボタンをクリックすると、WCF サービスと接続するためのクライアント側のプロキシ (ServiceReference1.OrderServiceClient クラス) が自動作成されます。</p>
<p style="margin-top:20px"><a href="#top"><img src="10039-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. コードを実装する</h2>
<p>ここでは、<a href="http://msdn.microsoft.com/ja-jp/netframework/ee676491.aspx">10 行でズバリ !! サービスの作成 (WCF) (C#)</a> で構築した WCF サービスの GetOrderById メソッド (オペレーション) を呼び出して、戻り値の OrderItem クラスの内容をコンソールに出力します。</p>
<p>Program.cs のコードを開き、上記で作成されたプロキシ クラスを使用したコードを下記の通り記述します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
{
  // プロキシの作成とオープン
  using (ServiceReference1.OrderServiceClient proxy = new ServiceReference1.OrderServiceClient())
  {
    // メソッドの呼び出し
    ServiceReference1.OrderItem item = proxy.GetOrderById(3);
    // コンソールへの出力
...
<p style="margin-top:20px"><a href="#top"><img src="10040-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. アプリケーションを実行する</h2>
<p>では、実行してみましょう。</p>
<p>アプリケーションを実行させるには [Ctrl] &#43; [F5] キーを押すか、[デバッグ] メニューから [デバッグなしで開始] を選択します。</p>
<p>WCF サービスの呼び出しに成功すると、下図の通り、コンソールに結果が表示されます。</p>
<p><img src="10041-image.png" alt=""></p>
<p><strong>図 3. アプリケーションの実行</strong></p>
<p style="margin-top:20px"><a href="#top"><img src="10042-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. おわりに</h2>
<p>Visual Studio を使用して、上記のプロジェクト (クライアントのプロジェクト) で作成された App.config を表示すると、クライアント用のサービス構成が設定されているのがわかります。</p>
<p>もし、接続先の WCF サービスのコントラクト (サービス コントラクト/データ コントラクト) などが変更された場合には、クライアント側のプロジェクトのソリューション エクスプローラーに作成されている ServiceReference1 をマウスで右クリックして、[サービス参照の更新] (図 4) を選択することで、プロキシ コードや、上述したサービス構成 (App.config) を自動で更新することができます。</p>
<p><img src="10043-image.png" alt=""></p>
<p><strong>図 4. サービス参照の更新</strong></p>
<hr>
<div>
<table>
<tbody>
