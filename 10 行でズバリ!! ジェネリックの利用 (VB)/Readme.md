...
エバンジェリスト 佐藤 直樹</p>
<p>最終更新日 2005 年 12 月 15 日</p>
<h2><img src="12841-image.png" alt=""> このコンテンツのポイント</h2>
<ul>
<li>.NET Framework 2.0 でのジェネリックのサポートの理解 </li><li>メソッド引数がジェネリック型の動作の理解 </li></ul>
<h2><img src="12842-image.png" alt=""> 今回紹介するコード</h2>
<h3>＜Form1.vb＞</h3>
...
<pre class="hidden"><code class="csharp">Public Shared Sub Swap(Of T)( ByRef LeftParam As T, ByRef RightParam As T)
'ジェネリック型を指定。入れ替えのため、ByRef で渡す
    Dim temp As T
    'パラメータの左側と右側を入れ替える
    temp = LeftParam
...
End Sub</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Public Shared Sub Swap(Of T)( ByRef LeftParam As T, ByRef RightParam As T)
'ジェネリック型を指定。入れ替えのため、ByRef で渡す
    Dim temp As T
    'パラメータの左側と右側を入れ替える
    temp = LeftParam
...
<h3><img src="12843-image.png" alt=""> 今回のシステム要件</h3>
<ul>
<li>Visual Studio 2005 Release Cadidate </li></ul>
<p>&nbsp;</p>
<h3>目次</h3>
<p><a href="#section1">はじめに</a><br>
<a href="#section2">アプリケーションの概要</a><br>
<a href="#section3">アプリケーションの作成</a><br>
<a href="#section4">コードを実装する</a><br>
<a href="#section5">実行する</a><br>
...
<tr>
<td align="center"><img src="12844-image.png" alt=""></td>
<td><a href="http://msdn.microsoft.com/ja-jp/netframework/b7b0c90b-7f31-401b-867f-dd868ff717b4">10 行でズバリ!! ジェネリックの利用 (C#)</a></td>
</tr>
</tbody>
</table>
<p>&nbsp;</p>
...
<p>.NET Framework 2.0 では、ジェネリックがサポートされました。ジェネリックは、特定の型に依存せず、かつ異なる型に対応する、プログラミングにおける機構および要素です。従来、ロジックは同一でも利用する型により複数のクラスやメソッドを記述していたものが、ジェネリックを利用することで汎用的なロジックとして記述することが可能となります。ここでは、ジェネリックタイプパラメータを持つメソッド、換言すると引数がジェネリック型とするメソッドを作成し、ジェネリックの活用例として紹介します。</p>
<h2 id="section2">アプリケーションの概要</h2>
<p>今回、2つの変数に入っている値を入れ替えるアプリケーションを作成します。ここでは、その変数に値を設定するため、テキストボックスでの値を入力するWindows フォームアプリケーションとします。また値入れ替えのロジックを別のパターンでも再利用します。</p>
<h2 id="section3">アプリケーションの作成</h2>
<p>Visual Studio 2005 を起動して、新規プロジェクトを作成します。ここでは、Visual Basic を選択し、&rdquo;Windows アプリケーション&rdquo; テンプレートを選択します。[プロジェクト名] テキストボックスにプロジェクト名を入力し(図1)、[OK] ボタンをクリックします。</p>
<p><img src="12845-image.png" alt=""></p>
<p><strong>図 1. Windows アプリケーションを新規に作成する</strong></p>
<p>今回作成する Windows フォームアプリケーションの開発準備が完了しました。</p>
<p>次にフォームにコントロールを追加して、画面デザインを行います。デザイン画面において、2つのテキストボックスコントロールと2つのボタンコント ロールを配置します。(図 2) 入れ替える2つの文字を入れるテキストボックスと、テキストボックスの文字列を入れ替えて表示するイベントを記述するボタンと、Integer 変数の入れ替えを行うイベントを記述するボタンになります。各コントロールは以下のようにプロパティを設定します。</p>
<table class="grid" border="1" cellspacing="0" cellpadding="5" style="border-collapse:collapse; margin-bottom:10px">
<tbody>
<tr>
...
<td>TextBox1</td>
<td>Text</td>
<td>&ldquo;りんご&rdquo;</td>
</tr>
<tr>
<td>TextBox2</td>
<td>Text</td>
<td>&ldquo;みかん&rdquo;</td>
</tr>
<tr>
<td>Button1</td>
<td>Text</td>
<td>&ldquo;入れ替え&rdquo;</td>
</tr>
<tr>
<td>Button2</td>
<td>Text</td>
<td>&ldquo;別の例&rdquo;</td>
</tr>
</tbody>
</table>
<p>&nbsp;</p>
<p><img src="12846-image.png" alt=""></p>
<p><strong>図 2. 画面上にコントロールを配置したところ</strong></p>
<h2 id="section4">コードを実装する</h2>
...
<pre class="hidden"><code class="csharp">Public Class Form1
    ...
    Public Shared Sub Swap(Of T)(ByRef LeftParam As T, ByRef RightParam As T)
    'ジェネリック型を指定。入れ替えのため、ByRef で渡す
        Dim temp As T
        'パラメータの左側と右側を入れ替える
        temp = LeftParam
...
<p>これは、ジェネリック型を引数とする Shared (共有される)メソッドになります。このジェネリック型を引数とすることで、Swap メソッドの呼び出し時に、String 型や Integer 型等を指定することができます。</p>
<p>次に、&quot;入れ替え&quot; ボタンをダブルクリックし、入力した2つの文字を入れ替えて、メッセージボックスに表示する実装を行います。以下の赤字で書かれているコードを記述します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>最後に、&quot;別の例&quot; ボタンをダブルクリックし、2つの Integer 変数の中身を入れ替えて、メッセージボックスに表示する実装を行います。以下の赤字で書かれているコードを記述します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Private Sub Button2_Click(ByVal sender As System.Object, _ 
ByVal e As System.EventArgs) Handles Button2.Click
    ...
    '引数に Integer 型を指定して Swap メソッドの呼び出し
    Swap(Of Integer)(a, b)

    '呼び出し結果の表示
    MessageBox.Show(&quot;a=&quot; &#43; a.ToString() &#43; &quot; , b=&quot; &#43; b.ToString())
End Sub</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Private Sub Button2_Click(ByVal sender As System.Object, _ 
ByVal e As System.EventArgs) Handles Button2.Click
    ' Integer 型の変数に値を設定
    Dim a As Integer = 100
    Dim b As Integer = 500
...
<p>以上でコードの実装が終わりました。</p>
<h2 id="section5">実行する</h2>
<p>コードの記述が完成しましたので、実行してみましょう。</p>
<p>アプリケーションを実行させるには [F5] キーを押すか、[デバッグ] メニューから [開始] をクリックします。&quot;入れ替え&quot; ボタンをクリックすることで、テキストボックスから入力し変数に設定した値が入れ替わり表示されたことがメッセージボックスを通じて確認できます。 (図3)</p>
<p><img src="12847-image.png" alt=""> <img src="12848-image.png" alt=""></p>
<p><strong>図 3. メッセージボックスでの表示</strong></p>
<p>また、&quot;別の例&quot; ボタンをクリックすることで、&quot;入れ替え&quot; 処理で呼び出しているSwap メソッドが呼び出されます。引数の型が Integer 型ですが、エラーが発生することなる呼び出されることが確認できます。(図4)</p>
<p><img src="12849-image.png" alt=""> &nbsp; <img src="12850-image.png" alt=""></p>
<p><strong>図 4. &ldquo;別の例&rdquo; ボタンの実行での結果</strong></p>
<p>なお、ブレイクポイントを設定し、&quot;入れ替え&quot; ボタンと&quot;別の例&quot; ボタンの処理を追いかけると、Swapメソッド呼び出し時に型が設定されていることが確認できます。(図5,6)</p>
<p><img src="12851-image.png" alt=""></p>
<p><strong>図 5. T 型であった temp 変数が String 型 となる &ldquo;入れ替え&rdquo; ボタンの実行</strong></p>
<p><img src="12852-image.png" alt=""></p>
<p><strong>図 6. T 型であった temp 変数が Integer 型となる &ldquo;別の例&rdquo; ボタンの実行</strong></p>
<h2 id="conclusion">おわりに</h2>
<p>今回は、引数をジェネリック型としたメソッドの実装を紹介しました。この例では、引数を Object 型にすることで類&#20284;のコードを記述することが可能ですが、ボックス化などの型変換によるパフォーマンス上のネックとなることや、実行時の型変換エラーが発生することが少なくありません。ジェネリックにより、コンパイルおよび実行時における型の安全性を保ちつつ、動作が等しいクラスやメソッドの重複記述がなくなるメリットが感じられたのではないでしょうか。その他にも、コレクションでの利用、イベントプロシージャのジェネリックデリゲートを用いた定義等、今回の例以外でも有効な場面が多々あります。このような特徴のもつジェネリックを今後の開発で有効活用して下さい。</p>
<hr>
<table>
<tbody>
<tr>
