...
<p>例 1 の IDisposable インターフェイスの定義では、メンバーとして、Dispose メソッドが定義されています。そもそも、インターフェイスの構成メンバーには実体はありません。ここでも、Sub と End Sub からなるブロック構造にはなっておらず、End Sub はありません。引数や戻り値などの形式を定義するだけです。つまり、メソッド呼び出しでのデータのやり取り方法などのコントラクト (契約情報) が定義されているだけであると言えます。</p>
<p>さらに、このような中身の定義を伴わないインターフェイスは、直接インスタンスを作ることはできません。つまり、次のように New キーワードを用いて、インターフェイス型のインスタンスを作成することはできないのです。これもインターフェイスの基本的で、かつ重要な性&#26684;です。</p>
<p><strong>例 3. インターフェイスから直接インスタンスを作成できない</strong></p>
<p style="margin:20px 0px; padding:10px; background-color:#dedfde">Dim obj As New IDisposable()&nbsp;&nbsp;&nbsp;&nbsp;'&larr;不可! コンパイルエラー!</p>
<p>その代り、クラス (または構造体) を定義する際に、そのインターフェイスを用いて、具体的な実装を定義します。それが、例 2 のインターフェイスを実装した例です。</p>
<p>例 2 の [2] の MyComp1 クラスの定義では、続く行の [3] のように Implements 文を使用して、インターフェイスを使用することを宣言しています。さらに、[4] のように Dispose メソッドの具体的な実装が定義されており、Sub から End Sub までのブロック構造になります。(ここでは簡単にするためブロックだけを記述しました。)</p>
<p>以上、構文上の基本的な記述方法もふまえ、インターフェイスの基本事項を改めて確認しましたが、ここまでは比較的難しくはないと思います。イメージしにくいのは、このあとどう利用したらよいかという点ではないでしょうか。順にこの点を確認していきましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16834-image.png" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. インターフェイス型の変数の扱い方</h2>
<p>初心者にとってインターフェイスの覚えたてのころに意外と見落としがちなのが、インターフェイス型の変数の扱いです。この変数の扱い方を正しく理解することは、インターフェイスの使い方を正しく理解する上で重要です。</p>
<p>次の例は、インターフェイス型の変数の基本的な使用方法を示した例です。例 4 の [1] では、インターフェイス型の変数を宣言しています。</p>
<p><strong>例 4. インターフェイス型の変数の利用</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Dim idisp As IDisposable	&larr;[1]
idisp = New MyComp1()		&larr;[2]</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Dim idisp As IDisposable	&larr;[1]
idisp = New MyComp1()		&larr;[2]</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
...
<p>ここで、例 4 の [1] では、「As IDisposable」と変数を宣言した以上、その変数はその「IDisposable 型」のインスタンスを表します。しかし、前述したように、IDisposable インターフェイスのインスタンスを作成することは出来ないので、こうした変数が宣言可能なのはおかしくないかと思われるかもしれません。</p>
<p>その答えは、[2] にあります。[2] は、インターフェイス型の変数にとって重要な、次の性質が現われています。</p>
<p>「インターフェイスを実装したオブジェクト インスタンスは、インターフェイス型の変数で表わすことができる (インターフェイス型の変数で参照することができる)」</p>
<p>この [2] では、MyComp1 クラスのインスタンスを作成しています。このインスタンスの参照を変数 idisp に代入して、変数 idisp が MyComp1 インスタンスを表すようにしています (参照するようにしています)。このような代入が可能なのは、MyComp1 インスタンスには、IDisposable インターフェイスが実装されているからです。</p>
<p>決して、代入文の右辺 (MyComp1 型) と左辺 (IDisposable 型) との型が同一でないからと言って、コンパイル エラーになることはありません。この性質は Visual Basic に限らず、一般的なオブジェクト指向プログラミング言語であれば備えている性質です。実際に、Visual Basic ではコンパイラの型のチェックを厳しくするために、コンパイルオプションを「Option Strict On」の状態に設定しても、[2] の構文は、正しい代入文として有効です。</p>
<div style="margin:10px 0; padding:10px 10px 0 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>Note:</strong> インターフェイス型の変数がインスタンスを参照することから想像付くと思いますが、インターフェイスは「参照型」に分類されます。</p>
</div>
<p>さて、これでインターフェイスの重要な性&#26684;が確認できました。これをふまえ、インターフェイスの効能を改めて確認し、その用途を見極めていきましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16835-image.png" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. インターフェイスの効能と用途 ～疎結合での活用～</h2>
<p>インターフェイスによってもたらされる主な効能の 1 つを端的に挙げると、「オブジェクト間の連携をする際に、依存部分を少なくし疎結合にする」ことです。次の例で確認してみましょう。</p>
<p><strong>例 5. IStore インターフェイスとインターフェイスを実装したクラス</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
