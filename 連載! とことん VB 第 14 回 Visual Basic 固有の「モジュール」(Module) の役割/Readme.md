...
<pre class="hidden"><code class="csharp">Private Sub TestProc01()
     ...
     Dim c = MyUtil.PI			&larr;[4] 共有変数としての使用
...
Private Sub Proc1(ByVal obj As MyUtilC)	&larr;[4]
    :
End Sub</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Dim obj1 As MyUtilC		&larr;[1]<br>Dim obj2 As New MyUtilS()	&larr;[2]<br>Dim a = obj2.PI			&larr;[3]<br>    :<br>Private Sub Proc1(ByVal obj As MyUtilC)	&larr;[4]<br>    :<br>End Sub</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>上記の [1] のようにインスタンスを表す変数として定義できるほか、[2] のようにインスタンスを作成した上で、[3] のように共有メンバーにアクセスすることもできます。(ただし、[3] の場合はコンパイル警告になります。) また、[4] のようなインスタンスを渡す引数としても記述できてしまいます。インスタンスが不要な「ユーティリティ クラス」という用途として使用するのであれば、これらの記述は意味がありません。「モジュール」として「ユーティリティ クラス」を定義してあれば、例 5 に挙げた記述はすべて不正となり、「ユーティリティ
 クラス」としての不適切な使用を構文レベルで抑止できるのです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16828-image.png" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. クラスとの内部実装の違い</h2>
<p>実は、前項のように構文レベルで「モジュール」がクラスよりも向いている点のほかに、内部的な実装でも「モジュール」が優れている点があります。</p>
<p>クラスは、例えば例 3 の [1] のように「ユーティリティ クラス」として共有メンバーだけを定義したとしても、コンストラクターを記述しないと、コンパイルの際に、必ず引数無しの既定のコンストラクターが生成されてしまうのです。この既定のコンストラクターは、正確にいうと、インスタンスの生成時の初期化を行うためのものなので、「インスタンス コンストラクター」とも呼ばれています。例 3 の「ユーティリティ クラス」では本来インスタンスの作成は不要ですから、この「インスタンス コンストラクター」が既定で生成されることは無駄であると言えます。</p>
<p>しかし、「モジュール」であれば、言語仕様上も「インスタンス コンストラクター」を持つことはできず、コンパイラーが既定の「インスタンス コンストラクター」を自動生成することもありません。「モジュール」は、この点でも「ユーティリティ クラス」に最適化されていると言えます。</p>
<div style="margin:10px auto; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px #333333; width:550px">
<p><strong>Note:</strong> 共有変数に初期値を設定した場合は、モジュール、クラス、および構造体のいずれも、共有変数を初期化するための「共有コンストラクター」が自動生成されます。(「<a href="http://beta.code.msdn.microsoft.com/13-4ee47d39">第 13 回 自動実装プロパティの役割と、そこから読み取れるプロパティの用途</a>」を参照)</p>
</div>
<p>以上、「モジュール」が「ユーティリティ クラス」として使用することに向いている点を、様々な角度から確認してみました。</p>
<p>最後に、この Visual Basic 固有の「モジュール」が他の言語からどう見えるか、つまり、他の言語と互換性があるか補足します。</p>
<p style="margin-top:20px"><a href="#top"><img src="16829-image.png" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. C# 言語との互換性</h2>
<p>結論からいうと、他の言語からも Visual Basic の「モジュール」を参照することができます。その参照方法は、言語によって若干異なるので、代表的な例として C# について取り上げます。</p>
<p>C# から見た場合、共有メンバーから構成される「モジュール」は、「静的クラス」(static クラス) として認識されます。C# の「静的クラス」も共有メンバーだけから構成され、インスタンスの作成はできません。このことから、C# の「静的クラス」は Visual Basic の「モジュール」に相当する型と言うことができます。そして、やはり「静的クラス」の代表的な使用方法は「ユーティリティ クラス」です。</p>
<div style="margin:10px auto; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px #333333; width:550px">
<p><strong>Note:</strong> 正確には C# の用語では、Visual Basic の「共有メンバー」のとこを「静的メンバー」 (static メンバー) と呼びます。よって、「静的メンバー」から構成されるクラスは「静的クラス」と呼ばれています。</p>
</div>
<p>そもそも、このような共有メンバーから構成され、インスタンスを作成できないという性質の型は、.NET Framework レベルで実装が可能であり、Visual Basic と C# では、それぞれ「モジュール」、「静的クラス」として表現していたのです。</p>
<div style="margin:10px auto; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px #333333; width:550px">
<p><strong>Note:</strong> ただし、「モジュール」と「静的クラス」はまったく同一というわけではありません。共有メンバーから構成され、インスタンス作成が不可である点は同じなのですが、Visual Basic ではコンパイラーが出力する際に、出力した「型」に StandardModule という独自の属性を追加します。この属性は、Visual Basic の環境が「モジュール」であることを認識するための内部的な属性です。.NET Framework のクラスライブラリに、この属性は StandardModuleAttribute
 クラスとして定義されていますが、この属性は .NET Framework の内部実装のために用意されたものであり、プログラマーが使用することを意図したものではありません。</p>
</div>
<p>参考までに、例 1 の「モジュール」を C# から利用した例を示します。各行の右側には、対応する Visual Basic のコードを書いておきました。</p>
<p><strong>例 6. C# からの Visual Basic のモジュールの使用</strong></p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">MyUtil util1;			// [1] &larr;不可! Dim util1 As MyUtil と同じ
