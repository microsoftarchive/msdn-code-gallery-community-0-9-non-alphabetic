# 10 行でズバリ!! [C#] Web アプリケーション設定情報の格納と取得
## License
...
<div id="top">
<p>更新日: 2010 年 10 月 22 日</p>
<table border="0" cellspacing="0" cellpadding="5" bgcolor="#e5f1f8" style="margin-bottom:10px">
<tbody>
<tr>
<td style="padding:10px 10px 10px 10px"><img src="9310-image.png" alt=""> Visual Basic の内容はこちらに掲載しています。<a href="http://msdn.microsoft.com/ja-jp/netframework/gg286373.aspx">10 行でズバリ!! [VB] Web アプリケーション設定情報の&#26684;納と取得</a></td>
</tr>
</tbody>
</table>
<h2><img src="9311-image.png" alt=""> このコンテンツのポイント</h2>
<ul>
<li>Web アプリケーションの構成情報を&#26684;納するために、Web.config を利用する </li><li>Web サーバー コントロールのタグやプログラム コードから、Web.config の構成情報を参照する </li></ul>
<h2><img src="9312-image.png" alt=""> 今回紹介するコード</h2>
...
&lt;configuration&gt;<br>
&nbsp; &lt;appSettings&gt;<br>
&nbsp;&nbsp;&nbsp; &lt;add key=&quot;SiteTitle&quot; value=&quot;CodeRecipe サンプル&quot;/&gt;<br>
&nbsp;&nbsp;&nbsp; &lt;add key=&quot;MyColor&quot; value=&quot;LightGreen&quot;/&gt;<br>
&nbsp; &lt;/appSettings&gt;<br>
&lt;/configuration&gt;</div>
<p>&nbsp;</p>
<p style="margin-bottom:0px">&nbsp;<strong>&lt;Site.master&gt;</strong></p>
<div style="margin:5px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
&lt;h1&gt;<br>
&nbsp;&nbsp; &lt;asp:Literal runat=&quot;server&quot; Text=&quot;&lt;%$ appSettings:SiteTitle %&gt;&quot; /&gt;<br>
&lt;/h1&gt;</div>
<p style="margin-bottom:0px"><strong>&nbsp;</strong></p>
<p style="margin-bottom:0px"><strong>&nbsp;</strong></p>
<p style="margin-bottom:0px"><strong>&lt;Default.aspx.cs&gt;</strong></p>
...
{
    string colorName = ConfigurationManager.AppSettings[&quot;MyColor&quot;];
    Label1.Text = colorName;
    Label1.BackColor = <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Drawing.Color.FromName.aspx" target="_blank" title="Auto generated link to System.Drawing.Color.FromName">System.Drawing.Color.FromName</a>(colorName);
}</code></pre>
</div>
</div>
...
<li><a href="#01">はじめに</a> </li><li><a href="#02">サンプル Web アプリケーションの作成準備</a> </li><li><a href="#03">Web 構成ファイルの修正</a> </li><li><a href="#04">Web 構成情報の参照 (ASP.NET 式による埋め込み)</a> </li><li><a href="#05">Web 構成情報の参照 (プログラムによる参照)</a> </li><li><a href="#06">おわりに</a> </li></ol>
<hr>
<h2 id="01">1. はじめに</h2>
<p>Web アプリケーションの設定は、実行環境ごとに値を変えたり、運用中に変更したいことがあります。その際、ソース コードに記述された値を変更すると、再コンパイルが必要になり、保守や運用にかかる手間やコストの負担が増えます。</p>
<p>ASP.NET には、あらかじめ変更が予想されるアプリケーションの設定情報を、アプリケーションの実装とは分離して、柔軟に変更できる仕組みが用意されています。</p>
<p>ここでは、簡単な Web アプリケーションを作成して、アプリケーションの構成情報を参照する方法について解説します。</p>
<p style="margin-top:20px"><a href="#top"><img src="9313-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. サンプル Web アプリケーションの作成準備</h2>
<p>まず、Visual Studio を起動して、[ファイル] メニューの [新規作成] から [プロジェクト] をクリックし、[新しいプロジェクト] を開きます。次に、[インストールされたテンプレート] で [Web] カテゴリーにある [ASP.NET Web アプリケーション] を選びます。[名前] には任意のプロジェクト名を指定できますが、ここでは「SiteConfig」とします。</p>
<p><img src="9314-image.png" alt=""></p>
<p><strong>図 1. [新しいプロジェクト] ダイアログで、ASP.NET Web アプリケーションを新規作成</strong></p>
<p>[OK] をクリックすると、SiteConfig という新しい Web サイト用のプロジェクトと、ソリューションが作成されます。</p>
<p><img src="9315-image.png" alt=""></p>
<p><strong>図 2. プロジェクト作成直後の Visual Studio 2010</strong></p>
<p style="margin-top:20px"><a href="#top"><img src="9316-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. Web 構成ファイルの修正</h2>
<p>ソリューション エクスプローラーで SiteConfig プロジェクトを見ると、Web.config というファイルが追加されていることがわかります。もし、このファイルがない場合は、ソリューション エクスプローラーで Web サイト名を右クリックし、[新しい項目の追加] を選び、「Web 構成ファイル」を追加します。</p>
<p>アプリケーションの設定は、この Web.config に追加します。ソリューション エクスプローラーで、Web.config をダブルクリックして編集画面に表示してください。このファイルには、すでに自動生成されたプロジェクトのために使われる設定が含まれています。アプリケーションのための設定を追加するには、appSettings という要素を追加します。ここでは、次のような要素を追加します。</p>
<div style="margin:5px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>&lt;configuration&gt;<br>
...
&nbsp;&lt;/connectionStrings&gt;<br>
<br>
<strong>&nbsp;&lt;appSettings&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;add key=&quot;SiteTitle&quot; value=&quot;CodeRecipe</strong><strong> サンプル&quot; /&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;add key=&quot;MyColor&quot; value=&quot;LightGreen&quot; /&gt;<br>
&nbsp;&lt;/appSettings&gt;</strong><br>
<br>
&nbsp;&lt;system.web&gt;<br>
&nbsp;&nbsp;&nbsp;&lt;compilation debug=&quot;false&quot; targetFramework=&quot;4.0&quot; /&gt;</p>
</div>
<p>設定を追加するときには、appSettings の中に add という要素を使い、key で値を参照するための文字列を、value で実際に使われる文字列を指定します。</p>
<div style="margin:10px 0; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>Note:</strong> Web.config は階層的に適用されます。&lt;Windows ディレクトリ&gt;\Windows\Microsoft.NET\Framework\&lt;バージョン&gt;\Config にある Web.config ファイルが、もっとも基本的な設定です。各ディレクトリに置かれる Web.config ファイルは、その子ディレクトリに適用され、設定をオーバーライドする場合に、Web.config ファイルを用意します。</p>
</div>
<p style="margin-top:20px"><a href="#top"><img src="9317-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. Web 構成情報の参照 (ASP.NET 式による埋め込み)</h2>
<p>次に、Web.config に設定した情報を利用する方法を紹介します。これには、.aspx ファイルで直接参照する方法と、プログラム コードから参照する方法があります。</p>
<p>まず、.aspx で参照する方法を見るために、ソリューション エクスプローラーで SiteConfig 下にある Site.Master をダブルクリックして編集できるようにします。もし、[デザイン] ページが表示されていたら、左下の [ソース] タブをクリックしてください。</p>
<p>見出し (h1) 要素として「マイ ASP.NET アプリケーション」という文字列が使われていますが、これを次のように書き換えます。</p>
<div style="margin:5px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>&lt;body&gt;<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;div class=&quot;title&quot;&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;h1&gt;<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;asp:Literal runat=&quot;server&quot; Text=&quot;&lt;%$ appSettings:SiteTitle %&gt;&quot; /&gt;</strong><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;/h1&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;/div&gt;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&lt;div class=&quot;loginDisplay&quot;&gt;</p>
</div>
<p>「&lt;%$」から「%&gt;」で囲まれた部分は、<a href="http://msdn.microsoft.com/ja-jp/library/d5bd1tad.aspx">ASP.NET 式</a>と呼ばれる記述方法で、構成情報 (Web.config) の appSetting などの情報を、静的に .aspx の中に埋め込むことができます。このように構成ファイルを利用することで、後から文字列を変更したい場合に、.aspx を修正せず、構成ファイルだけを変更すれば済むようになります。</p>
<p>プログラムを実行した様子を図 3 に示します。</p>
<p><img src="9318-image.png" alt=""></p>
<p><strong>図 3. 構成ファイルで設定したタイトルが使われている (赤い囲み部分)</strong></p>
<p>なお、ASP.NET 式を使う場合には、構成ファイルに対応するキーが含まれていない場合、エラーが発生して Web ページが正しく表示されなくなることに注意してください。</p>
<p style="margin-top:20px"><a href="#top"><img src="9319-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. Web 構成情報の参照 (プログラムによる参照)</h2>
<p>次に、ソリューション エクスプローラーで、Default.aspx をダブルクリックします (または、編集画面の上部に [Default.aspx] があれば、それをクリックします)。デフォルトの設定では、Default.aspx を編集するソース ビューが表示されます。ここで編集画面の左下にある [デザイン] タブをクリックして、Web ページのビジュアル編集画面に切り替えます。ここで、「ツール ボックス」の標準コントロールから、Button と Label を図 4 のように配置します。</p>
<p><img src="9320-image.png" alt=""></p>
<p><strong>図 4. Default.aspx に Button と Label を配置する</strong></p>
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.aspx" target="_blank" title="Auto generated link to System.Web.UI">System.Web.UI</a>;<br>
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.WebControls.aspx" target="_blank" title="Auto generated link to System.Web.UI.WebControls">System.Web.UI.WebControls</a>;<br>
<strong>using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Configuration.aspx" target="_blank" title="Auto generated link to System.Configuration">System.Configuration</a>;<br>
&nbsp;</strong><br>
public partial class _Default : <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.Page.aspx" target="_blank" title="Auto generated link to System.Web.UI.Page">System.Web.UI.Page</a><br>
{<br>
...
&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;string colorName = ConfigurationManager.AppSettings[&quot;MyColor&quot;];<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Label1.Text = colorName;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Label1.BackColor = <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Drawing.Color.FromName.aspx" target="_blank" title="Auto generated link to System.Drawing.Color.FromName">System.Drawing.Color.FromName</a>(colorName);<br>
&nbsp;&nbsp;&nbsp;}</strong><br>
}</p>
</div>
<p>ConfigurationManager は構成ファイルの情報を管理するためのクラスです。このクラスの AppSettings メンバーは連想配列になっており、指定したキー文字列に対応する値文字列を返します。戻り値は常に文字列型です。したがって、ASP.NET の要素として「BackColor=&quot;Yellow&quot;」のように指定する場合でも、プログラムではプロパティの型に合わせた値 (BackColor の場合は、<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Drawing.Color.aspx" target="_blank" title="Auto generated link to System.Drawing.Color">System.Drawing.Color</a> 型) に変換する必要があります。</p>
<p>ここでは、MyColor というキーに割り当てられた文字列を Label1 の Text に、その文字列に対応する色の値を BackColor に代入しています。プログラムを実行して、ボタンをクリックすると、図 5 のようになります。</p>
<p><img src="9321-image.png" alt=""></p>
<p><strong>図 5. プログラム コードで構成ファイルの設定情報を利用している様子</strong></p>
<p style="margin-top:20px"><a href="#top"><img src="9322-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. おわりに</h2>
<p>Web サーバーを運用する環境が違う場合、あるいは後から設定を変更するような場合でも、Web.config から設定情報を参照することで、Web アプリケーション自身のソース コードを変更する必要がなくなります。これにより、Web アプリケーションの保守や運用の手間が軽減できます。</p>
<hr>
<div>
<table>
<tbody>
