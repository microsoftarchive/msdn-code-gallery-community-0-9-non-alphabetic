# 連載! とことん C#: 第 15 回 部分クラス (partial class) でクラスを分ける
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 3 月 19 日</span></h4>
</div>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「生徒のジョシュアです。ところで、フィールドが 1 つあるのですが、ソース ファイルを探しても定義がありません。」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「ふむふむ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「ところが、Visual Studio の『定義へ移動』のメニューを使うと、別のファイルが開いて private の定義に飛んでいくんです。これっておかしくないですか? private のフィールドは同じクラス内でしかアクセスできないはずですよね?」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「partial じゃ!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「partial ってなんですか?」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">部分クラス (partial) でクラス定義は分かれる</a> </li><li><a href="#02">部分クラスは、なぜ必要とされるのか?</a> </li><li><a href="#03">ツールの力を借りてソースを書こう</a> </li><li><a href="#04">結末</a> </li></ol>
<hr>
<h2 id="01">1. 部分クラス (partial) でクラス定義は分かれる</h2>
<p>さっそく、partial キーワードを使った部分クラスの定義を見ていきましょう。</p>
...
<p>このソース コードには A というクラスの定義が 2 つあります。しかし、意味的に実際に存在するクラスは 1 つであり、別のソース コード (上記のように、分離された定義) 上にある private メンバーへのアクセスも可能です。例えば、上記の例で、フィールド a は、同じクラスにあると見なされるので、2 番目の (A の) クラス定義の中からでもアクセス可能です。</p>
<p>この 2 つの定義は、別のソース ファイル上にあっても構いません。しかし、それら複数のソース ファイルは同時にコンパイルする必要があります。単に、ソース コードがファイルとして分割されているだけで、実体としては 1 つのクラスだからです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16600-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 部分クラスは、なぜ必要とされるのか?</h2>
<p>では、なぜこのような機能が必要とされているのでしょうか?</p>
<p>以下の解釈は必ずしも正しくありません。</p>
<ul>
<li>ソース コードを書き換えることなく機能を拡張するため </li></ul>
<p>クラスのメンバーを増やすためであれば、単に継承を行うだけで良いのです。しかも、継承で機能を追加する場合は、追加する相手が別モジュールにあるバイナリーであっても構いません。あるいは、拡張メソッドという手段もあります。</p>
<p>また、以下の解釈も、同様に必ずしも正しくありません。(そもそも、そのような巨大なクラスを書くことは、保守性を下げるので避けるべきです。)</p>
<ul>
<li>巨大なクラスを適切なサイズのソース ファイルとして分割するため </li></ul>
<p>では、理由はいったい何でしょうか?</p>
<p>実際、Visual Studio ではこの機能が多用されています。例えば、Windows フォーム アプリケーションや ASP.NET アプリケーションでは、デザイナーでコントロールを配置した結果を反映するソース ファイルと、実際にプログラマー自身がプログラムを記述するソース ファイルが分離されていて、これらは、部分クラス (partial) を使った 1 つのクラスになっています。(そのため、プログラマーは、Visual Studio が自動的に生成した private なメンバーを参照してコードを記述できます。)</p>
<p>このように、ツール (Visual Studio の IDE) が自動生成するソース ファイルとプログラマー自身が書くソース ファイルを分けることで、例えば、ツールにそのファイルを再作成させてもプログラマー自身が作成したファイルは変更しないで済みます。また、プログラマーが記述した箇所が、ツールによってうっかり書き換えられてしまうケースも無くなるでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16601-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. ツールの力を借りてソースを書こう</h2>
<p>一例として ASP.NET のプロジェクトを作成してみましょう。以下は、Visual Studio で ASP.NET のプロジェクトを作成し、フォーム上にボタンとラベルを配置し、ボタンをダブルクリックして、ラベルの表示内容を書き換えるコード (ボタン クリック時のイベントのコード) を書き込んだ例です。</p>
...
namespace WebApplication1 {
    ...
    public partial class _Default {
        ...
        /// &lt;/remarks&gt;
        protected global::<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.WebControls.Button.aspx" target="_blank" title="Auto generated link to System.Web.UI.WebControls.Button">System.Web.UI.WebControls.Button</a> Button1;
        ...
        /// 変更するには、フィールドの宣言をデザイナー ファイルから分離コード ファイルに移動します。
        /// &lt;/remarks&gt;
        protected global::<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.WebControls.Label.aspx" target="_blank" title="Auto generated link to System.Web.UI.WebControls.Label">System.Web.UI.WebControls.Label</a> Label1;
    }
}</code></pre>
</div>
</div>
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.aspx" target="_blank" title="Auto generated link to System.Web.UI">System.Web.UI</a>;
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.WebControls.aspx" target="_blank" title="Auto generated link to System.Web.UI.WebControls">System.Web.UI.WebControls</a>;

namespace WebApplication1
{
    public partial class _Default : <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Web.UI.Page.aspx" target="_blank" title="Auto generated link to System.Web.UI.Page">System.Web.UI.Page</a>
    {
...
<p>(ボタン コントロールと、上記の Button1_Click ハンドラーの関連付けは、.aspx ファイルに記述されています。)</p>
<p>この例から言えることは、この 2 つファイルは役割によって明確に差があるということです。</p>
<p>どちらのファイルも、ツール (Visual Studio) が作成したものです。(Default.aspx.designer.cs のすべての行と、Default.aspx.cs の Label1.Text = &quot;Hello!&quot;; 以外のすべての行は、ツールが用意してくれたものです。) ところが、そのあとの管理という点では、まったく異なります。つまり、ソース コードの更新を必要とするとき、以下のような決定的な差があります。</p>
<ul>
<li>Default.aspx.designer.cs の更新はすべてツール (デザイナー) に任せるべき (Visual Studio のデザイナーで変更すべき)
</li><li>Default.aspx.cs の更新はすべてプログラマーが責任をもって行うべき </li></ul>
<p>つまり、Default.aspx.designer.cs はデザイナーで修正する限り整合性が取れます。一方で、Default.aspx.cs は、ファイルそのものは確かに自動生成で用意してくれるものの、それは初期状態を提供しているだけです。あとから、いかなる修正を加えても、整合性さえ取れていれば構いません。例えば、クラスの名前を変えるのは整合性が取れなくなるので NG ですが、不要な using を取ったり、メソッドの順番を入れ替えるのは構いません。Button1_Click というメソッド名も、あくまでツール
 (Visual Studio) が仮に作成したものであり、Button1 コントロールの Click イベントのハンドラーがその名前 (Button1_Click) である必要はありません。他の名前で作成しなおして、デザイナーのプロパティ (ボタン コントロールの OnClick) を変更しても構いません。</p>
<p>ソース コードの意味が理解できてくると、つい自分ですべてのコードをなおしたくなるかもしれませんが、この partial を使って作成されたクラスについては、ツール (Visual Studio のデザイナーなど) に任せて変更をおこなうべきでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16602-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">結末</h2>
<table border="0" cellspacing="0" cellpadding="0" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました。クラス定義は複数のファイルに分割できるわけですね!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。クラスを最初から最後まで見たと思っても、別のファイル上に定義の続きが残っている場合もあるのじゃ!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「ありがとうございます。これでソースの理解も倍増です!」</td>
</tr>
</tbody>
</table>
