# 連載! とことん C#: 第 4 回 型について知ろう (decimal と double/float の違い)
## Requires
- 
## License
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、float 型を使って怒られたことがあります。でも、代わりに double 型を使っても怒られました」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「どういうプログラムだね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ドルの値段を処理するプログラムで、セントは小数点以下の数として扱います。だから整数の int ではダメなんです」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「なるほど。それは計算誤差を減らすために decimal を使えという要望だね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「decimal って何ですか?」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">本当は 3 つある数値型</a> </li><li><a href="#02">実数の罠</a> </li><li><a href="#03">1 セントはどこに消えた?</a> </li><li><a href="#04">decimal を使った解決</a> </li><li><a href="#05">decimal は万能ではない</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. 本当は 3 つある数値型</h2>
<p>プログラムで使用する型は様々です。</p>
<p>大きく分けると以下の種類に分かれます。</p>
<ul>
<li>自分で定義したり、ライブラリが提供する構造体やクラス等 </li><li>プログラムを示すデリゲート型 </li><li>文字列を示す string 型 </li><li>数値を扱う数値型 </li></ul>
<p>この中で、特に数値型は種類が多く、充実しています。</p>
<ul>
<li>整数 (int や ushort など) </li><li>2 進表現の浮動小数点実数 (float,double) </li><li>10 進表現の浮動小数点実数 (decimal) </li></ul>
<p>整数は種類が多いものの、扱いは簡単です。種類は、ほとんど、ビット幅と符号の有無で区別されています。必要な数値を記録するに十分な表現力があれば、それで良いわけです。</p>
<p>しかし、この点を誤るとおかしな結果に容易にたどり着きやすいのも整数です。以下のサンプル コードは、キャスト 1 つで同じ数値の持つ値がまるで別物になるという例です。</p>
...
<p>実行結果:</p>
<p>32768==-32768 -1==4294967295</p>
</div>
<p>これらの結果に至る理由は簡単です。キャストによって型を変換する際に、型の表現力が足りないため、同じビットが&#20284;て非なる別の数値に見えるのです。例えば、short は最大 32767 であるため、32768 は入りません。強引に入れればおかしな数値に化けます。同様に負数が表現できない符号無し整数の uint に負数を強引にキャストすれば、意味のない正の整数に化けてしまいます。</p>
<p>このような問題が起きることへの対策として、以下のことが行われます。</p>
<ul>
<li>特に理由が無ければ、表現力が豊かであり、負数も表現でき、昨今の PC の CPU が効率よく処理できる 32bit の符号付き整数である int 型を使っておく
</li><li>事前のチェックを強化し、不正な値は例外で跳ねるなどの措置を執る (check コンテキストもその一部) </li></ul>
<p>しかし、int を使わない理由もいくつかあります。代表的な例は以下です。</p>
<ul>
<li>符号無しの意図を特別に明示したいとき </li><li>大量に確保するのでメモリを節約したいとき </li><li>int でも表現力が足りないとき </li></ul>
<p>しかし、だいたいは int を使っておけば良いでしょう。int 以外の使用を検討するのは、明らかに int で不都合がある場合だけで良いでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16750-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 実数の罠</h2>
<p>整数の最大の制約は、小数点以下の数を扱えないことです。もし、小数点以下の数も扱うとすれば整数型では不十分です。ただし、上手くやれば整数演算に持ち込める場合もあります。</p>
<p>例えば、消費税が 5% としたとき、常識的には 5% は 0.05 であるため小数の計算が必須に思えます。以下は、100 円の品物の消費税を計算するものです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    static void Main(string[] args)
    {
        ...
        Console.WriteLine(&quot;{0}, {1}&quot;,price,tax);
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>しかし、この程度なら整数だけの計算に持ち込めます。tax の型と計算式を以下のように直します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">int tax = price * 5 / 100;</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">int tax = price * 5 / 100;</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>これで実行結果は同じですが、すべて整数で計算されます。</p>
<p>しかし、このようなテクニックは常に可能というわけではありません。どうしても小数点以下の数が必要とされるとき、実数は避けて通れません。しかし、なぜ float や double ではダメな場合があるのでしょうか?</p>
<p style="margin-top:20px"><a href="#top"><img src="16751-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. <span style="margin-left:15px">1 セントはどこに消えた?</span></h2>
<p>以下のプログラムは、一千兆ドルを持っている大金持ちの口座に、1 セントを入金した例です。ところが、1 セントを入金したのに、その 1 セントは消えてしまいました。なぜでしょう? あまりに少額なので無視されたのでしょうか?</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>実は、double 型は計算結果が狂う可能性があり、プログラマの中にはできれば避けたいと考えている方も多いでしょう。float 型も表現できるビット数が少ないだけで同様です。では、なぜ double/float 型は計算結果が狂いやすいのでしょうか?</p>
<p>その理由は 2 つあります。</p>
<ul>
<li>小数点を左右に移動させて巨大数や極小数も扱えるため、実際の桁数より大きな数値、小さな数値も扱えてしまう </li><li>内部は 2 進数だが出力は 10 進数なので、見た目は割り切れても実際は割り切れない数が出てしまう </li></ul>
<p>先ほどの「消えた 1 セント」の結果は、主に 1 つ目の理由によります。確かに、double 型は一千兆という巨大数を扱えますが、それは小数点を移動して得られた数字であり、全ての桁数を表現できるわけではありません。それゆえに、大きい方の桁が優先されて小さい桁は消失します。つまり、1 セントは表現できずに消失します。</p>
<p>後者も、体感するのは容易です。以下のプログラムは小さな桁数の 2 つの数値を比較しています。双方は同じだと思えるかもしれませんが、結果は True (同じ) になりません。2 進数に変換した時点で、桁数の範囲内で単純に割り切れる数値にならないためです。そのため、桁数の少ない float 型定数 0.1f と、桁数の多い double 型定数 0.1d は同じになりません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p style="margin-top:20px"><a href="#top"><img src="16752-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. decimal を使った解決</h2>
<p>1 セントが消えてしまう問題を解消する最も簡単な方法は、double 型ではなく decimal 型を使うことです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>100000000000000.01</p>
</div>
<p>このように、1 セントが消えてしまうことがありません。このような結果が出る理由は以下の通りです。(特に、今回は、2 番目の理由により解決されます。)</p>
<ul>
<li>内部で日常生活で使用する 10 進数を基数としている (表現できる桁数内なら、誤差は出ない) </li><li>また、有効な桁数も多い </li></ul>
<p>そのため、特に小数の絡むお金の計算は double/float を避け、整数計算に持ち込むか、decimal 型で計算する場合がよくあります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16753-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. decimal は万能ではない</h2>
<p>一方、decimal は小数を移動させる範囲は小さいため、逆に double と違い、極端に大きい数や、極端に小さい数を扱うことができません。そのため、1 セントでも欠けると問題になるお金の計算には適しますが、天文学などの巨大な数値の計算には向きません。</p>
<p>例えば、以下のようなコードは書けません。いくら、decimal 型の桁数が大きくとも、それを越えてしまえば手も足も出ません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">decimal a = 100000000000000000000000000000m;</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">decimal a = 100000000000000000000000000000m;</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>しかし、double を使うと以下のコードは記述できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">double a = 100000000000000000000000000000d;</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">double a = 100000000000000000000000000000d;</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>これは、桁数が少なくとも小数点を移動して対処できるからです。計算の正確さより、巨大な桁数を扱うことを優先する場合、decimal 型よりもむしろ double 型の方が良い場合もあるのです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16754-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました。decimal を使えば、それだけ誤差を減らせるのですね!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。しかし、万能ではない。型は適切に選んで使うことが重要だが、そのためには既存の型をちゃんと理解して使う必要があるのじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
