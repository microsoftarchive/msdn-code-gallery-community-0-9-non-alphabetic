...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 7 月 23 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、せっかく 4 コアの最新 CPU を搭載した開発機を支給されたのに、なかなか CPU パワーを使い切れません。どうすれば、このパワーを余すところ無く使えるプログラムが書けますか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「それは並列処理を活用することじゃ!」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">バックグラウンドで処理させたい</a> </li><li><a href="#02">タスク (Task) が終わったとき実行するタスク (Task)</a> </li><li><a href="#03">実行すべき処理が複数あるとき</a> </li><li><a href="#04">私はだあれ? (データに応じた並列処理)</a> </li><li><a href="#05">並列処理できるコレクション</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. バックグラウンドで処理させたい</h2>
<p>バックグラウンドで実行させたいコードがある場合、<a href="http://msdn.microsoft.com/ja-jp/ff427442.aspx">第 12 回</a>でも紹介した「スレッド」を使用すれば実現は難しくありません。しかし、.NET Framework 4.0 以降 (Visual Studio 2010 以降) になると、もっと簡単に記述できるようになります。</p>
...
<p>この実行結果からわかるように、StartNew メソッドで作成されたタスクが 0 から 9 まで数えている (出力している) 間に、メイン スレッドからのメッセージ出力 (「in progress」の出力) が割り込んでいます。</p>
<div style="margin:10px 0; padding:10px 10px 0 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>Note:</strong> ここで使用している「タスク」という用語に注意してください。<a href="http://msdn.microsoft.com/ja-jp/ff427442.aspx">第 12 回</a>で説明した「スレッド」と、ここで使用している「タスク」は異なり、上記のタスクの作成では、必ずしも、個々のタスクについてそれぞれスレッドが作成されるわけではなく、.NET Framework が持つタスク スケジューラにより、各タスクと使用するスレッドが管理されています。</p>
</div>
<p>この場合、問題となるのは同期です。特に重要なのは、バックグラウンドで実行しているコードの実行が終了したことを、いかにして知るかです。これは上述の通り、単に Task オブジェクトの Wait メソッドを呼び出すだけで、終了を待つことができます。タスクが複数ある場合は、WaitAll メソッドや WaitAny メソッドが使用できます。(どちらのメソッドも配列で複数の Task オブジェクトを渡し、すべて、あるいはいずれか 1 つが終了した時点で制御が戻ってきます。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16725-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. タスク (Task) が終わったとき実行するタスク (Task)</h2>
<p>ある Task が終わったとき、次に実行すべき Task を登録したいことがあります。迅速さを要求されないときや、手前の Task で必要なリソースの準備を完了させたあとで処理が必要な場合などです。例えば、あるファイルを前提にした処理を実行させるなら、そのファイルを作成する Task の後に実行する必要があります。</p>
<p>その場合のやり方は簡単で、ContinueWith メソッドで実行すべきラムダ式を登録します。すると、その Task が終わった時点で、そのラムダ式を実行してくれます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>012345678101112131415161718done</p>
</div>
<p>この場合のポイントは、ContinueWith メソッドが返してくる値もやはり Task オブジェクトだという点です。ですから、同じように Wait メソッドなどで終了するまで待つことができます。</p>
<p>Task を使った処理では、この他にも、Task どうしをアタッチすることで、Task どうしの親子関係を築く (すべての子の Task が終わるまで、親の Task は終了しない) など、さまざまなパターンで処理を実現できます。これらのメソッドは、並行性 (Concurrency) を実現するための「タスク」の概念を使った各実装パターンを表現できるようになっています。</p>
<p style="margin-top:20px"><a href="#top"><img src="16726-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 実行すべき処理が複数あるとき</h2>
<p>単純に並行に実行すべき処理が複数あるときは、上述のような Task クラスを使った制御ではなく、Parallel.Invoke メソッドを使用した方が便利な場合もあります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>0 10 1 11 2 12 3 13 4 14 5 15 6 16 7 17 8 18 done</p>
</div>
<p>Parallel.Invoke メソッドを使うと、いくつでも処理を並列実行でき、すべてが終わるまで処理を待機します。</p>
<p style="margin-top:20px"><a href="#top"><img src="16727-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 私はだあれ? (データに応じた並列処理)</h2>
<p>こうした複数の処理の同時実行は便利ですが、同様のメソッドを引数 (すなわち、データ) を変えるなどして繰り返し実行するような場合には、冗長になる場合があります。例えば、同じ処理を 3 回実行するために、以下のようなプログラムを書くことは、あまり望ましいことではありません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    static void Main(string[] args)
    {
        Parallel.Invoke(method,method,method);
    }
}</code></pre>
</div>
</div>
...
<p>Invoke メソッドの引数に、同じメソッド名を 3 回も書いている点は冗長です (これが 10 個になった場合は、10 個のメソッドを記述することになります)。また、それぞれのメッセージが、何番目のメソッド呼び出しで実行された結果であるか分かりません。</p>
<p>このような問題を解決には、Parallel.For メソッドを使用できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このように、0、1、2 という連番の番号が生成され、それが method メソッドの引数に渡されているので、容易に「私は 1 番目」のように識別できます。また、method という名前も 1 回だけで済んでいます。</p>
<p>ほとんど同じ処理で、データによって処理をわけるような場合には、同じメソッドを呼んで、条件判断で処理を分けることもできます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16728-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 並列処理できるコレクション</h2>
<p>あるコレクションに対して一律に処理を行いたい場合は、通常 foreach 文などを使用しますが、これも並列処理の対象にできます。対象とするコレクション (データ) の条件は、ただ単に列挙インターフェース (IEnumerable) を持っているというだけです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このような処理は、個々の要素の処理が他の要素に依存しない処理を一気に行う場合に有効です。(複数のプロセッサに分散して処理が行われるからです。) 例えば、配列や行列の計算をおこなうケースで、計算の順番に結果が依存しないとわかっているならば、こうした並列な表現を活用して処理を最適化することができます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16729-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
...
<td valign="top" style="padding:10px">「分かりました。僕の最新の 4 コア CPU のパソコンも、foreach 文より Parallel.ForEach メソッドでパワーを発揮できるわけですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。普通に foreach 文だと、通常は 1 つの処理で実行されるが、Parallel クラスなどを使えば複数コアもパワー全開できるのじゃ。タスクを使ったパターン、データに応じた処理パターンなど、必要に応じてさまざまな並列の表記が可能じゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「はい。今度から、分散できる処理はこうしたクラスを活用してみます!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「プログラムは、よくよく見ると、本来順番に依存しない箇所は多く存在する。C# でも、こうした並列化を意識した非手続き的な表現をうまく組み合わせることが可能じゃ」</td>
</tr>
</tbody>
</table>
