# 連載! とことん C#: 第 12 回 スレッド プールと Web アプリの問題
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 3 月 1 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、いきなりたくさんのリクエストを同時にこなすプログラムを書く羽目になりました。こういう時はスレッドを使うのですね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。しかし、いちいちスレッドを作成しては、すぐシステムが重くなるぞ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ええ!? スレッドってすぐ作成できるんじゃないのですか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「もちろんすぐできるが、作る時間も惜しいこともあるのじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「どうしたらいいんですか!?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「スレッド プールから借りてきて使うのじゃ!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
...
<h2>目次</h2>
<ol>
<li><a href="#01">スレッドとは何だろう?</a> </li><li><a href="#02">フォームとスレッドの同期</a> </li><li><a href="#03">スレッド プールとは何だろう?</a> </li><li><a href="#04">スレッド プールの何が問題なのだろう?</a> </li><li><a href="#05">Web アプリケーションとスレッド ポート枯渇</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. スレッドとは何だろう?</h2>
<p>最近のパソコンのオペレーティング システム (OS) の大多数はマルチタスクです。複数のプログラムが同時に動作します。複数の CPU、コアがある場合は分散し、それでも足りなければ CPU の時間を細切れにして、分け与えます。これにより、速度が低下する場合もありますが、見かけ上、同時に複数のプログラムが&#31292;働します。</p>
<p>さて、このようなマルチタスクですが、1 つのプログラムから別のプログラムに切り替える際の時間は回数が増えるとバカにならない累積時間になります。そこで、すべてのケースでプログラムそのものを毎回切り替える必要はなく、1 つのプログラム内での複数同時実行の機能に縮小されたものがスレッドです。</p>
<p>このスレッドを用いることで、例えば、以下のようなプログラムも記述できます。</p>
<ul>
<li>ReadLine メソッドで 1 行入力を待つ </li><li>常時、現在の秒を表示し続ける </li></ul>
<p>以下が、そのサンプル コードです。</p>
...
<p>実行結果: (開始と終了のタイミングにより数字は異なります)</p>
<p>Type Enter Key to stop!<br>
10<br>
11<br>
12</p>
</div>
<p>ここでは、まず、以下の点を理解しておいてください。</p>
<ul>
<li>スレッドのオブジェクトの作成 (new Thread) と、スレッドの実行 (Start) は違う概念である </li><li>Abort 等で明示的に止めるか、自分自身で終わらないと、スレッドは走り続ける </li><li>自発的に止まるのを待つ場合は Join メソッドでも良い </li><li>Abort メソッドで強制的に止めると、どこで止まるか分からない </li></ul>
<p style="margin-top:20px"><a href="#top"><img src="16579-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. フォームとスレッドの同期</h2>
<p>スレッドを使うようになると、すぐに同期という問題が出てきます。同時にアクセスしてはならないリソースは珍しくないからです。このため、Semaphore クラスや Mutex クラス (いずれも、排他制御で一般的に使用されるクラスです) を使用して複数のスレッド間の調停を行うのが普通ですが、特に注意が必要となるのが、Windows フォーム アプリケーションでしょう。様々なフォームやコントロール (例えば、テキスト ボックスなど) には、「別のスレッド (UI を処理するスレッド以外のスレッド) からの同時アクセスができない」という制約があり、アクセスするには
 Invoke メソッド等を呼び出す必要があるからです。</p>
<p>実際に Invoke メソッドを使った例は以下の通りです。Visual StudioでWindows フォーム アプリケーションのプロジェクトを作成し、フォーム上に Label コントロールを貼り付けた上で以下のコードを入力します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>Action デリゲートは、実行すべき処理を設定するために使用されています。このデリゲートで書かれたコードは、Control クラス (この場合は、Form1) の Invoke メソッドに渡されますが、この Invoke メソッドは、渡された Action デリゲートのコードをフォームのメイン スレッドで実行してくれます。これで、label1 の Text プロパティへの代入は、フォームのメインスレッドと競合することなく成功します。</p>
<p style="margin-top:20px"><a href="#top"><img src="16580-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. スレッド プールとは何だろう?</h2>
<p>スレッドを毎回作成する方法はスレッドが少数であれば上手く行きます。しかし、オブジェクトの生成と解放は数が多くなると負担が重くなります。その場合は、スレッドをいちいち生成するのではなく、プールされたスレッドを借りるという方法を使います。</p>
<p>ThreadPool クラスの静的メンバである QueueUserWorkItem メソッドは、スレッドの待ち行列にデリゲート型を追加します。順番がまわってくれば指定されたデリゲート型の値が実行されます。第 2 引数には、デリゲートの処理に渡す何らかの値 (引数) を追加することができます。以下はそれを使用した例です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    {
        Console.WriteLine(&quot;Thead is running: {0}&quot;, number);
    }
}</code></pre>
</div>
</div>
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果: (出力される値は前後するかもしれません)<br>
Type Enter Key to stop!<br>
Thead in running: 6<br>
Thead in running: 1<br>
Thead in running: 2<br>
Thead in running: 3<br>
Thead in running: 4<br>
Thead in running: 5</p>
</div>
<p>このソース コードでは 1 回もインスタンスを new キーワードで作成していませんが、キューから借り出したスレッドで実行できています。この実行結果は、何回も実行していると順番が変化します。それぞれが異なるスレッドで出力しているためです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16581-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. スレッド プールの何が問題なのだろう?</h2>
<p>このプログラムを少し改変してみましょう。スレッドを一瞬で終わらないように 1000 ミリ秒、つまり 1 秒間待たせてから終了するようにしてみましょう。(太字が、コードを変更した箇所です。)</p>
<div style="margin:20px 0px; padding:10px; background-color:#dedfde">
<p>using System;<br>
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Threading.aspx" target="_blank" title="Auto generated link to System.Threading">System.Threading</a>;</p>
<p>class Program<br>
{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;static void Main(string[] args)<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.QueueUserWorkItem(say, 1);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.QueueUserWorkItem(say, 2);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.QueueUserWorkItem(say, 3);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.WriteLine(&quot;Type Enter Key to stop!&quot;);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.WriteLine(Console.ReadLine());<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;private static void say(Object number)<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.WriteLine(&quot;Thead is running: {0}&quot;, number);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>Thread.Sleep(1000);</strong><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>Console.WriteLine(&quot;Thead is done: {0}&quot;, number);</strong><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
}</p>
</div>
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果: <br>
Type Enter Key to stop!<br>
Thead is running: 1<br>
Thead is running: 2<br>
Thead is done: 1<br>
Thead is running: 3<br>
Thead is done: 2<br>
Thead is done: 3</p>
</div>
<p>ソース コードを見ると 1 番から 3 番までのスレッドをまとめて待ち行列に入れていますが、3 番にリクエストしたスレッドが実行されるのは、1 番の実行終了後 (つまり、1 秒待機した後) まで遅延されています。このように動作する理由は、スレッド プールが持つ最小スレッド数が「2」だからです。スレッドは 2 個しか借りられませんから、3 番目が使うには返却を待つしかありません。つまり、即座に処理させたいためにスレッドを使っているにも関わらず、即座に反応できないケースがあることを示します。</p>
<p>この問題は、直接的には最小スレッド数を 2 から 3 に引き上げれば解決します。</p>
<div style="margin:20px 0px; padding:10px; background-color:#dedfde">
<p>using System;<br>
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Threading.aspx" target="_blank" title="Auto generated link to System.Threading">System.Threading</a>;</p>
<p>class Program<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;static void Main(string[] args)<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>int workerThreads, completionPortThreads;<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.GetMinThreads(out workerThreads,<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;out completionPortThreads );<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.SetMinThreads(3, completionPortThreads);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.QueueUserWorkItem(say, 1);<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.QueueUserWorkItem(say, 2);<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;ThreadPool.QueueUserWorkItem(say, 3);<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>Console.WriteLine(&quot;Type Enter Key to stop!&quot;);<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>Console.WriteLine(Console.ReadLine());<br>
}</p>
<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;private static void say(Object number)<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.WriteLine(&quot;Thead is running: {0}&quot;, number);<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Thread.Sleep(1000);<br>
<strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong>Console.WriteLine(&quot;Thead is done: {0}&quot;, number);<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br>
}</p>
</div>
<p>ここで、SetMinThreads は、第 1 引数、第 2 引数のそれぞれに、ワーカー スレッドと I/O 完了スレッドの最小数を指定します。ここで設定したいのはワーカー スレッドだけなので、第 2 引数の I/O 完了スレッドの最小数は、システムに設定された値を GetMinThreads メソッドで取得して複製しています。</p>
<p>しかし、ここでは以下の問題があることに注意が必要です。</p>
<ul>
<li>スレッドの最小数を増やすと、たとえ使わないとしても貸し出し用のスレッドをあらかじめ確保するので、メモリーなどのリソースの使用量が増える </li></ul>
<p>つまり、無駄に増やすことはシステムの負荷を大きくするので好ましくありません。「いくつなら妥当であるか」はプログラムやシステム次第であり、1 つの正しい値はありません。</p>
<p style="margin-top:20px"><a href="#top"><img src="16582-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. Web アプリケーションとスレッドポート枯渇</h2>
<p>この問題は、実は、割と簡単に遭遇します。例えば、マイクロソフトのサポート情報にある以下の記事はまさにこの問題について述べています。</p>
<p><strong>サポート技術情報 (821268): ASP.NET アプリケーションから Web サービス要求を行うと、競合、パフォーマンスの低下、およびデッドロックが発生する</strong><br>
<a href="http://support.microsoft.com/kb/821268/ja" target="_blank">http://support.microsoft.com/kb/821268/ja</a></p>
<p>原因は以下の通りであると説明されています。</p>
<div style="margin:20px 0px; padding:10px 10px 0px 10px; border:solid 1px">
<p>この問題は、ASP.NET で、呼び出しが要求の実行に使用するワーカー スレッドと完了ポート スレッドの数が制限されているために発生することがあります。</p>
</div>
<p>例えば、2 つのスレッドを借りて動くサービスがあり、このサービスが複数リクエストされたとき、すべてのリクエストが 1 つだけスレッドを占有し、さらにキューのスレッドが枯渇し、もう 1 つを占有しようと待ってしまうと、これらはデッドロックを起こしてタイムアウトするまで待ち続けることになりかねません。</p>
<p>しかし、このような問題に正解はありません。スレッド プールやタイムアウトの値を適切に設定してチューニングするのは、主に開発者や運用者の責務です。スレッドを使用する場合は、こうした利用される状況に応じて適切な設定をしなければならないという点を理解しておいてください。</p>
<p style="margin-top:20px"><a href="#top"><img src="16583-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました。スレッドは、ただ作れば良いというわけではないのですね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。何度も使う場合は、スレッド プールから借りてくるという方法もあって、その方が軽いのじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「ありがとうございます。これで効率の良いプログラムが作成できます」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「しかし、借りたらすぐ返すことを忘れないようにな!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「はい! 枯渇しないように素早く終了させます!」</td>
</tr>
</tbody>
</table>
