# 連載! とことん C#: 第 28 回 動的言語との相互運用 (Python の例)
## Requires
- 
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 6 月 25 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、先輩から、Python で書かれたプログラムをもらいました」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。既存の資産はいろいろなプログラミング言語で書かれている可能性があるからのう。。。」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、僕はせっかく C# を学んできたのに、これ、全部、C# に書き直さないと使えないのでしょうか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「ほう。実は書き直さなくても呼び出す方法があるのじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">C# から Python のコードを呼び出す</a> </li><li><a href="#02">もっと短く!</a> </li><li><a href="#03">オブジェクトの返却</a> </li><li><a href="#04">単に実行させるだけなら</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. C# から Python のコードを呼び出す</h2>
<p>.NET Framework はプログラミング言語に対して中立です。そのため、Visual Studio を見ても、Visual Basic、C&#43;&#43;/CLI、F# など C# 以外の言語を使用できます。これらはプログラミング言語としてはすべて別ですが、CTS (Common Type System) に即してプログラム コードを書く限り、容易に相互運用することができます。</p>
<p>ところが、世の中には「動的」なプログラミング言語が存在し、一般的にそのような言語は「型」の使用方法が大きく異なります。「型」は存在していても、それを明示しないでプログラムを作成できてしまうのです。</p>
<p>例えば、Python という言語には、.NET Framework 上で動作する IronPython というプログラミング言語がありますが (Ruby についても、同様に、IronRuby が存在します)、C# のプログラムから呼び出すことは一筋縄では行きません。以下、実際に Python で記述されたメソッドを呼び出したサンプルを見てみましょう。(ここでは、Visual Studio 2010 と Pythod 2.6.1 を使用しています。)</p>
<p>まず、前準備として、以下を必要とします。</p>
<ol>
<li>IronPython をインストールしておく </li><li>Visual Studio を開き、C# のコンソール アプリケーションを作成する </li><li>上記 2 のアプリケーションで、IronPython に含まれている IronPython.dll、Microsoft.Dynamic.dll、Microsoft.Scripting.dll の参照を追加しておく
</li><li>テキスト ファイルを 2 のプロジェクトに追加して、sample.py というファイル名を付けておく </li></ol>
<p>これで、以下の 2 つのプログラムを用意します。</p>
<p>C# 側 (Program.cs):</p>
...
class Program
{
    static void Main(string[] args)
    {
        ...
        ScriptScope pythonScope = py.UseFile(@&quot;..\..\sample.py&quot;);
        var pythonFunction = (PythonFunction)pythonScope.GetVariable&lt;object&gt;(&quot;addPython&quot;);
        var targetMthod = (Func&lt;PythonFunction, object, object&gt;)pythonFunction.func_code.Target;
        var result = targetMthod(pythonFunction, &quot;Iron&quot;);
...
<pre class="hidden">&lt;code class=&quot;csharp&quot;&gt;def addPython(name):
       return name &#43; &quot;Python&quot;&lt;/code&gt;</pre>
<pre class="csharp">&lt;code&nbsp;<span class="cs__keyword">class</span>=<span class="cs__string">&quot;csharp&quot;</span>&gt;def&nbsp;addPython(name):&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">return</span>&nbsp;name&nbsp;&#43;&nbsp;<span class="cs__string">&quot;Python&quot;</span>&lt;/code&gt;&nbsp;
&nbsp;
</pre>
</div>
</div>
...
<p>F5 キーを押すと実行 (デバッグ実行) され、C# のプログラム側に含まれる &quot;Iron&quot; と、Python 側に含まれる &quot;Python&quot; という文字列が合体し、&quot;IronPython&quot; という文字列が出力されます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16689-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. もっと短く!</h2>
<p>上記の例は、見て頂いてわかるように、C# 側が極端に長すぎます。そこで、C# 4.0 以降で導入された dynamic 型を用いて、さらに短くしてみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    static void Main(string[] args)
    {
        ...
        var result = sample.addPython(&quot;Iron&quot;);
        Console.WriteLine(result);
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>ここで最も重要なポイントは、最初の例では文字列として記述されていたメソッド名 (addPython) が、&quot; (クォーテーション マーク) 抜きで通常のメソッド名として記述されていることです。このように記述されてはいますが、決して C# コンパイラが addPython というメソッドの存在を察知している訳ではありません。実際には、メソッドの検出は実行時まで遅延されていて、存在しなければ実行時に例外が発生します。</p>
<p>そして、こうした機能性を実現してくれるのが、上記で登場した dynamic という型です。dynamic 型は、単に名前の無い型を扱いやすくするだけでなく、動的な言語との相互運用における面倒な手順を大幅に簡素化してくれるのです。</p>
<div style="margin:10px 0; padding:10px 10px 0 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>Note:</strong><a href="http://msdn.microsoft.com/ja-jp/ff404128.aspx">「第 10 回 型を書かなくても強い型付け! (var とジェネリック)」</a>で説明した var が、&quot;コンパイル時&quot; に型を推論するのに対して、dynamic は、&quot;実行時&quot; に解釈されます。</p>
</div>
<p style="margin-top:20px"><a href="#top"><img src="16690-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. オブジェクトの返却</h2>
<p>上記の例は文字列を受け渡すだけなので、それほど深刻な問題は発生しません。戻ってくるオブジェクトは object 型の変数 result で受けていますが、もちろん文字列なので string にキャストすれば文字列としていくらでも活用できます。</p>
<p>しかし、もしも、クラスのような複雑な型だったらどうでしょうか?</p>
<p>Python 側で以下のような定義があったとしましょう。</p>
<p>sample.py:</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">python</span>
<pre class="hidden">&lt;code class=&quot;csharp&quot;&gt;class InPythonClass:
	x=123
	y=456

def getTheObject():
       return InPythonClass()&lt;/code&gt;</pre>
<pre class="csharp">&lt;code&nbsp;<span class="cs__keyword">class</span>=<span class="cs__string">&quot;csharp&quot;</span>&gt;<span class="cs__keyword">class</span>&nbsp;InPythonClass:&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;x=<span class="cs__number">123</span>&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;y=<span class="cs__number">456</span>&nbsp;
&nbsp;
def&nbsp;getTheObject():&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">return</span>&nbsp;InPythonClass()&lt;/code&gt;&nbsp;
&nbsp;
</pre>
</div>
</div>
...
<p>この場合、C# 側で、いったいどういう型を指定すれば x や y の値を取得できるのでしょうか?</p>
<p>実はこの問題は非常にややこしい状況になります。というのは、上記の InPythonClass というクラス定義は、あくまで Python のソース コード上に存在するものであり、C# の定義には存在していません。そのため、C# のコンパイラはこのような型の存在を認識することができず、InPythonClass という型を経由して x や y の値を読み書きすることができません。普通の方法では、そもそもコードを書けないのです。</p>
<p>ところが、ここでも dynamic 型を介在させると、「定義不要」という dynamic 型のメリットが活きてきます。dynamic 型を経由すれば、コンパイル時に C# 側に定義が存在しなくても、何の問題も無くなってしまいます。ですから、以下のようなコードを容易に記述できるのです。</p>
<p>Program.cs:</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
...
<p>この場合、getTheObject というメソッドの存在だけでなく、x や y の存在も、コンパイル時にはチェックされず、実行時にまで遅延されます。(このため、もしも記述した名前のメンバーが存在しなければ、実行時の例外となります。)</p>
<p style="margin-top:20px"><a href="#top"><img src="16691-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 単に実行させるだけなら</h2>
<p>これまでの例は、Python のプログラムをライブラリとして呼び出していましたが、ただ単に実行させるだけならもっと容易です。</p>
<p>Program.cs</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
...
<div class="pluginEditHolderLink">スクリプトの編集</div>
<span class="hidden">python</span>
<pre class="hidden">&lt;code class=&quot;csharp&quot;&gt;print &quot;Hello!&quot;&lt;/code&gt;</pre>
<pre class="csharp">&lt;code&nbsp;<span class="cs__keyword">class</span>=<span class="cs__string">&quot;csharp&quot;</span>&gt;print&nbsp;<span class="cs__string">&quot;Hello!&quot;</span>&lt;/code&gt;&nbsp;
&nbsp;
</pre>
</div>
</div>
...
<p>なんと、Python 側のソースコードはたった 1 行になってしまいました。単に実行するだけなら、もはやメソッドの名前すら必要ありません。こうした記述でも実行できてしまいます。書かねばならないコードの量が極端に少ないのが動的なプログラミング言語の特徴であり、愛好者たちに好まれる理由の 1 つです。</p>
<p>なお、上記で、UseFile メソッドではなく、ExecuteFile メソッドを使用しても結果は同じですが、この両者は、常に同じとは限りません。例えば、UseFile メソッドは Python 側で指定された検索パス上のファイルも探してくれますが、ExecuteFile メソッドはそのような探索の機能を持っていないので注意してください。</p>
<p style="margin-top:20px"><a href="#top"><img src="16692-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
...
<td valign="top" style="padding:10px">「分かりました! つまり、ポイントは dynamic 型の活用ですね!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。C# で定義されていないデータを扱った外部のプログラミング言語との相互運用も、これがあればバッチリじゃな」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「これで Python の資産を活かしつつ、得意な C# を活用できます。ありがとうございます!」</td>
</tr>
</tbody>
</table>
