...
<hr>
<p>.NET Framework では <a href="http://msdn.microsoft.com/ja-jp/library/system.io.file.aspx">
System.IO.File</a> クラスに準備されているメソッドを使用することでファイルのコピーや移動を実現することができます。例えば &quot;C:\text&quot; フォルダーに &quot;test.txt&quot; というファイルが存在しており、そのファイルを &quot;C:\text2&quot; に同名のファイル名でコピーする場合はこのようになります。</p>
...
<pre id="codePreview" class="vb"><code class="csharp">Imports <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.IO.aspx" target="_blank" title="Auto generated link to System.IO">System.IO</a>
Module Module1
    Sub Main()
        Dim soucePath As String = &quot;C:\text\test.txt&quot;
        Dim targetPath As String = &quot;C:\text2\test.txt&quot;
        File.Copy(soucePath, targetPath, True)
    End Sub
End Module</code></pre>
</div>
</div>
...
<p>Copy メソッドの 3 番目の引数は同名のファイルが存在する場合に、上書きを行うかどうかを指定します。省略した場合はファイルの上書きが行われず、例外が発生します。指定のファイルが存在しているかどうかは次のように判定することができます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="vb"><code class="csharp">If My.Computer.FileSystem.FileExists(soucePath) Then
End If</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
...
<p>VB では .NetFramework 2.0 以降のバージョンにおいて <a href="http://msdn.microsoft.com/ja-jp/library/0b485hf7.aspx">
My.Computer.FileSystem</a> の CopyFile メソッドを使用しても同様の結果を得る事ができます。また、この場合は、エンド ユーザーのファイル操作のように上書き確認などのダイアログを表示できます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="vb"><code class="csharp">My.Computer.FileSystem.CopyFile(soucePath, targetPath, _
                                            FileIO.UIOption.AllDialogs, FileIO.UICancelOption.DoNothing)</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>ファイルを移動させる場合は File.Move メソッドを使用しますが、同名のファイルが移動先に存在する場合は例外が発生するので注意が必要です。VB では My.Computer.FileSystem.MoveFile メソッドを使うことで同名のファイルが存在する場合にダイアログを表示させることができます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="vb"><code class="csharp">My.Computer.FileSystem.MoveFile(soucePath, targetPath, _
                                         FileIO.UIOption.AllDialogs, FileIO.UICancelOption.DoNothing)</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
...
<p>これらは全て同期処理となるため、大きいサイズのファイルをコピーした場合はその間処理が止まってしまいます。非同期に処理を行う方法の 1 つとして FileStream クラスの BeginRead, BeginWrite メソッドを利用した<a href="http://msdn.microsoft.com/ja-jp/library/kztecsys.aspx">非同期 I/O 処理</a>を実装する方法があります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
Module Module1
    Sub Main()
        Dim soucePath As String = &quot;C:\text\test.txt&quot;
        Dim targetPath As String = &quot;C:\text2\test.txt&quot;
        Dim st As New Status(soucePath, targetPath)
        If st.sourceStream.CanRead Then
            st.sourceStream.BeginRead(st.buffer, _
                                      0, _
                                      st.buffer.Length, _
                                      AddressOf userReadCallBack, _
                                      st)
        End If
        Console.ReadKey()
    End Sub

    Private Sub userReadCallBack(ByVal _result As IAsyncResult)
        If _result.IsCompleted Then
            Dim st As Status = TryCast(_result.AsyncState, Status)
            If st IsNot Nothing Then
                Try
                    Dim percent As Double = (CDbl(st.targetStream.Position) / st.sourceStream.Length)
                    If st.sourceStream.Length = 0 Then
                        percent = 1.0
                    End If
                    Console.WriteLine(&quot;コピーファイルの書き込み ...&quot; &amp; percent.ToString(&quot;P&quot;))
                    Dim length As Long = st.buffer.Length
                    If st.sourceStream.Length - st.buffer.Length &lt; st.targetStream.Length Then
                        length = st.sourceStream.Length - st.targetStream.Length
                    End If
                    st.targetStream.BeginWrite(st.buffer, _
                                               0, _
                                               CInt(length), _
                                               AddressOf userWriteCallBack, _
                                               st)
                Catch ex As Exception
                    st.sourceStream.Close()
                    st.targetStream.Close()
                    Console.WriteLine(&quot;書き込み中にエラーが発生しました...何かキーを押してください&quot;)
                End Try
            End If
        End If
    End Sub

    Private Sub userWriteCallBack(ByVal _result As IAsyncResult)
        If _result.IsCompleted Then
            Dim st As Status = TryCast(_result.AsyncState, Status)
            If st IsNot Nothing Then
                Try
                    st.targetStream.Flush()
                    If st.sourceStream.Length &gt; st.sourceStream.Position Then
                        st.sourceStream.BeginRead(st.buffer, _
                                                  0, _
                                                  st.buffer.Length, _
                                                  AddressOf userReadCallBack, _
                    ...
                    Else
                        st.sourceStream.Close()
                        st.targetStream.Close()
                        Console.WriteLine(&quot;...完了, 何かキーを押してください。&quot;)
                    End If
                Catch ex As Exception
                        st.sourceStream.Close()
                        st.targetStream.Close()
                        Console.WriteLine(&quot;読み込み中にエラーが発生しました...何かキーを押してください&quot;)
                End Try
            End If
        End If
    End Sub
End Module
Class Status
    Public sourceStream As FileStream
    Public targetStream As FileStream
    Public buffer As Byte() = New Byte(4095) {}
    Public Sub New(ByVal _soucePath As String, ByVal _targetPath As String)
        sourceStream = New FileStream(_soucePath, _
                                      FileMode.Open, _
                                      FileAccess.Read, _
                                      FileShare.Read, _
                                      &amp;H1000, _
        ...
        targetStream = New FileStream(_targetPath, _
                                      FileMode.Create, _
...
<p>上記のサンプル コードでは BeginRead を使用し、非同期でのデータ読み込みをおこないます。次に非同期読み込みが終了した際に呼び出されるコールバック メソッドにて、コピー先のストリームにバッファ分の情報を BeginWrite を用いて非同期に書き込みます。</p>
<p>更に、非同期書き込みが終了した際に呼び出されるコールバック メソッドにて再度 BeginRead を呼び出す処理を行っており、非同期での読み込み、書き込みを交互に行っています。</p>
