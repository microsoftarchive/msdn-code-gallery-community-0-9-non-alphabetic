...
<hr>
<p>.NET においては文字列は UTF-16 で保持され、既定のファイル保存形式は UTF-8 となりますが、日本語でよく用いられるテキストの形式としてシフト JIS (Shift-JIS) が挙げられます。ここでは文字コードを指定して保存する方法を解説します。</p>
...
class Program
{
    ...
    {
        using (var sw = new StreamWriter(&quot;out.txt&quot;, false, Encoding.GetEncoding(&quot;shift_jis&quot;)))
            sw.WriteLine(&quot;こんにちは！&quot;);
    }
}</code></pre>
</div>
...
<p>このように、<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.IO.StreamWriter.aspx" target="_blank" title="Auto generated link to System.IO.StreamWriter">System.IO.StreamWriter</a> のコンストラクターに Encoding.GetEncoding (&quot;shift_jis&quot;) と指定することで既定のエンコーディングである UTF-8 ではなく、Shift-JIS 形式で保存することが可能となります。</p>
<p>エンコーディングを指定する方法には &quot;shift_jis&quot; のように文字列として渡す方法と、932 のようにコード ページを数値として渡す方法の 2 種類があります。サポートされているエンコーディングは以下のコードを実行することで列挙することができます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    static void Main(string[] args)
    {
        foreach (var e in Encoding.GetEncodings())
            Console.WriteLine(&quot;{0}\t: {1}&quot;, e.CodePage, e.DisplayName);
    }
}</code></pre>
</div>
</div>
...
<p>エンコーディング名に対応するコード ページ番号が分かっている場合は、そちらを利用することをおすすめします。</p>
<p>エンコーディングについてさらに詳しく知るには MSDN ライブラリの</p>
<ul>
<li><strong>エンコーディングの理解</strong><br>
<a href="http://msdn.microsoft.com/ja-jp/library/ms404377.aspx">http://msdn.microsoft.com/ja-jp/library/ms404377.aspx</a>
</li><li><strong>Encoding クラス (<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Text.aspx" target="_blank" title="Auto generated link to System.Text">System.Text</a>)</strong><br>
<a href="http://msdn.microsoft.com/ja-jp/library/system.text.encoding.aspx">http://msdn.microsoft.com/ja-jp/library/system.text.encoding.aspx</a>
</li></ul>
<p>をご参照ください。</p>
