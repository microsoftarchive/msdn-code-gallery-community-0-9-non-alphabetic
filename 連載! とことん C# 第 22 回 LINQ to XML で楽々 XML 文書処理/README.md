# 連載! とことん C#: 第 22 回 LINQ to XML で楽々 XML 文書処理
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 5 月 14 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで XML の処理って長々と書く必要があって、面倒です。何か良い方法はないですか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「もしかして、DOM を使っていないかね ?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「DOM って、XmlDocument クラスとかを使ったアレですよね。それを使ってますけど . . .」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「ならば、LINQ to XML に乗り換えてみるのじゃ!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、クエリだけ楽になってもあまり意味がありませんが . . .」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「クエリだけではないのじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">DOM の例</a> </li><li><a href="#02">LINQ to XML の例</a> </li><li><a href="#03">XML 文書を作成する例</a> </li><li><a href="#04">結末</a> </li></ol>
<hr>
<h2 id="01">1. DOM の例</h2>
<p>まず、旧世代の技術である DOM から始めましょう。DOM は XML を操作する基本技術として、幅広く多くのプログラミング言語や開発環境でサポートされていますが、記述すべき手間が多いことでも有名です。</p>
<p>以下は、とある XML 名前空間 URI に属する name という要素 (&lt;name /&gt;) を列挙し、id 属性の文字列が M で始まる内容だけを出力する例です。ここでは、このソースコードを理解する必要はありませんが、どのような概念で実装しているか追ってみてください。</p>
...
<p>要求された機能と比べて、このソースコードは長すぎると思いませんか?</p>
<ul>
<li>GetElementsByTagName メソッドは、ごく基本的でよく使われるにもかかわらず、あまりに名前が長すぎます (20 文字) </li><li>名前が長い割に、指定した名前と一致した要素を列挙する機能しか持ちません </li><li>今回は、要素の名前である &quot;name&quot; で絞り込んでいますが、要素の名前以外で絞り込むには、さらに別途 if 文が必要となります </li></ul>
<p>通常、このような過剰な長さを回避するために、XPath という技術を用います。以下はそれを使用した例です。(前後は同じなので省略しています。同様に、ここでは、このソースコードの詳細を理解する必要はありません。)</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">static void Main(string[] args)
    {
        ...
        nsmgr.AddNamespace(
            &quot;ex&quot;, &quot;http://www.piedey.co.jp/example/linqtoxml201003&quot;);
...
<p>ここでは、SelectNodes メソッドで引数に「XPath 式」を指定することで、属性の判定も 1 つのメソッドで処理することができています。</p>
<p>しかし、名前空間を指定するために、XmlNamespaceManager クラスのオブジェクトを作成する必要が生じています。その分の行が増えている他、以下のような問題も生じています。</p>
<ul>
<li>XmlNamespaceManager も名前が長すぎます (GetElementsByTagName メソッドの 20 文字に匹敵する 19 文字) </li><li>XmlNamespaceManager オブジェクトのコンストラクタに指定した DOM ドキュメントの NameTable と、実際に使用する DOM ドキュメントの対応関係はプログラマーが注意して維持する必要があります (食い違ってもコンパイラは教えてくれません)
</li><li>AddNamespace メソッドで追加した名前空間接頭辞（ここでは &quot;ex&quot;）と、XPath 式の中に書き込んだ名前空間接頭辞が同じであるよう、プログラマー自身が注意深く維持する必要があります (食い違ってもコンパイラは教えてくれません)
</li></ul>
<p>つまり、XPath の導入でプログラムは簡素化されたものの、別の問題も一緒に連れてきたわけです。これは、前回説明した SQL で例えるなら、プログラミング言語とは別の SQL 文によって生じる課題とも&#20284;ています。SQL 文というプログラミング言語とは別の新たな構文を理解する必要が生じ、かつ、SQL 文のスペルミスをコンパイラはチェックしてくれません。</p>
<p>これが、LINQ to XML が登場する前の状況です。</p>
<p style="margin-top:20px"><a href="#top"><img src="16651-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. LINQ to XML の例</h2>
<p>では、LINQ to XML を使って同じ機能を書き下ろしてみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
&quot;;
    ...
    static void Main(string[] args)
    {
        ...
        var query = from n in doc.Descendants(ex &#43; &quot;name&quot;)
                    where n.Attribute(&quot;id&quot;).Value.StartsWith(&quot;M&quot;)
                    select n;
...
<p>これを見ると、あらゆる問題がすべて解消されていることが分かります。</p>
<p>まず、長すぎる名前のメソッドは 1 つもありません。そして、クエリ式の where 節で条件も判定できるので、別途 if 文が必要ということもありません。XML 名前空間を使用するために作成するオブジェクト (上記のコードの ex) が必要とされますが、単に文字列を代入するだけで必要な名前空間を示すオブジェクトが生成されています。(これは演算子のオーバーロードが存在するためです。) また、ex は文字列としての識別符号ではなく、C# の変数なので、名前の不一致があればコンパイラが検出して教えてくれます。つまり、実行時ではなくコンパイル時に既に分かってしまいます。また、ex
 &#43; &quot;name&quot;という式は、これも &#43; 演算子がオーバーロードされており、名前空間付きの XML 名を内部で生成します。単なる文字列の合体ではありません。</p>
<p>このような、LINQ to XML の誕生を経て、ようやく XML は「息をするように自然に活用するデータ」の記述フォーマットになったと言えるでしょう。もはや、XML は数値や文字列のように気軽に扱えるようになったのです。</p>
<p>ちなみに、XElement クラスは XML の要素を示すクラスであり、LINQ to XML の登場によって新たに追加されたクラスです。Parse メソッドは XML 文書を解析し、Descendants メソッドは指定された名前を持つ子孫要素を列挙してくれます。(ツリー構造の XML データを活用する場合には必要となるメソッドです。) そして、XNamespace クラスは名前空間を保持するクラスです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16652-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. XML 文書を作成する例</h2>
<p>しかし、単にクエリだけが楽になれば良いという話ではありません。XML データの処理はクエリだけに限らないからです。LINQ to XML を使うと、XML データを生成するコードも大幅に簡素化されるのです。例えば、以下のような XML データを生成したいとしましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">&lt;names xmlns='http://www.piedey.co.jp/example/linqtoxml201003'&gt;
  &lt;name id='M0001'&gt;一郎&lt;/name&gt;
&lt;/names&gt;</code></pre>
</div>
</div>
...
<p>DOM を使用して正攻法で記述すると以下のようにかなり回りくどくなります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:<br>
&lt;names id=&quot;M0001&quot; xmlns=&quot;http://www.piedey.co.jp/example/linqtoxml201003&quot;&gt;&lt;name&gt;一郎&lt;/name&gt;&lt;/names&gt;</p>
</div>
<p>しかし、LINQ to XML を使用すると XElement クラスのコンストラクタに任意個数の XElement オブジェクト (XML の要素) や XAttibute オブジェクト (要素内の属性) や文字列を記述することができ、それらが各オブジェクトの子として扱えます。もちろん、それらのオブジェクトのコンストラクタに、更に別のオブジェクトを渡すこともできます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:<br>
&lt;names id=&quot;M0001&quot; xmlns=&quot;http://www.piedey.co.jp/example/linqtoxml201003&quot;&gt;<br>
&lt;name&gt;一郎&lt;/name&gt;<br>
&lt;/names&gt;</p>
</div>
<p>この通り、見やすくするために挿入された改行や空白を除けば最初の要求と同じ XML データが、たった 1 回のオブジェクト作成文で生成されているのがわかります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16653-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 結末</h2>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました。LINQ to XML という技術は、単にクエリ式で XML を使えるというだけではなく全体として XML を使いやすくしてくれるクラスライブラリなのですね」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。コーディング量も大幅に減り、コードの理解性も非常に高くなるのじゃ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「特に名前空間の扱いがとても楽になるのが嬉しいですね」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「しかし、演算子のオーバーロードは型を間違えると機能せんから、そこは注意が必要じゃぞ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「はい !」</td>
</tr>
</tbody>
</table>
