...
<pre id="codePreview" class="csharp"><code class="csharp">using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.IO.aspx" target="_blank" title="Auto generated link to System.IO">System.IO</a>;
namespace CodeRecipe_CopyMoveFiles_CS
{
    class Program
    {
        static void Main(string[] args)
        {
            string soucePath = @&quot;C:\text\test.txt&quot;;
            string targetPath = @&quot;C:\text2\test.txt&quot;;
...
<pre id="codePreview" class="csharp"><code class="csharp">if (File.Exists(soucePath))
{
}</code></pre>
</div>
</div>
...
<p>ファイルを移動させる場合は File.Move メソッドを使用しますが、同名のファイルが移動先に存在する場合は例外が発生するので注意が必要です。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">File.Move(soucePath, targetPath);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>また、 <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/Microsoft.VisualBasic.aspx" target="_blank" title="Auto generated link to Microsoft.VisualBasic">Microsoft.VisualBasic</a> アセンブリを参照に追加することで上書き確認ダイアログ表示の機能を使うことができます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp"><a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/Microsoft.VisualBasic.Devices.Computer.aspx" target="_blank" title="Auto generated link to Microsoft.VisualBasic.Devices.Computer">Microsoft.VisualBasic.Devices.Computer</a> computer = new Computer();
computer.FileSystem.MoveFile(soucePath, targetPath, 
    Microsoft.VisualBasic.FileIO.UIOption.AllDialogs, 
    Microsoft.VisualBasic.FileIO.UICancelOption.DoNothing);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
...
namespace CodeRecipe_Copy_MoveFiles_Async_CS
{
    class Program
    {
        static void Main(string[] args)
        {
            string soucePath = @&quot;C:\text\test.txt&quot;;
            string targetPath = @&quot;C:\text2\test.txt&quot;;
            Status st = new Status(soucePath, targetPath);
            if (st.sourceStream.CanRead)
            {
                st.sourceStream.BeginRead(
                    st.buffer,
                    0,
                    st.buffer.Length,
                    userReadCallBack, st);
        ...
        private static void userReadCallBack(IAsyncResult _result)
        {
            if (_result.IsCompleted)
            {
                Status st = _result.AsyncState as Status;
                if (st != null)
                {
                    try
                    {
                        double percent = ((double)st.targetStream.Position / st.sourceStream.Length);
                        Console.WriteLine(&quot;コピーファイルの書き込み ...&quot; &#43; percent.ToString(&quot;P&quot;));
                        long length = st.buffer.Length;
                        if (st.sourceStream.Length - st.buffer.Length &lt; st.targetStream.Length)
                        {
                            length = st.sourceStream.Length - st.targetStream.Length;
                        }
                        st.targetStream.BeginWrite(
                            st.buffer,
                            0,
                            (int)length,
                            userWriteCallBack,
        ...
        private static void userWriteCallBack(IAsyncResult _result)
        {
            if (_result.IsCompleted)
            {
                Status st = _result.AsyncState as Status;
                if (st != null)
                {
                    try
                    {
                        ...
                        if (st.sourceStream.Length &gt; st.sourceStream.Position)
                        {
                            st.sourceStream.BeginRead(
                                st.buffer,
                                0,
    ...
    class Status
    {
        ...
        public Status(string _soucePath, string _targetPath)
        {
            sourceStream =
                new FileStream(_soucePath,
                            FileMode.Open,
                            FileAccess.Read,
                            FileShare.Read,
                            0x1000,
