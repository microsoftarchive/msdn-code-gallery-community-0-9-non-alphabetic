		...
		Private Shared Function Description(ByVal Language As LanguageManager.Language, ByVal ShortDescription As Boolean) As String
			...
			Else
				If Language = LanguageManager.Language.Italian Then
					Return "Plugin che aggiunge un robot autorisponditore il quale simula un operatore e assiste i visitatori nel sito web. Occorre preventivamente programmare il robot tramite un apposito linguaggio: http://www.cmsaspnet.com/?p=82&a=26&t=instructionschatlanguage"
				Else
					Return "Plugin to the addition of a robot autoresponder that simulates an operator to assist the visitors in the website. It should advance to program the robot using a special language: http://www.cmsaspnet.com/?p=82&a=26&t=instructionschatlanguage"
				End If
			End If
		End Function
    ...
    End Function
		...
		Private Shared Sub Plugin_MasterPagePreRender(ByVal MasterPage As Components.MasterPage) Handles Plugin.MasterPagePreRender
			...
			If QuestionsByLanguage.ContainsKey(MasterPage.Setting.Language) Then
				...
				If Session.SessionID <> Request.QueryString("r") Then
            ...
            End If
					...
					Else
						...
						Dim ChatRoom As String = Request.QueryString("r")
						...
						If ChatRoom = "_" & Session.SessionID Then
							...
							If Session("UserChatInfo") Is Nothing Then
								...
								'UserChatInfo.Entity.Name = FirstUpper(Replace(Request("r"), "_", ""))
								UserChatInfo.Entity.Name = FirstUpper(Request.QueryString("u"))
								UserChatInfo.Language = MasterPage.Setting.Language
								UserChatInfo.Entity.PrivateChatRoom = Request.QueryString("r")
