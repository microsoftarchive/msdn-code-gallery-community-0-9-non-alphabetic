      ...
      If Plugin Is Nothing Then Plugin = New PluginManager.Plugin(AddressOf Description, , , , PluginManager.Plugin.Characteristics.StandardPlugin, GetType(SkinCustomizationConfiguration))
		...
		Private Shared Function Description(ByVal Language As LanguageManager.Language, ByVal ShortDescription As Boolean) As String
			If Language = LanguageManager.Language.Italian Then
				...
				Else
					Return "Plugin per il ritocco della skin in uso. Resettare la cache del browser ad ogni cambiamento! Per una maggiore personalizzazione si veda qui: http://cmsaspnet.com/?p=32&a=26&t=realizzazionetemplate"
				End If
			Else
				If ShortDescription Then
					Return "Skin customization"
				Else
					Return "Plugin for the retouching of skin in use. Reset the browser cache after every change! For greater customization, see here: http://cmsaspnet.com/?p=32&a=26&t=realizzazionetemplate"
				End If
			End If
		End Function
