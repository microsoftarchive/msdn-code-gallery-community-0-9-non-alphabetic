	...
	Public Module Sitemap
      ...
      Dim Pages As StringCollection = Nothing
          ...
          Pages = LoadObject(GetType(StringCollection), "googleidx_" & Domain)
			...
			Sitemap.GenerateSiteMap(Response.OutputStream, Domain, Pages, Request.Url.AbsoluteUri.StartsWith("http://www."), Type)
			Response.End()
    End Sub
			...
			For Each Setting As SubSite In DomainConfiguration.SubSites
				...
				Select Case Type
					Case TypeOfSitemap.Generic
						'Add home page
						AddPage(www, SiteMap, Domain, Href(DomainConfiguration, Setting.Name, False, "default.aspx"), Exclude, 0.80000000000000004, Now.ToUniversalTime, ChangeFrequency.Daily)
						...
						If Setting.EnableRelatedBlogAggregator AndAlso Setting.Blog IsNot Nothing AndAlso Setting.Blog.Count > 0 Then
							For Each Blog As Notice In Setting.Blog
								If Not UrlPointToThisNetwork(Blog.Link) Then
									AddPage(www, SiteMap, Domain, Common.Href(CurrentSetting.Name, False, "default.aspx", "s", DefaultPageShowType.ExternalPage, "u", Blog.Link.Substring(Blog.Link.IndexOf("/"c) + 2)), Exclude, 0.80000000000000004, Blog.pubDate, ChangeFrequency.Never)
								End If
							Next
						End If
				End Select
					...
					For Each Notice As Notice In Notices
						If Type = TypeOfSitemap.Generic OrElse (Type = TypeOfSitemap.Images AndAlso Notice.Image <> "") Then
							...
							If Type = TypeOfSitemap.Images Then
								Image = Notice.Image
							End If
							AddPage(www, SiteMap, Domain, Href(Setting.Name, False, "default.aspx", "c", CryptUrl(Notice.Link.Substring(Notice.Link.IndexOf("/"c) + 2))), Exclude, 0.80000000000000004, Notice.pubDate, ChangeFrequency.Never, Image)
						End If
					Next
				End If
