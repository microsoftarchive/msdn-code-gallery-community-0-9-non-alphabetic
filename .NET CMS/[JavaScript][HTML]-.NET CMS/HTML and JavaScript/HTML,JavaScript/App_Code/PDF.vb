...
Namespace WebApplication
    ...
    Function HrefPdf(ByVal Source As SourceType, ByVal DomainConfiguration As DomainConfiguration, ByVal SubSite As String, ByVal Base As Object, ByVal Id As Integer, ByVal FileName As String) As String
      Dim NameFilePdf As String = HttpUtility.UrlEncode(Source) & "_" & HttpUtility.UrlEncode(Base) & "_" & HttpUtility.UrlEncode(Id) & "_" & HttpUtility.UrlEncode(Alphanumeric(FileName)) & ".pdf"
      Return Href(DomainConfiguration, SubSite, False, NameFilePdf)
    End Function
      ...
      Dim TagName As String = Nothing
		...
		Public Function PdfDocument(Url As String) As Boolean
			...
			If Url.EndsWith(".pdf") Then
				...
				Dim Params() As String = FileName.Split("_")
    ...
    End Function
		...
		Public Sub PdfDocument(ByVal Source As SourceType, ByVal Base As Object, ByVal Id As Integer)
			...
			Select Case Source
				Case SourceType.Forum
					If Setting.EnablePdfVersion.TopicsOfForum = EnabledStatus.No OrElse (Setting.EnablePdfVersion.TopicsOfForum = EnabledStatus.Default AndAlso Setup.RenderingEngine.EnablePdfVersion.TopicsOfForum = False) Then
						'Redirect 
						HttpContext.Current.Response.Redirect(Href(Setting.Name, False, "forum.aspx", "f", Base, "t", Id, "p", "1"), True)
					End If
				Case SourceType.Photo
					If Setting.EnablePdfVersion.Photos = EnabledStatus.No OrElse (Setting.EnablePdfVersion.Photos = EnabledStatus.Default AndAlso Setup.RenderingEngine.EnablePdfVersion.Photos = False) Then
						'Redirect
						HttpContext.Current.Response.Redirect(Href(Setting.Name, False, "showphoto.aspx", "album", Base, "n", Id), True)
					End If
				Case SourceType.Archive
					If Setting.EnablePdfVersion.Pages = EnabledStatus.No OrElse (Setting.EnablePdfVersion.Pages = EnabledStatus.Default AndAlso Setup.RenderingEngine.EnablePdfVersion.Pages = False) Then
						'Redirect
						Dim Menu As Menu = Menu.Load(Int(Base), Setting.Language)
						HttpContext.Current.Response.Redirect(Menu.ItemsMenu.Page(Id).Href(CurrentDomainConfiguration, Setting), True)
					End If
			End Select

			Dim clPDF As New clsPDFCreator(HttpContext.Current.Request.Url.Host, HttpContext.Current.Request.Url.Host, HttpContext.Current.Request.Url.Host)
			...
			With clPDF
				...
				Select Case Source
					Case SourceType.Forum
						Dim Topic As Topic = Topic.Load(Base, Id)
              ...
              Ed2kNameFile = Topic.Title & " " & ReplaceBin(Topic.Keywords, ",", " ")
							...
							AddFooterToPdfDoc(clPDF, Host & ForumManager.Link(ActionType.Show, Setting.Name, , , Base, Id, , 1))
							Dim Replies As Reply() = Topic.AllReplies(Base, Id, New UserPreferences(CurrentUser).ShowCensored, EnableShowHidden)
                  ...
                  End If
							...
							For Each Reply As Reply In Replies
								...
								If Reply.QuoteID <> 0 AndAlso Reply.TextQuote <> "" Then
									Dim ReplyQuote As Reply = Reply.Load(Base, Id, Reply.QuoteID)
                  ...
                  End If
						...
						End If
					Case SourceType.Photo
						Dim Photo As Photo = PhotoManager.Photo.Load(Id, Base)
						If Photo Is Nothing Then
							HttpContext.Current.Response.RedirectPermanent(Href(Setting.Name, False, "default.aspx"), True)
						Else
							Dim AlbumLower As String = Base.ToLower
							Dim ObjAlbum As PhotoAlbum = PhotoManager.PhotoAlbum.Load(Base)
              ...
              clPDF.LoadImg(Alphanumeric(Photo.Album & "id" & Photo.Name), Photo.Image(Setting))
							AddFooterToPdfDoc(clPDF, HttpContext.Current.Request.Url.Host)
							.BeginPage()
							AddPhoto(clPDF, Photo, Setting)
              Photo.Dispose()
              AddSubTitleToPdfDoc(clPDF, "© " & Host & " ", &HFF)
            ...
            Dim Html As String = Nothing
						Try
							Html = ReadAll(MenuManager.PageNameFile(Base, Id, Setting.Language), True)
						Catch ex As Exception
							HttpContext.Current.Response.RedirectPermanent(Href(Setting.Name, False, "default.aspx"), True)
						End Try
            ...
            Dim Photo As Photo = Nothing
                ...
                clPDF.LoadImg(Alphanumeric(Photo.Album & "id" & Photo.Name), Photo.Image(Setting))
						...
						AddFooterToPdfDoc(clPDF, ItemMenu.Href(Setting, Id, Base, MetaTags.Title, True))
						.BeginPage()
    ...
    End Sub
      ...
      H = W * (Photo.Height / Photo.Width)
			...
			clPDF.DrawImg(Alphanumeric(Photo.Album & "id" & Photo.Name), (21 - W) / 2, clPDF.Y, W, H)
			clPDF.Y = clPDF.Y - H
...
Namespace WebApplication
      ...
      WriteObj("<<" & vbCrLf & _
       ...
       ">>")
		...
		Public Sub EndPage()
			...
			intOffsetTemp = Stream.Position - intOffsetTemp
			WriteObj("endstream")
			EndObj()
    ...
    Public Sub LoadImgFromArray(ByVal Name As String, ByRef ImgBuf() As Byte, ByRef ImgColor() As Byte, ByVal ImgWidth As Int32, ByVal ImgHeight As Int32, ByVal ImgBPP As Byte, Optional ByVal ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB)
    ...
    End Sub
      ...
      ToStr = ReplaceBin(CStr(Math.Round(Valore, Dec)), ",", ".")
		End Function
		...
		Public Function Length(ByVal Phrase As String, ByVal FontName As String, ByVal Fontsize As Single) As Single
			...
			If k > 0 Then
				...
				With arrFONT(UsedFont)
					For i = 1 To l
					  ...
					  If (C >= .FirstChar) And (C <= .LastChar) Then
              k += .Widths(C)
            Else
              k += .MissingWidth
            End If
            If C = 32 Then j = j + 1 ' conta gli spazi
					Next
				End With
        Return ToUser(((k * Fontsize / 1000) + (j * mvarWordSpacing) + (l * mvarCharSpacing)) * (mvarTextScaling / 100))
			End If
      Return 0.0!
		End Function
    ...
    Private Function ReadBMP(ByVal Image As System.Drawing.Image, ByRef ImgBuf() As Byte, ByRef ImgColor() As Byte, ByRef ImgWidth As Int32, ByRef ImgHeight As Int32, ByRef ImgBPP As Byte, Optional ByVal ColorSpace As pdfColorSpace = pdfColorSpace.pdfRGB) As Boolean

      Const ToBitplane As Integer = 24 'Convert to 8 or 24 bitplane [change this value to set bitplane convertsion]

      Dim NewImage As System.Drawing.Image = Image.Clone  'Image Rescale
      ...
      ReDim ImgBuf((Bitmap.Width * Bitmap.Height) * ToBitplane / 8)
      Dim P As Integer = 1
    ...
    End Function
		...
		Private Sub FileGet2(ByVal Stream As System.IO.Stream, ByRef Obj As Object, Optional ByVal RecordNumber As Integer = -1)
			...
			If TypeOf Obj Is Array Then
				ReDim Bytes(UBound(Obj))
				Stream.Read(Bytes, 0, Bytes.Length)
				For N As Integer = 0 To UBound(Obj)
					Obj(N) = Bytes(N)
				Next
		...
		Private Function ToPdfStr(ByVal Temp As String) As String
			If Temp <> "" Then
        Temp = ReplaceBin(ReplaceBin(ReplaceBin(Temp, "\", "\\"), "(", "\("), ")", "\)")
    ...
    End Function
      ...
      fdTemp.BaseFont = ReplaceBin(FontName, " ", "")
			fdTemp.FirstChar = 32
			fdTemp.LastChar = 255
			fdTemp.MissingWidth = awTemp(2)
      Dim AddToBase As String = Nothing
      If Style And Drawing.FontStyle.Bold Then
        StemH = MaxWidth / 5.96
        Flag = Flag Or 2 ^ 18
        AddToBase &= "Bold"
      Else
        StemH = MaxWidth / 10.96
      End If
      If Style And Drawing.FontStyle.Italic Then
        ItalicAngle = -11
        Flag = Flag Or 64
        AddToBase &= "Itlic"
      Else
        ItalicAngle = 0
      End If
      If Style And Drawing.FontStyle.Underline Then
        AddToBase &= "Underline"
      End If
      If Style And Drawing.FontStyle.Strikeout Then
        AddToBase &= "Strikeout"
      End If
			If AddToBase <> "" Then
				fdTemp.BaseFont &= "," & AddToBase
			End If
			If InStr(FontName, "serif", CompareMethod.Text) Then
				Flag = Flag Or 2
			End If
			If InStr(FontName, "symbol", CompareMethod.Text) Then
				Flag = Flag Or 4
			Else
				Flag = Flag Or 32
			End If
