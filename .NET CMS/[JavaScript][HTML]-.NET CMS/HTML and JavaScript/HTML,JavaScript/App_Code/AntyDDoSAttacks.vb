...
Namespace WebApplication
    ...
    Public Sub AntiAbnormalSimultaneousRequests(Request As HttpRequest)
      ...
      SyncLock IpDDoSAttacks
        ...
        '      ADDED.Add(IP)
        '      Log("RecordDDoS", 1000, Counter, IP, Request.UserAgent, HostUser, Request.Url.AbsoluteUri)
        '    End If
        '  End SyncLock
        'End If
        '+++++++++++++++++++
        ...
        If Counter > Max Then
          ...
          If Setup.Security.DdosPrevention.LogEvents Then
            ...
            Log("DDoS_AntiAbnormalSimultaneousRequests", 1000, Counter.ToString, IP, Request.UserAgent, HostUser, Request.Url.AbsoluteUri, UrlReferrer)
          End If
          StopResponse()
    ...
    Sub TemporarilyCrawlersStoppingDuringOverload(Request As HttpRequest)
      If IsCrawler(Request) Then
        If MonitorOfResources.AddCurrentThreadToQueue Then
          If Setup.Security.DdosPrevention.LogEvents Then
            Dim UrlReferrer As String = Nothing : If Request.UrlReferrer IsNot Nothing Then UrlReferrer = Request.UrlReferrer.AbsoluteUri
            Log("DDoS_TemporarilyCrawlersStoppingDuringOverload", 1000, Request.UserHostAddress, Request.UserAgent, HostUser, Request.Url.AbsoluteUri, UrlReferrer)
          End If
          StopResponse()
        End If
      End If
                ...
                Else
                  ...
                  If IpHit = Hits.IP Then
                    ...
                    If Counter > MaxRequest Then
                      ...
                      If Setup.Security.DdosPrevention.LogEvents Then
                        Dim UrlReferrer As String = Nothing : If Request.UrlReferrer IsNot Nothing Then UrlReferrer = Request.UrlReferrer.AbsoluteUri
                        Log("DDoS_DisableTheSourcesOfAbnormalHighTraffic", 1000, Counter.ToString, IpHit, Request.UserAgent, HostUser, Request.Url.AbsoluteUri, UrlReferrer)
                      End If
                      StopResponse()
