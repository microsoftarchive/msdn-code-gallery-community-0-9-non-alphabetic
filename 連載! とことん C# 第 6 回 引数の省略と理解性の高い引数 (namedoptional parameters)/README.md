# 連載! とことん C#: 第 6 回 引数の省略と理解性の高い引数 (named/optional parameters)
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 2 月 8 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、ボクはできるだけ自由度を高めるためにメソッドの引数を多く用意したのに、いちいち値を渡すのは面倒だと仲間に怒られました。ほとんどは指定しなくても動作する引数が多いんですが、できるだけ使い方を制限したくありません。どうしたらいいでしょうか?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「なるほど。やり方はいろいろあるのじゃ!」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">オーバーロードする</a> </li><li><a href="#02">引数の既定値 (デフォルト値)</a> </li><li><a href="#03">名前付き引数</a> </li><li><a href="#04">引数省略の見せ場はどこだ!?</a> </li><li><a href="#05">暗黙の引数は定数でなければならない</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. オーバーロードする</h2>
<p>昔から、C# における有効な書き方として、オーバーロードという方法がありました。これは、同じ名前のメソッドでも、引数の型や数が違っていれば複数のメソッドを同じ名前で記述できることを利用したものです。</p>
<p>ここでは、下記の通り指定された writer に、ある名前がある物体であるという文章を組み立てて出力する関数を作成してみましょう。</p>
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.IO.aspx" target="_blank" title="Auto generated link to System.IO">System.IO</a>;

class A
{
    public static void WriteIt( TextWriter writer, string name, string obj )
    {
        writer.WriteLine(&quot;{0} {1} {2}.&quot;, name, name == &quot;I&quot; ? &quot;am&quot; : &quot;is&quot;, obj);
    }
    public static void WriteIt(TextWriter writer)
    {
        WriteIt(writer, &quot;I&quot;, &quot;a body&quot;);
    }
}
...
class Class1
{
    ...
    {
        A.WriteIt(Console.Out);
        A.WriteIt(Console.Out, &quot;She&quot;,&quot;a girl&quot;);
    }
}</code></pre>
</div>
</div>
...
<p>このように、同じ名前のメソッドでも、引数の個数が違えば共存して使い分けることができます。しかし、C# 4.0 以降ではもっと短く書くことができます。次にその事例を紹介します。</p>
<p style="margin-top:20px"><a href="#top"><img src="16763-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 引数の既定値 (デフォルト値)</h2>
<p>C# 4.0 では、実は、引数の既定の値 (デフォルト値) を指定することで、呼び出す際の引数を省略できます。これを使用して上記のソースコードを書き換えてみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class A
{
    public static void WriteIt( TextWriter writer, string name = &quot;I&quot;, string obj = &quot;a boy&quot; )
    {
        writer.WriteLine(&quot;{0} {1} {2}.&quot;, name, name == &quot;I&quot; ? &quot;am&quot; : &quot;is&quot;, obj);
    }
}
...
<p>この通り、引数が 1 つの WriteIt メソッド (オーバーロードされたメソッド) は消滅して、ソースコードは大幅に簡潔になりました。そして、呼び出し側はそれを意識する必要はありません。</p>
<p>しかし、ここには 1 つの問題があります。それは、中間の引数は省略できない、という点です。つまり、WriteIt メソッドを使って、&quot;I am a girl.&quot; という結果を得たいとき、指定したいのは引数 obj だけであるにも関わらず、指定する必要のない引数 name まで指定する必要があります。</p>
<p>次の例では、この問題を解消してみましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16764-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 名前付き引数</h2>
<p>C# 4.0 では、さたにメソッド呼び出しする時に名前を指定することにより、引数の順番から解放されます。以下の例を見てください。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">A.WriteIt(writer: Console.Out, obj: &quot;a girl&quot;);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
...
<p>この例では、呼び出し側で引数の名前を指定することで、その順番から解放されています。順番から解放されるということは、当然のことなら「値を指定する引数より手前の引数は省略できない」という制約からも解放されます。ですから、引数 name は省略されていますが &quot;I&quot; という値が補われます。</p>
<p>また順番がないわけですから、引数の順序を入れ替えても結果は同じです (下記)。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">A.WriteIt(obj: &quot;a girl&quot;, writer: Console.Out);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>また、最初の引数から順に指定する場合には、途中まで名前を省略して、途中から上記の通り名前を指定して書くこともできます (下記)。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">A.WriteIt(Console.Out, obj: &quot;a girl&quot;);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>ただし、この機能によって「引数の名前がみだりに変えられなくなった」ということに注意が必要です。C# 3.0 までは (多くの他のプログラミング言語でも同様ですが)、引数とは、型と順番さえ合っていればよく、引数の名前はメソッド内だけで自由に変更できることが多かったと言えるでしょう。</p>
<p>とはいえ、3.0 までの C# でもリフレクションやスタック トレースを使用する場合は引数の名前が問題になることもあり、C# 4.0 以降の引数は、上記の通り「名前によって指定することができる」わけですから、名前の変更に対して更に敏感になったといえるでしょう。つまり、引数の名前はいい加減に決めるのではなく、「きちんと決めて後からできるだけ変更しない」という配慮が必要になります。</p>
<p>たとえ変更する場合でも、Visual Studio のリファクタリング機能を用いて、呼び出し元も全て変更するなどの配慮が必要となります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16765-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 引数省略の見せ場はどこだ!?</h2>
<p>実は、自由度のために多数の引数を提供しつつ、利便性のために &quot;省略可能&quot; としているソフトは珍しくありません。</p>
<p>例えば、その一例は Microsoft Excel です。引数を省略しないで書くとすれば、かなり冗長になります。以下は、Excel のブックを開く例です。(なお実行に際しては、Microsoft Office 相互運用機能の <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/Microsoft.Office.Interop.Excel.aspx" target="_blank" title="Auto generated link to Microsoft.Office.Interop.Excel">Microsoft.Office.Interop.Excel</a> の参照を追加し、sample.xlsx というファイルが存在していると仮定します。)</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    static void Main(string[] args)
    {
        var excel = new <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/Microsoft.Office.Interop.Excel.Application.aspx" target="_blank" title="Auto generated link to Microsoft.Office.Interop.Excel.Application">Microsoft.Office.Interop.Excel.Application</a>();
        excel.Visible = true;
        excel.Workbooks.Open(
          @&quot;C:\sample.xlsx&quot;, Type.Missing, Type.Missing, Type.Missing,
...
<p>Open メソッドには、上記のように非常に多くの引数が必要です。引数が省略できない C# 3.0 までは、その引数を特に指定しない場合には、上記のように Type.Missing を渡して明示的に引数がないことを示す必要がありました。しかし、C# 4.0 以降では、この Open メソッド呼び出しは、以下のように簡素になります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">excel.Workbooks.Open(@&quot;C:\sample.xlsx&quot;);</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p style="margin-top:20px"><a href="#top"><img src="16766-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 暗黙の引数は定数でなければならない</h2>
<p>前述で紹介した規則に従えば、以下のプログラムは動作します。メソッド a は、引数が省略されている場合はオブジェクトを作成して補うことになります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    private static void a(MyClass p = null)
    {
        if (p == null) p = new MyClass(&quot;匿名希望&quot;);
        Console.WriteLine(&quot;名前は{0}です。&quot;, p.Name);
    }
...
<p>しかし、注意が必要なのは、「メソッド a をもっと短くできるだろう」と思ってうっかり以下のように書き直すと動作しない点です。(コンパイルが通りません。)</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">private static void a(MyClass p = new MyClass(&quot;匿名希望&quot;))
    {
        Console.WriteLine(&quot;名前は{0}です。&quot;, p.Name);
    }</code></pre>
</div>
...
<p>なぜコンパイルを通らないのかと言えば、省略時の既定値は &quot;定数&quot; に限られるからです。この場合、null は定数値なので受け付けられますが、new MyClass (&quot;匿名希望&quot;) は定数値ではないので受け付けられません。</p>
<p>そこで、次の疑問は、「なぜ定数に限られるのか?」です。その理由は簡単で、「メソッドが省略された引数を補っている」のではなく、メソッドは「省略時に送って欲しい値」をメタデータで公開しているだけなのです。つまり、コードを記述する際には「引数が省略」されていますが、コンパイル後の実行ファイルのレベルでは「引数が省略」されているのではなく、「指定された既定値 (デフォルト値) を補って呼び出している」のです。この仕掛けの関係上、既定値 (デフォルト値) は定数である必要があります。つまり、動的な値は扱えません。</p>
<p>ちなみに、この制約から DateTime.Now のような値も既定値に指定できないことに注意が必要です。「引数を省略したら現在時刻が引数になる」というメソッドは記述できません。どうしてもこのような処理を記述したい場合は、以下のようになります。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    private static void a(DateTime? dt = null)
    {
...
<p>また上述した理由から、既定値 (デフォルト値) を持つメソッドを別のアセンブリやモジュールにあるメソッドから呼び出す場合、この既定値を変更した場合には、先ほど述べた仕組みにより、呼び出し側も再コンパイルしなければならないので注意してください。</p>
<p style="margin-top:20px"><a href="#top"><img src="16767-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました。名前を指定して既定の値を指定すれば、渡したい引数にだけ値を渡せるわけですね!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。しかし、その場合は、必要とされる配慮や罠もいくつかあるので注意が必要じゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、この新しい仕組みによって、引数が多いメソッドと少ないメソッドの 2 つを作成する手間からは解放されたということですね。ありがとうございます!」</td>
</tr>
</tbody>
</table>
