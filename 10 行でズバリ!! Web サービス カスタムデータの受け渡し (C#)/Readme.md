# 10 行でズバリ!! Web サービス カスタムデータの受け渡し (C#)
## Requires
- 
## License
...
<tbody>
<tr valign="bottom">
<td><img src="14860-image.png" alt=""></td>
...
<td>
<ul>
<li>Web サービスでのカスタムデータの受け渡し方法について理解する。 </li><li>カスタムデータの受け渡しで使用されている、 XML シリアル化について理解する。 </li></ul>
</td>
</tr>
</tbody>
</table>
...
<tbody>
<tr valign="bottom">
<td><img src="14861-image.png" alt=""></td>
<td><strong>今回紹介するコード</strong></td>
</tr>
</tbody>
</table>
<p>＜Service1.asmx.cs＞</p>
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">public class Weather
{
	// 天気の列挙型
...
<pre id="codePreview" class="csharp"><code class="csharp">public class Weather
{
	// 天気の列挙型
	public enum EnumWeatherType { Fair = 0, Cloudy = 1, Rain = 2 }
...
{
	WSWeather.Service1 ws = new WSWeather.Service1();
	WSWeather.Weather objWeather;

	// Web サービスよりカスタムデータ(天気オブジェクト) の取得
...
<a href="#ws_customdatapass_cs-1">はじめに</a><br>
<a href="#ws_customdatapass_cs-2">作成するアプリケーションの概要</a><br>
<a href="#ws_customdatapass_cs-3">Web サービスの作成</a><br>
<a href="#ws_customdatapass_cs-4">Web サービス クライアントの作成</a><br>
<a href="#ws_customdatapass_cs-5">動作と解説</a><br>
<a href="#ws_customdatapass_cs-6">おわりに</a><br>
<a href="#ws_customdatapass_cs-7">補足情報</a></p>
<table border="0" cellspacing="0" cellpadding="5" bgcolor="#e1f0f8">
<tbody>
<tr>
<td>
<p>Visual Basic .NET の内容はこちらに掲載しています。<br>
<a href="http://msdn.microsoft.com/ja-jp/events/dd283180.aspx">10 行でズバリ !! Web サービス カスタムデータの受け渡し (VB.NET)</a></p>
</td>
</tr>
</tbody>
</table>
...
<hr>
&nbsp;&nbsp;<br>
<h2 id="ws_customdatapass_cs-1">はじめに</h2>
<p>Web サービスでは、文字列や数値等の単純なデータ以外のデータの受け渡しを行うことが可能です。<br>
今回の 10 行シリーズでは、XML シリアル化を使用したカスタムデータの取得方法について、簡単なサンプル アプリケーションを通して解説します。<br>
なお、Web アプリケーションの基本的な開発方法についての説明は割愛しています。<br>
まず初めに「<a href="http://msdn.microsoft.com/ja-jp/events/dd253123.aspx">10 行でズバリ !! ASP.NET による Web サービスの開発 (C#)</a>」および「<a href="http://msdn.microsoft.com/ja-jp/events/dd283294.aspx">10 行でズバリ !! Web サービス クライアント アプリケーションの開発 (C#)</a>」 をご覧ください。</p>
<p>&nbsp;</p>
<h2 id="ws_customdatapass_cs-2">作成するアプリケーションの概要</h2>
<p>今回のサンプルでは、カスタムデータを返す Web サービスとカスタムデータを取得する Web サービス クライアントの 2 つのアプリケーションを作成します。<br>
Web サービスでは、今回カスタムデータとして受け渡す Weather クラスおよび 同クラスを返す Web メソッドを作成します。<br>
Web サービス クライアントでは、 Web 参照を通して Weather クラスの使用および Web メソッドの呼び出しを行います。</p>
<p>&nbsp;</p>
<h2 id="ws_customdatapass_cs-3">Web サービスの作成</h2>
<p>Visual Studio .NET 2003 を起動して、新規にプロジェクトを作成します。<br>
ここでは、[Visual C# プロジェクト] で、[ASP.NET Webサービス] テンプレートを使用します。</p>
<p><img src="14862-image.png" alt=""> <br>
<strong>図 1. ASP.NET Web サービスの選択</strong></p>
<p>新しく追加された Web サービスへコードを記述する前に、カスタムデータとして渡す Weather クラスを作成します。<br>
メニューより [プロジェクト] － [クラスの追加] を選択します。</p>
<p><img src="14863-image.png" alt=""> <br>
<strong>図 2. Weather クラス追加</strong></p>
<p>ファイル名 Weather.cs で Weather クラスを作成後、以下のコードを記述します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>Weather クラスでは、静的メンバーとして天気の列挙型、インスタンスメンバーとして天気のフィールドおよび気温のフィールドを定義します。</p>
<p>次に、Web サービス (&quot;Service1.asmx&quot;) へコードを記述します。<br>
ソリューション エクスプローラより Service1.asmx を選択後、メニューより [表示] － [コード] を選択し、以下 Web メソッド WeatherForecast のコードを記述します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>Web メソッド WeatherForecast では、Weather オブジェクトの生成および天気 / 気温フィールドへ適当な乱数を設定を行い、Web メソッドの戻り値として Weather オブジェクトを返しています。</p>
<p>コードの記述が完了したら、 [ビルド] メニューより [WSWeather のビルド] を選択し、 Web サービスのプロジェクトをビルドしておきます。</p>
<p>&nbsp;</p>
<h2 id="ws_customdatapass_cs-4">Web サービス クライアントの作成</h2>
<p>Web サービス クライアント として、今回は Windows アプリケーション を作成します。<br>
[ファイル] メニューの [プロジェクトの追加] [新しいプロジェクト] を選択し、[Visual C# プロジェクト] で、[Windows アプリケーション] テンプレートを使用します。</p>
<p><img src="14864-image.png" alt=""> <br>
<strong>図 3. Windows アプリケーション の選択</strong></p>
<p>新しく追加された Windows フォーム (&quot;Form1.cs&quot;) に、Button コントロール 1 つを配置し、以下の様にプロパティを設定します。</p>
<table class="grid" border="1" cellspacing="0" cellpadding="5" style="border-collapse:collapse; margin-bottom:10px">
<tbody>
<tr>
<td bgcolor="#f6f6f6"><strong>コントロール ID</strong></td>
<td bgcolor="#f6f6f6"><strong>プロパティ名</strong></td>
<td bgcolor="#f6f6f6"><strong>設定値</strong></td>
</tr>
<tr>
<td>Button1</td>
<td>Text</td>
...
<p>&nbsp;</p>
<p><img src="14865-image.png" alt=""> <br>
<strong>図 4. Windows フォームのデザイン</strong></p>
<p>[プロジェクト] メニューの [Web 参照の追加] を選択し、今回作成した Web サービス Service1 への参照を追加します。<br>
この例では、Web 参照名 を WSWeather にて追加しています。</p>
<table border="1" cellspacing="0" cellpadding="5" width="600" style="border-collapse:collapse; background-color:#f6f6f6">
<tbody>
<tr valign="top">
<td>
<p><img src="14866-image.png" alt=""> Web 参照を実施したタイミングで、参照したプロジェクトへプロキシ クラスと呼ばれるクラスが自動的に作成されます。<br>
プロキシ クラスには、今回使用している WeatherForecast メソッドや Weather クラスなどを含めた様々な情報が含まれており、実行時の Web サービス呼び出しに関与する以外に、インテリセンスによる入力候補の表示やコンパイル時のチェックなどの機能でも利用されます。<br>
なお、Web 参照を実施した後で、Web メソッドの追加やパラメータの変更などが行われた場合は、Web 参照の更新などにて、適時プロキシ クラスの再作成を実施します。</p>
<p><img src="14867-image.png" alt=""> <br>
<strong>図 A. Web 参照の更新</strong></p>
<p>※ [プロジェクト] メニューの [すべてのファイルを表示] を選択することにより、ソリューション エクスプローラ上で、プロキシ クラス (Reference.vb) を表示することが可能になります。</p>
<p><img src="14868-image.png" alt=""> <br>
<strong>図 B. プロキシ クラス</strong></p>
</td>
</tr>
</tbody>
</table>
<p>次に、Form1.cs デザイン画面の Button1 コントロールをダブルクリックし、以下の赤字で書かれているコードを記述します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
Web メソッド WeatherForecast の戻り値である Weather クラスのオブジェクト objWeather を取得します。<br>
その後で、メッセージボックスへ objWeather のメンバー WeatherType (天気) および C (気温) の内容を表示します。</p>
<p>&nbsp;</p>
<h2 id="ws_customdatapass_cs-5">動作と解説</h2>
<p>まず始めに、メニューより [デバッグ] － [開始] (または [F5] キー) を選択し、Web サービス WSWeather をデバッグ実行します。<br>
InternetExplorer が起動して Web サービス用のポータルサイトが立ち上がるので、ポータルサイトを辿り Web メソッド WeatherForecast を起動します。</p>
<p><img src="14869-image.png" alt=""> <br>
<strong>図 5. Web メソッド WeatherForecast 実行結果</strong></p>
<p>Weather オブジェクトの WeatherType フィールドおよび C フィールドが 自動的に XML 形式にシリアル化され、Web メソッドの戻り値として返っているのが確認できます。</p>
<p>次に、Web サービス クライアントである Windows アプリケーションのプロジェクトを スタートアップ プロジェクトに設定し、デバック実行します。</p>
<p>Windows アプリケーションが起動して、[天気予報] ボタンをクリックします。</p>
<p><img src="14870-image.png" alt=""> <br>
<strong>図 6. Windows アプリケーション 実行結果</strong></p>
<p>この動作は、下記の順序にて WeatherForecast オブジェクトが内部的にシリアル化 / 逆シリアル化されることにより、実現されています。</p>
<table border="0" cellspacing="0" cellpadding="2">
<tbody>
<tr>
<td><strong>(1) Web メソッド呼び出し / Web サービス クライアント</strong></td>
</tr>
<tr>
<td><strong>(2) Weather オブジェクト生成 / Web サービス サーバー</strong></td>
</tr>
<tr>
<td>Web サービスにて Weather オブジェクトのインスタンス化および各フィールドメンバーへの値の設定を行います。</td>
</tr>
<tr>
<td><strong>(3) Weather オブジェクトをリターン / Web サービス サーバー</strong></td>
</tr>
<tr>
<td>生成した Weather オブジェクトを Web メソッドの戻り値として返します。<br>
この際に、Web サービスの内部処理にて自動的に XML シリアル化が実施されます。</td>
</tr>
<tr>
<td><strong>(4) Weather オブジェクト取得 / Web サービス クライアントサーバー</strong></td>
</tr>
<tr>
<td>Weatherオブジェクトを Web メソッドの戻り値として取得します。<br>
この際に、Web サービスの内部処理にて自動的に XML 逆シリアル化が実施されます。</td>
</tr>
</tbody>
</table>
<table border="1" cellspacing="0" cellpadding="5" width="600" style="border-collapse:collapse; background-color:#f6f6f6">
<tbody>
<tr valign="top">
<td><img src="14871-image.png" alt=""> Web サービスが内部的に行っている XML シリアル化 / 逆シリアル化については、.NET Framework クラス ライブラリの XmlSerializer クラスを使用しています。<br>
Serialize / Deserialize メソッドを使用してプログラムでのシリアル化の実装も可能です。</td>
</tr>
</tbody>
</table>
<p>&nbsp;</p>
<h2 id="ws_customdatapass_cs-6">おわりに</h2>
<p>このように、Web サービスの XML シリアル化の機能を使用し、カスタムデータを取得することが可能です。<br>
今回のサンプルプログラムでは、戻り値としてカスタムデータを使用しましたが、Web メソッドへのパラメータとしてカスタムデータを使用することが可能です。<br>
Web サービスを使用したアプリケーションの開発に、ぜひ活用ください。</p>
<p>&nbsp;</p>
<h2 id="ws_customdatapass_cs-7">補足情報</h2>
<p>オブジェクトの XML シリアル化の対象としては、パブリック プロパティとパブリック フィールドのみとなります。<br>
従って、プライベート フィールドなどの非パブリック データやメソッドなどはシリアル化の対象となりません。<br>
XML シリアル化に関する詳細の情報については、下記の資料に記載されていますのでご参照ください。</p>
<p>XML シリアル化の概要</p>
<p></p>
<hr>
<p></p>
