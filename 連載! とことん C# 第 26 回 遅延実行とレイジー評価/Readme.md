...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 6 月 11 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、あるメソッドで、返り値を配列にしたら、効率が悪いと言われました。列挙インターフェースを返す方が良いって。それってどう違うのですか ?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「配列は要素をすべて作成しないと返せないが、列挙は必要なデータさえ見つかれば途中で打ち切れるからじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">実行を遅延させる</a> </li><li><a href="#02">レイジー評価が必須の例</a> </li><li><a href="#03">どちらでも大差ない例</a> </li><li><a href="#04">結末</a> </li></ol>
<hr>
<h2 id="01">1. 実行を遅延させる</h2>
<p>例えば、以下のような事例があるとしましょう。</p>
<ul>
<li>0 から 2000 まで 100 刻みの数値の列を生成するメソッドがある </li><li>これに 1.05 を掛けて税込み価&#26684;を得る </li><li>200 円を超える最初の値だけを得る </li></ul>
<p>これをストレートに書き下ろしたのが下記の例です。Calc メソッドのみに注目してください。</p>
...
class Program
{
    ...
    static int[] Calc()
    {
        ...
        foreach (var n in Generate()) list.Add((int)(n * 1.05));
        return list.ToArray();
    }

    static void Main(string[] args)
    {
        Console.WriteLine(&quot;The Answer is {0}&quot;,Calc().First((n)=&gt;n &gt; 200));
    }
}</code></pre>
...
<p>実行結果:<br>
0 100 200 300 400 500 600 700 800 900 1000 1100 1200 1300 1400 1500 1600 1700 1800 1900 2000 The Answer is 210</p>
</div>
<p>このサンプルの Calc メソッドでは、配列 (int[]) そのものを返しています。これは、後述するレイジー評価に対して「集中評価 (Eager Evaluation)」と呼ばれます。</p>
<p>それに対して、int[] ではなく列挙インターフェースを返す実装もあり得ます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    // 計算処理を行う
    static IEnumerable&lt;int&gt; Calc()
    {
        foreach (var n in Generate()) yield return (int)(n * 1.05);
    }
...
{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__com">//&nbsp;0から2000まで100刻みで列挙する</span>&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">static</span>&nbsp;IEnumerable&lt;<span class="cs__keyword">int</span>&gt;&nbsp;Generate()&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">for</span>&nbsp;(<span class="cs__keyword">int</span>&nbsp;i&nbsp;=&nbsp;<span class="cs__number">0</span>;&nbsp;i&nbsp;&lt;=&nbsp;<span class="cs__number">2000</span>;&nbsp;i&nbsp;&#43;=&nbsp;<span class="cs__number">100</span>)&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.Write(<span class="cs__string">&quot;{0}&nbsp;&quot;</span>,i);&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;yield&nbsp;<span class="cs__keyword">return</span>&nbsp;i;&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;
&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__com">//&nbsp;計算処理を行う</span>&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">static</span>&nbsp;IEnumerable&lt;<span class="cs__keyword">int</span>&gt;&nbsp;Calc()&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">foreach</span>&nbsp;(var&nbsp;n&nbsp;<span class="cs__keyword">in</span>&nbsp;Generate())&nbsp;yield&nbsp;<span class="cs__keyword">return</span>&nbsp;(<span class="cs__keyword">int</span>)(n&nbsp;*&nbsp;<span class="cs__number">1.05</span>);&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;
&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">static</span>&nbsp;<span class="cs__keyword">void</span>&nbsp;Main(<span class="cs__keyword">string</span>[]&nbsp;args)&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.WriteLine(<span class="cs__string">&quot;The&nbsp;Answer&nbsp;is&nbsp;{0}&quot;</span>,Calc().First((n)=&gt;n&nbsp;&gt;&nbsp;<span class="cs__number">200</span>));&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;
}</pre>
</div>
</div>
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:<br>
0 100 200 The Answer is 210</p>
</div>
<p>この 2 つの実行結果を見ると、最後の答えは同じであるにもかかわらず、途中経過がまるで違うことが分かると思います。int[] を返す場合は、すべてのリストを作成してから始めて目的の数値を探せるのに対して、上記の列挙インタフェースを返す方法では、すべての値を作成する必要はありません。この差は、データ量が増えてくると、メモリ消費量や実行速度の大きな違いとなって効いてきます。</p>
<p>このように、処理の実行を必要とされるまで遅らせることを「遅延実行」と呼び、そのように遅らされた処理によって式を評価することを「レイジー評価 (Lazy Evaluation)」と呼びます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16675-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. レイジー評価が必須の例</h2>
<p>以下のプログラムは、永遠に '0' だけを列挙するメソッドを経由して 10 回だけ繰り返したら終了するものです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
{
    // 永遠に'0'を列挙し続ける
    static IEnumerable&lt;char&gt; Generate()
    {
        for (; ; )
        {
            yield return '0';
...
<p>しかし、このプログラムは Generate メソッドを char[] を返すように書き換えることはできません。この Generate メソッドは無限個数の '0' を返す機能を持っており、この結果を実際に char[] に入れることはできません。(無限個数の配列は確保できないからです。)</p>
<p>このような例は、レイジー評価を使うことで実現できます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16676-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. どちらでも大差ない例</h2>
<p>最初の例に、ソート機能を付けてみましょう。といっても難しくありません。OrderBy メソッドの呼び出しを 1 つ挿入するだけです。以下のように Main メソッドの内容を書き換えてみましょう。(ここでは、実際に、順番の変更は行わない無意味な処理です。)</p>
<div style="margin:20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>Console.WriteLine(&quot;The Answer is {0}&quot;, Calc().OrderBy((x)=&gt;x).First((n) =&gt; n &gt; 200));</p>
</div>
<p>さて、このように書き換えると実行結果は大幅に変わります。</p>
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>「集中評価」の場合の実行結果:<br>
0 100 200 300 400 500 600 700 800 900 1000 1100 1200 1300 1400 1500 1600 1700 1800 1900 2000 The Answer is 210</p>
</div>
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>「レイジー評価」の場合の実行結果:<br>
0 100 200 300 400 500 600 700 800 900 1000 1100 1200 1300 1400 1500 1600 1700 1800 1900 2000 The Answer is 210</p>
</div>
<p>なんと、両者の差が無くなってしまいました。</p>
<p>なぜ差が無くなったのかといえば、すべてのデータが揃わないとソート (OrderBy) ができないからです。つまり、集中評価でもレイジー評価でも、すべてのデータを揃えてからソートするという要請は同じであり、どちらのケースでも全データを処理する必要性が発生します。</p>
<p>このように、クエリの方法次第で、レイジー評価の意味が低くなってしまうこともあります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16677-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 結末</h2>
...
<td valign="top" style="padding:10px">「分かりました。遅延評価すると効率が劇的に上がる場合もあるわけですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。しかし、常に良くなるわけではない」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「はい。そこは見極めるプログラマーの力量が問われるわけですね」</td>
</tr>
</tbody>
</table>
