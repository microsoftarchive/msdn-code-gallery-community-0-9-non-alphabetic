# [C#] CultureInfo で和暦・西暦を指定して時刻を表示する
## License
...
<hr>
<p>.NET Framework では西暦 (グレゴリオ暦) のほかにいろいろな暦がサポートされています。日付の表示を和暦で表示させる場合、<a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.cultureinfo(VS.80).aspx">System.Globalization.CultureInfo</a> クラスの、<a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.datetimeformatinfo.calendar(VS.80).aspx">DateTimeFormat.Calendar</a>
 プロパティに <a href="http://msdn.microsoft.com/ja-jp/library/909bt4c5(VS.80).aspx">JapaneseCalendar</a> を指定します。特定のカルチャで使用できる暦の一覧については、<a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.cultureinfo.optionalcalendars(VS.80).aspx">CultureInfo.OptionalCalendars</a>
 で表示することが可能です。&quot;ja-JP&quot; - 日本語 (日本) カルチャでサポートされているカレンダーを利用し、西暦、和暦を明示的に指定するサンプル コードは下記の通りです。</p>
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Globalization.aspx" target="_blank" title="Auto generated link to System.Globalization">System.Globalization</a>;

namespace CodeRecipe_DateTime_Format_CS
{
    class Program
    {
        static void Main(string[] args)
        {
            CultureInfo ci1 = CultureInfo.CreateSpecificCulture(&quot;ja-JP&quot;);
            ci1.DateTimeFormat.Calendar = new GregorianCalendar();
            CultureInfo ci2 = CultureInfo.CreateSpecificCulture(&quot;ja-JP&quot;);
            ci2.DateTimeFormat.Calendar = new JapaneseCalendar();

            Console.WriteLine(String.Format(ci1, &quot;現在の時刻は {0:F} です。&quot;, DateTime.Now));
            Console.WriteLine(String.Format(ci2, &quot;現在の時刻は {0:F} です。&quot;, DateTime.Now));
            Console.ReadKey();
        }
    }
}</code></pre>
...
<p>実行結果は下記の通りです。結果は地域と言語設定の長い形式の設定に依存します。</p>
<div style="margin:20px 0px; padding:10px 10px 3px 10px; background-color:#dedede">
<p>現在の時刻は 2010/06/15 6:39:03 です。<br>
現在の時刻は 平成 22/6/15 6:39:03 です。</p>
</div>
<p>このサンプル コードでは <a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.cultureinfo.createspecificculture(VS.80).aspx">
CultureInfo.CreateSpecificCulture</a> メソッドを使用し &quot;ja-JP&quot; - 日本語 (日本) カルチャを作成しています。</p>
<p>また、それぞれの CultureInfo で使用する暦はローカル マシンの地域と言語の設定に関係なく明示的に <a href="http://msdn.microsoft.com/ja-jp/library/49s9fh14(VS.80).aspx">
GregorianCalendar</a> (西暦)、<a href="http://msdn.microsoft.com/ja-jp/library/909bt4c5(VS.80).aspx">JapaneseCalendar</a> (和暦) を指定しています。</p>
<div style="margin:10px 0; padding:10px 10px 0 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>Note:</strong> 日本語 (日本) カルチャにおける西暦では <a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.gregoriancalendartypes(VS.80).aspx">
GregorianCalendar.CalendarType</a> プロパティを設定することで、曜日などの表記を英語と日本語から選ぶことが可能です。</p>
<p>なお、.NET Framework 2.0 以降には <a href="http://msdn.microsoft.com/ja-jp/library/t6y8c1ez(VS.80).aspx">
JapaneseLunisolarCalendar</a> クラスが存在しており、こちらを用いることで干支や六曜を示す int 値を取得することができます。ただし、この暦は特定のカルチャに紐づいていないため、和暦太陰太陽暦を計算する場合のみに使用します。</p>
</div>
<p>&nbsp;</p>
<h3>関連リンク</h3>
<ul>
<li><a href="http://msdn.microsoft.com/ja-jp/library/system.globalization.cultureinfo(VS.80).aspx">CultureInfo クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/c6dw49cz(VS.80).aspx">DateTimeFormatInfo クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/909bt4c5(VS.80).aspx">JapaneseCalendar クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/t6y8c1ez(VS.80).aspx">JapaneseLunisolarCalendar クラス</a>
</li></ul>
