# 連載! とことん C#: 第 30 回 DynamicObject から継承した独自の dynamic クラス
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 7 月 9 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。この前教えてもらった dynamic の話ですが、いくら動的と言っても、自分で定義するクラスは、名前をコンパイル時に決める必要がありますよね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「いや、そうとは限らん。実行時に決まるメンバーを持つクラスも作ることはできるのじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">動的なプロパティ</a> </li><li><a href="#02">不特定のメンバーを扱う</a> </li><li><a href="#03">動的なメソッド</a> </li><li><a href="#04">動的な二項演算子</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. 動的なプロパティ</h2>
<p>さっそくですが、以下のプログラムを見てください。これはコンパイルできますが、一見すると、動作しないように思えます。Hello というメンバーはクラス A には存在しないからです。</p>
<p>しかし、実際は、下記の実行結果のように、ちゃんと動作します。</p>
...
class A : DynamicObject
{
    ...
    {
        result = &quot;Hello&quot;;
        if (binder.Name != &quot;Hello&quot;) return false;
        return true;
    }
...
<p>このプログラムで、World メンバーは、普通に見かけるクラスのメンバーです。しかし、Hello については、以下のように動作しています。</p>
<ul>
<li>まず、<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Dynamic.aspx" target="_blank" title="Auto generated link to System.Dynamic">System.Dynamic</a> 名前空間を using しています </li><li>クラス A は、この名前空間の DynamicObject クラスを継承しています </li><li>DynamicObject クラスを使用することで、存在しないメンバーの読み出しには、TryGetMember メソッドへの呼び出しが発生します (書き込みは TrySetMember メソッド呼び出しになります)
</li><li>ここで、binder.Name によって、メンバーの名前を実行時に調べています </li><li>処理できる場合は true を返し、処理できない場合は false を返しています </li><li>引数 result に、実行結果 (返り値) を渡しています </li></ul>
<p>このような記述によって、dynamic 型を経由して、x.Hello は実行可能となり、&quot;Hello&quot; という値を返してくれます。実行時に、メンバーの存在をチェックして動作するクラスが、簡単に作成できてしまいました。</p>
<p style="margin-top:20px"><a href="#top"><img src="16712-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 不特定のメンバーを扱う</h2>
<p>さて、これだけ見ると、こうした実装方法はまったく必要ないように思えます。ある名前のメンバーを呼び出すことが分かっていれば、上記の World メンバーのように、その名前のメンバーをあらかじめ用意しておいた方がはるかに簡単です。</p>
<p>しかし、この方法を使うと、例えば、どのようなメンバーが必要とされるか、実行時にならないと判断できないようなオブジェクトを C# を組み合わせて使用できます。例えば、以下のサンプル コードのクラス A は、いかなるプログラムから、いかなるメンバー変数にアクセスされても、それに応じた動作を提供できます。下記で、クラス A とクラス Program は便宜上同じソース コード上にあり、呼び出されるメンバー変数が決まっていますが、もちろん、別のアセンブリにあって、別の Program と組み合わせても構いません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class A : DynamicObject
{
    ...
    {
        int r;
        dic.TryGetValue(binder.Name,out r );
        result = r;
        return true;
    }
    public override bool TrySetMember(SetMemberBinder binder, object value)
    {
        dic[binder.Name] = (int)value;
        return true;
    }
}
...
class Program
{
    static void Main(string[] args)
    {
        dynamic x = new A();
        x.OneUp = 1;
        x.SheWasFallInLove = 0;
        Console.WriteLine(x.OneUp);
        Console.WriteLine(x.HeIsDead);
    }
}</code></pre>
</div>
</div>
...
<p>このプログラムでは、クラス A の作成者は、どのようなメンバーが必要であるか (呼び出されるか) 何も知りません。しかし、任意の名前を受け入れるように作成されています。また、すべてのメンバー変数をファイル上などにロード/セーブするという要求にも簡単に対処できます。(メンバー dic に含まれる情報をファイルから読み書きすれば良いのです。)</p>
<p>ちなみに、存在しない HeIsDead というメンバーの読み出しがエラーになっていないことから分かる通り、このプログラムは存在しないメンバーからの読み出しの際は 0 を返すように意図的に作られています。ですから、0 で初期化するという手順も不要です。値を入れない限り、すべての名前は 0 という値を持っているという前提で作られています。</p>
<p style="margin-top:20px"><a href="#top"><img src="16713-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 動的なメソッド</h2>
<p>メソッド呼び出しも、同じように動的に処理することができます。TryGetMember メソッドではなく、TryInvokeMember メソッドをオーバーライドします。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class A : DynamicObject
{
    ...
    {
        Console.WriteLine(&quot;name {0} is {1} characters.&quot;, binder.Name, 
          binder.Name.Length);
        result = null;
        return true;
    }
...
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>name Hello is 5 characters.<br>
name WorldCup is 8 characters.</p>
</div>
<p>この通り、存在しないメソッドへの呼び出しは、TryInvokeMember メソッドの呼び出しになり、そこで使用された名前を調べることができます。これによって、例えば、呼び出し時に補助的な情報がメンバー名の一部に埋め込まれたメソッドなども提供可能で、引数ではなく、名前から情報を取り出すことができます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16714-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 動的な二項演算子</h2>
<p>x が「1」、y が「2」の文字列のとき、C# では x&#43;y を通常「12」とします。しかし、多くの動的言語では数値と文字列の違いを区別せず、3 を結果として返します。</p>
<p>DynamicObject を継承すると、TryUnaryOperation (単項演算子) や TryBinaryOperation (二項演算子) をオーバーロードすることで演算子の動作もカスタマイズできます。ここでは、サンプルとして、二項演算子を用意してみましょう。2 つのオブジェクト間で、座標計算のように、同じ名前のメンバーを加算して結果を出します。座標軸 (x, y, z など) は、動的に変化します。</p>
<p>これを実際に書き下ろした内容は、以下の通りです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Dynamic.aspx" target="_blank" title="Auto generated link to System.Dynamic">System.Dynamic</a>;
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Collections.Generic.aspx" target="_blank" title="Auto generated link to System.Collections.Generic">System.Collections.Generic</a>;
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Linq.Expressions.aspx" target="_blank" title="Auto generated link to System.Linq.Expressions">System.Linq.Expressions</a>;

class A : DynamicObject
{
    private Dictionary&lt;string, int&gt; val = new Dictionary&lt;string,int&gt;();
    public override bool TrySetMember(SetMemberBinder binder, object value)
    {
        val[binder.Name] = (int)value;
        return true;
    }
    public override bool TryGetMember(GetMemberBinder binder, out object result)
    {
        result = val[binder.Name];
        return true;
    }
      ...
      object arg, out object result)
    {
        ...
        {
            if (((A)arg).val.ContainsKey(key))
            {
                r.val.Add(key, val[key] &#43; ((A)arg).val[key]);
            }
            else
            {
                r.val.Add(key, val[key]);
            }
        }
        result = r;
        return true;
...
class Program
{
    static void Main(string[] args)
    {
        ...
        y.Z = 5;
        dynamic z = x &#43; y;
        Console.WriteLine(&quot;({0},{1},{2})&quot;,z.X,z.Y,z.Z);
    }
}</code></pre>
</div>
</div>
...
<p>この場合のポイントは、y.Y が存在しないにも関わらず、TryBinaryOperation の実装を工夫して存在しないメンバーの加算処理は回避している点です。これで、無い値は無いものとして全体の加算処理が実現されています。</p>
<p>上記で、ExpressionType.Add は加算を意味しています。他に、様々な演算子を示す値が存在するので、動的に演算子の種類を判定して動作を変えることもできます。また、この値を使用するために、<a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Linq.Expressions.aspx" target="_blank" title="Auto generated link to System.Linq.Expressions">System.Linq.Expressions</a> の名前空間を追加している点にも注意してください。</p>
<p>今回は DynamicObject を使用した実装例をみましたが、次回は、もう少しお手軽に、こうした実装を試みてみましょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16715-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
...
<td valign="top" style="padding:10px">「分かりました。DynamicObject から継承すれば、実行時に名前を判定できるクラスを作れるわけですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。呼び出される側は、呼び出されるメソッドを用意してリフレクションで処理すれば、いくらでも実行時に解決できるぞ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「なるほど。コンパイル時に何もかも決まってしまう言語とは、そこが違うわけですね」</td>
</tr>
</tbody>
</table>
