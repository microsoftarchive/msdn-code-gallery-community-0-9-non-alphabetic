...
<ul>
<li>まず、System.Dynamic 名前空間を using しています </li><li>クラス A は、この名前空間の DynamicObject クラスを継承しています </li><li>DynamicObject クラスを使用することで、存在しないメンバーの読み出しには、TryGetMember メソッドへの呼び出しが発生します (書き込みは TrySetMember メソッド呼び出しになります)
...
<p>上記で、ExpressionType.Add は加算を意味しています。他に、様々な演算子を示す値が存在するので、動的に演算子の種類を判定して動作を変えることもできます。また、この値を使用するために、System.Linq.Expressions の名前空間を追加している点にも注意してください。</p>
