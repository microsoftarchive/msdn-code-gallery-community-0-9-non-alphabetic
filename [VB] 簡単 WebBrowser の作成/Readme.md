...
<p>.NET Framework 2.0 以降では <a href="http://msdn.microsoft.com/ja-jp/library/system.windows.forms.webbrowser.aspx">
WebBrowser</a> コントロールが追加されています。Visual Studio 2010 においてこのコントロールを使用することで、Windows フォーム アプリケーションで Web ページを表示、移動することが可能になります。WebBrowser コントロールを使用する場合、表示させるアドレスが
<a href="http://msdn.microsoft.com/ja-jp/library/system.windows.forms.webbrowser.url.aspx">
WebBrowser.Url</a> プロパティに設定されます。また、<a href="http://msdn.microsoft.com/ja-jp/library/szwchatt.aspx">WebBrowser.Navigate</a> メソッドを使用し次のアドレスに移動します。</p>
<p>例えば Code Recipe のサイト (<a href="http://msdn.microsoft.com/ja-jp/samplecode.recipe.aspx">http://msdn.microsoft.com/ja-jp/samplecode.recipe.aspx</a>) へ移動させる場合のサンプル コードは下記の通りです。</p>
...
<pre class="hidden"><code class="csharp">&lsquo; Uri の設定
Dim nextUri As new Uri(&quot;http://msdn.microsoft.com/ja-jp/samplecode.recipe.aspx&quot;)
' ナビゲート
WebBrowser1.Navigate(New Uri(nextUrl))</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">&lsquo; Uri の設定
Dim nextUri As new Uri(&quot;http://msdn.microsoft.com/ja-jp/samplecode.recipe.aspx&quot;)
' ナビゲート
WebBrowser1.Navigate(New Uri(nextUrl))</code></pre>
</div>
...
<p>また、<a href="http://msdn.microsoft.com/ja-jp/library/system.windows.forms.webbrowser.navigated.aspx">WebBrowser.Navigated</a> イベントをハンドルすることで、ページ内リンクをクリックしページ遷移が行われた後に現在の Url を表示させるコードは下記の通りです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Private Sub WebBrowser1_Navigated(ByVal sender As System.Object, ByVal e As System.Windows.Forms.WebBrowserNavigatedEventArgs) Handles WebBrowser1.Navigated
    'コンボボックスのテキスト変更
    ComboBox1.Text = WebBrowser1.Url.AbsoluteUri
End Sub</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Private Sub WebBrowser1_Navigated(ByVal sender As System.Object, ByVal e As System.Windows.Forms.WebBrowserNavigatedEventArgs) Handles WebBrowser1.Navigated
    'コンボボックスのテキスト変更
    ComboBox1.Text = WebBrowser1.Url.AbsoluteUri
End Sub</code></pre>
</div>
...
<p>ComboBox と WebBrowser コントロールを配置し、Web ページを表示させる方法は下記の通りです。なお、Windows フォーム アプリケーションの作成方法については
<a href="http://msdn.microsoft.com/ja-jp/netframework/gg317718.aspx">[VB] Windows フォームによるクライアント アプリケーション開発</a>を参照してください。</p>
<div>
<ol>
<li>Windows フォーム アプリケーション プロジェクトを作成し、フォームに ComboBox と WebBrowser コントロールを追加します。ComboBox の Dock プロパティは Top と設定します。
<p style="margin-top:10px"><img src="15523-image.png" alt=""></p>
</li><li>ComboBox のイベント一覧を表示し、KeyDown イベントを &quot;comboBox1_KeyDown&quot; と設定します。KeyDown イベント ハンドラーでは入力された文字列を Uri に変換し、ナビゲートするコードを記述します。
<p style="margin-top:10px"><img src="15524-image.png" alt=""></p>
</li></ol>
</div>
<div class="CodeHighlighter">
<div style="clear:both">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">vb</span>
<pre class="hidden"><code class="csharp">Private Sub ComboBox1_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ComboBox1.KeyDown
    If e.KeyCode = Keys.Enter Then
        Try
            ' Uri の設定
            Dim nextUrl As String = ComboBox1.Text
            ' ナビゲート
    ...
    End If
End Sub</code></pre>
<pre id="codePreview" class="vb"><code class="csharp">Private Sub ComboBox1_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles ComboBox1.KeyDown
    If e.KeyCode = Keys.Enter Then
        Try
            ' Uri の設定
            Dim nextUrl As String = ComboBox1.Text
...
<div class="endscriptcode">&nbsp;</div>
<div class="endscriptcode">3.　webBrowser1 コントロールの Navigated イベントを &quot;webBrowser1_Navigated&quot; と設定します。イベント ハンドラーでは現在の Url を comboBox1 に表示させます。</div>
</div>
</div>
<ol>
<p style="margin-top:10px"><img src="15525-image.png" alt=""></p>
</ol>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
...
<p>実行し Code Recipe サイトの Url (<a href="http://msdn.microsoft.com/ja-jp/samplecode.recipe.aspx">http://msdn.microsoft.com/ja-jp/samplecode.recipe.aspx</a>) を入力した結果は下記の通りとなります。</p>
<p><img src="15526-image.png" alt=""></p>
<p>その他、コンボボックスに今まで表示された Url をリスト形式にて追加し、あとで選択できるようにする機能や、フォームのタイトルにドキュメントのタイトルを表示させる方法については
<a href="http://msdn.microsoft.com/ja-jp/netframework/dd279483.aspx">10 行でズバリ!! 簡単 WebBrowser の作成 (VB)</a> において解説されていますのでこちらもご確認ください。</p>
<h3>関連リンク</h3>
<ul>
<li><a href="http://msdn.microsoft.com/ja-jp/library/system.windows.forms.webbrowser.aspx">WebBrowser</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/3s8ys666.aspx">方法: Windows フォーム アプリケーションに Web ブラウザーの機能を追加する</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/netframework/dd279483.aspx">10 行でズバリ!! 簡単 WebBrowser の作成 (VB)</a>
</li></ul>
