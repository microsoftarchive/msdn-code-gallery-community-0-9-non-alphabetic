...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 5 月 7 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。今回、SQL Server を使ったプログラムを作ることになりましたが、ドキュメントが膨大で何から勉強していいか分かりません」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。データベースは機能が多い上に、独立した 1 つの世界そのものじゃからのう。全貌を理解しようと思うと、新しい OS を 1 から勉強するようなものじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「じゃあ、僕には手も足も出ないのですね」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「いや、Visual Studio と LINQ を使えれば、道が開けてくるかもしれんぞ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">LINQ to SQL は LINQ to Objects と違う</a> </li><li><a href="#02">サーバー エクスプローラーと O/R デザイナー</a> </li><li><a href="#03">実際に書いてみよう</a> </li><li><a href="#04">システム任せは怖いなら</a> </li><li><a href="#05">クエリ式の翻訳と LINQ to SQL のメリット</a> </li><li><a href="#06">結末</a> </li></ol>
<hr>
<h2 id="01">1. LINQ to SQL は LINQ to Objects と違う</h2>
<p>SQL Server はデータのストレージとして幅広く使用されており、追加コスト無し (無償) で使用できる SQL Server Express Edition (これは、Visual Studio の利用者にはお馴染みでしょう) も含めれば、利用頻度はかなり多いと言えます。そのため SQL Server 上に蓄積されたデータ資産も膨大であり、それを利用するプログラムを作成するニーズも決して少なくはないでしょう。</p>
<p>しかし、「データベース」というものに馴染んでいない人にとっては、それは楽なことではありません。SQL Server は大量のデータを迅速に扱うことを目的に進化してきた非常に高度で柔軟な存在です。リレーショナル データベースの誕生以来、それは言語 (C# 言語) や開発環境などとは別に独自に進化を遂げ、大きな開発プロジェクトなどではデータベースまわりを専用に扱うチームが居るなど、かなり専門性の高い分野であると言えるでしょう。</p>
<p>例えば、データベースから配列にすべてのデータを読み込んで、あとは言語の世界で自作プログラムを使って処理すれば楽に思えますが、もちろん、この方法は効率が良くありません。このような処理を行うと、データをわざわざサーバーからローカルにすべて転送するような無駄が発生するだけでなく、せっかくのデータベース エンジンの恩恵が無になってしまいます。本当に転送を必要としているデータは一握りであるはずです。</p>
<p>では、「どう検索を行えば良いのか」といえば、一般に、以下のような手順を踏みます。</p>
<ul>
<li>&#31292;働している SQL Server に接続する </li><li>検索を行う問い合わせ文を送り、検索をしてもらう </li><li>結果を受け取る </li></ul>
<p>これを実現するには、かつて以下の知識が必要とされました。</p>
<ul>
<li>問い合わせ文を記述する T-SQL という言語 (SQL Server の場合) の文法を習得する </li></ul>
<p>しかし、LINQ が登場した今や、その知識抜きで既存のデータベースを活用できるようになりました。LINQ to SQL を使えば、C# 言語の世界でクエリ式を書くだけで、SQL Server を検索できます。</p>
<p>ですが先ほど、自作プログラムで検索を行うことは効率が良くないと説明したばかりです。やはり、クエリ式で検索を行うことは効率が悪いのでしょうか?</p>
<p>いいえ、そうではありません。</p>
<p>実は、LINQ to SQL という技術は、配列などを検索する LINQ to Objects とはまったく別ものなのです。</p>
<ul>
<li>LINQ to Objects は、すべてのデータを配列などに読み込んでからクエリ式で検索します (効率が良くない) </li><li>LINQ to SQL は、検索を依&#38972;する問い合わせ文を生成して、SQL Server に検索を依&#38972;します (効率が良い) </li></ul>
<p>ただし、LINQ to SQL には以下のようなメリットとデメリットがあります。</p>
<ul>
<li>LINQ to SQL のメソッドを使用するか、クエリ式を記述すれば、T-SQL を知らなくても問い合わせを発行できます </li><li>C# のソース コードがシンプルだからといって、自動的に (内部で) 生成される問い合わせ文が効率的であるかは分かりません。性能を限界まで引き出すなら、生成された結果を調べることも重要です (データベース専用の SQL は、書かなくても良いが、読めるだけの知識はあった方が Better です)
</li><li>別マシンや別プロセスの SQL Server のデータベース エンジンにクエリを依&#38972;して実行してもらうという内部の動作上の理由によって、配慮しなければならないこともいくつかあります。例えば、クエリ式が書かれた場所 (C# で記述したプログラムが実行されている場所) から普通に利用できる変数やメソッドなどであっても、SQL Server からはアクセスできない場合があり得ます。(具体的な事例はこのあとで紹介します。)
</li></ul>
<p style="margin-top:20px"><a href="#top"><img src="16642-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. サーバー エクスプローラーと O/R デザイナー</h2>
<p>LINQ to SQL を使う上では、Visual Studio のサーバー エクスプローラーと O/R デザイナーを知っておくと良いでしょう。</p>
<p>サーバー エクスプローラーはVisual Studio から SQL Server などへの接続を管理する機能です。これを使って Visual Studio 上でサーバーやデータベースへ接続しておくと、そのような接続を含むプログラムを容易に作成できます。</p>
<p><img src="16643-image.png" border="0" alt="図 1"></p>
<p>図 1 Visual Studio のサーバー エクスプローラー</p>
<p>まず、自作プログラムのプロジェクトに「LINQ to SQL クラス」を追加します (ソリューション エクスプローラーでプロジェクトをマウスで右クリックし、[追加] - [新しい項目. . .] で [LINQ to SQLクラス] を選択して追加します)。</p>
<p><img src="16644-image.png" border="0" alt="図 2"></p>
<p>図 2 LINQ to SQL クラスの追加</p>
<p>そして、サーバー エクスプローラー上から必要なテーブルなどをこの「LINQ to SQL クラス」にドラッグします。その結果、O/R デザイナーによってデータベースのデータにアクセスするための C# プログラム (コード ファイル) が自動的に生成されます。例えば、Name というフィールドを持つデータベースがあるとすれば、Name というメンバーを持つ C# のクラスが生成され、これを経由して読み出せるようになります。(サーバー エクスプローラーを使用せず、SqlMetal.exe というコマンドライン
 ユーティリティを使用してコードを自動生成することもできます。)</p>
<p>つまり、Visual Studio でサーバー エクスプローラーとO/R デザイナーさえ利用すれば面倒な接続用のクラスなどは自動生成され、あとはクエリ式を書くだけです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16645-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 実際に書いてみよう</h2>
<p>以下は、上記で生成されたコードを使用して、実際に SQL Server のデータを検索している一例 (サンプル コード) です。</p>
...
class Program
{
    ...
    {
        var db = new ConsoleApplication5.DataClasses1DataContext();
...
<p>実行結果: (データベースに以下のようなデータが含まれていたと仮定します)<br>
太郎&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2010/02/22 90points</p>
</div>
<p>ここで特に注目すべきことは、「ほとんどすべてのお膳立てを Visual Studio がやってくれている」という点です。つまり、システムが用意してくれた ConsoleApplication5.DataClasses1DataContext というクラスのインスタンスを作成するだけで、あとはプログラムを使ってクエリし、結果を列挙するだけです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16646-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. システム任せは怖いなら</h2>
<p>しかし、システムにすべてお任せというのは怖い、ということであれば、内部でどのようなクエリが送信されているかを調べることもできます。その方法は簡単で、上記のサンプル コードで、変数 db を new している行のあとに以下の行を挿入するだけです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">db.Log = Console.Out;</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">db.Log = Console.Out;</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>このコードでは、作成されたデータ コンテキスト (上記の db) の Log プロパティとして、コンソールの標準出力である Console.Out を設定して、ログの情報をコンソール上に表示するように指定しています。コンソール以外に出力させたい場合 (例えば、ファイルに出力させたい場合など) は、Console.Out 以外の出力先を指定します。</p>
<p>出力結果は、以下のようになります。</p>
<div style="margin:20px 0px; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px">
<p>実行結果:<br>
SELECT [t0].[Name], [t0].[Date], [t0].[Point]<br>
FROM [dbo].[Table1] AS [t0]<br>
WHERE [t0].[Point] &gt;= @p0<br>
-- @p0: Input Int (Size = -1; Prec = 0; Scale = 0) [80]<br>
-- Context: SqlProvider(Sql2005) Model: AttributedMetaModel Build: 4.0.30128.1</p>
<p>太郎&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2010/02/22 90points</p>
</div>
<p>LINQ to SQL を利用するために、必ずしもデータベースも最先端のバージョンにしておく必要はありません。最先端の Visual Studio 2010 で開発していながら、実は古い世代の SQL Server 2005 にもクエリを発行できます。</p>
<p>もう 1 つのポイントは、ソース上のクエリ式に記述された意図が SQL 文に翻訳されている点です。上記 (実行結果) の 3 行目は、『Point が「@p0」以上である場合』という条件 (Where 句) を示し、4 行目では @p0 として「80」が設定されていることがわかります。つまり、このクエリの中には「n.Point &gt;= 80」という条件そのものが埋め込まれ、SQL Server 側で処理されているわけです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16647-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. クエリ式の翻訳と LINQ to SQL のメリット</h2>
<p>しかし、このクエリ式の翻訳は万能ではありません。例えば、以下のプログラムは動きません。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>これはコンパイルはできますが、実行時に例外になります。</p>
<p>なぜ、前述の例と意図は同じなのに、式を上記のようにメソッドとして分離するとダメなのでしょうか? その理由は、クエリを実行する SQL Server のプロセス空間は、このプログラムを実行しているプロセス空間とは分離されていて、このプログラム上のメソッドを SQL Server のプロセスからは呼び出せないからです。</p>
<p>ここが、あくまで実行しているプログラムと同じプロセス空間内で処理される LINQ to Objects と違う部分です。LINQ to Objects であれば上記の check メソッドの呼び出しは可能であり、同様の以下の LINQ to Objects のクエリ式はちゃんと動作します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Program
{
    ...
    {
        Person[] p = new Person[]{
            new Person(){ Name = &quot;太郎&quot;, Date=new DateTime(2010,2,22), Point=90}
        };

        var query = from n in p
                    where check(n.Point)
                    select n;
...
<p>このように LINQ to SQL では、いくら式を翻訳する機能が強力でも、呼び出せない別世界に存在するメソッドは呼び出せません。仮に強引に呼び出す機能が提供されたとしても、ネットワークやプロセス境界を越えて呼び出すことは時間が掛かりすぎて現実的ではないでしょう。</p>
<p>ただし、LINQ to SQL であっても、以下のような get80 メソッドを用意した場合はクエリできます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この場合、get80 メソッドはローカル上で計算可能であり、その計算結果である「80」をサーバーへ送ることでクエリは成立するためです。</p>
<p>LINQ to SQL では、上記のようなメソッドを混在した記述だけではなく、さまざまな言語 (C# 言語) 固有の記述スタイルを SQL に翻訳してくれます。例えば、SQL Server の T-SQL では、DataTime 型の値から年、月、日などの値を取り出すには DATEPART 関数を使用します。これが LINQ to SQL の場合には、通常の C# による DateTime 型のプロパティにアクセスするだけで、あとは LINQ to SQL が適切な T-SQL に翻訳をおこなってくれます。またリレーションシップを持ったテーブル間のナビゲーションも、JOIN
 などの T-SQL 固有の記述を使わず、プロパティなどを経由して直感的な記述方法で参照できるのです。</p>
<p>ここでは、功と罪の両方のケースを意識して例示しましたが、LINQ to SQL による恩恵は、まさにここで説明した「翻訳」による妙技であると言えるでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16648-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. 結末</h2>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました。今更 SQL の構文を 1 から覚えなくても、LINQ to SQL が自動的に翻訳してくれるのですね!」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。しかし、機械的な変換は無駄もあり得るから、ある程度、どのような SQL 文が生成されるかを把握しておく必要はあるじゃろう」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「最低限の SQL は勉強しておいて損はない、ということですね」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ」</td>
</tr>
</tbody>
</table>
