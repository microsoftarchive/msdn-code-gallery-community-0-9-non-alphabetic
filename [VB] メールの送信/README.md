...
<hr>
<p>.NET Framework 2.0 より追加された <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient">
System.Net.Mail.SmtpClient</a> クラスを使用することで、SMTP (Simple Mail Transfer Protocol) を使用してメールの送信を行うことが可能です。</p>
<p>現在コンピューターにログオンしているユーザーのパスワードが &quot;P@ssw0rd&quot;、メール サーバー &quot;mail.contoso.com&quot; に SSL を利用して接続し、送信元のメール アドレス &quot;from@contoso.com&quot; から、送信先のメール アドレス &quot;to@contoso.com&quot; にメールを送信するサンプル コードは下記の通りです。実際に使用する場合は正しい値を使用してください。</p>
...
<pre id="codePreview" class="vb"><code class="csharp">Imports <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Net.Mail.aspx" target="_blank" title="Auto generated link to System.Net.Mail">System.Net.Mail</a>
Imports <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.Net.aspx" target="_blank" title="Auto generated link to System.Net">System.Net</a>

Module Module1

    Sub Main()
        Dim client As New SmtpClient(&quot;mail.contoso.com&quot;)
        client.Credentials = New NetworkCredential(Environment.UserName, &quot;P@ssw0rd&quot;, Environment.UserDomainName)
        client.EnableSsl = True

        Using message As New MailMessage(&quot;from@contoso.hoge&quot;, &quot;to@contoso.hoge&quot;, &quot;Code Recipe 逆引きサンプルコード&quot;, &quot;Code Recipe 逆引きサンプルコードで作成しました。&quot;)
            Console.WriteLine(&quot;送信開始&quot;)
            Try
                client.Send(message)
            Catch ex As Exception
                Console.WriteLine(&quot;例外が発生しました&quot; &amp; vbCrLf &amp; &quot;{0}&quot;, ex)
            End Try
            Console.WriteLine(&quot;送信完了しました。何かキーを押してください。&quot;)
            Console.ReadKey()
        End Using
    End Sub
...
<p>上記サンプル コードは <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient(VS.80).aspx">
SmtpClient</a> クラスでメール サーバーを指定する <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient.host(VS.80).aspx">
Host</a>、差出人の認&#35388;に使用する資&#26684;情報 <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient.credentials(VS.80).aspx">
Credentials</a>、接続暗号化の使用の有無を決定する <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient.enablessl(VS.80).aspx">
EnableSsl</a> プロパティをそれぞれ設定しています。<br>
その後、<a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage(VS.80).aspx">MailMessage</a> クラスを作成し、メールの差出人情報を
<a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage.sender(VS.80).aspx">
Sender</a>、送信先情報を <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage.to(VS.80).aspx">
To</a>、件名を <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage.subject(VS.80).aspx">
Subject</a>、本文を <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage.body(VS.80).aspx">
Body</a> プロパティにそれぞれ設定し、<a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient.send(VS.80).aspx">SmtpClient.Send</a> メソッドで送信処理を行っています。<br>
なお、メールの送信を非同期で行うには <a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient.sendcompleted(VS.80).aspx">
SendCompleted</a> イベントをハンドルするデリゲートを設定したのち、<a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient.sendasync(VS.80).aspx">SendAsync</a> メソッドを実行します。</p>
<div style="margin:10px 0; padding:10px 10px 0px 10px; background-color:#efefef; border:solid 1px #333333">
<p><strong>Note</strong>: <a href="http://msdn.microsoft.com/ja-jp/library/4971yhhc(VS.80).aspx">
SmtpClient</a> クラス、<a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage(VS.80).aspx">MailMessage</a> クラスなどは既定で UTF-8 エンコーディングが使用されます。.NET Framework においては、MailMessage クラスの Subject で &quot;iso-2022-jp&quot; エンコーディングを使用した場合、特定の文字が正しくエンコードされないことがあります。解決方法についてはこちらの
<a href="http://support.microsoft.com/kb/933866/ja" target="_blank"><span title="新しいウィンドウで開きます">KB</span></a> を参照してください。</p>
</div>
<p>&nbsp;</p>
<h3>関連リンク</h3>
<ul>
<li><a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.smtpclient(VS.80).aspx">SmtpClient クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailmessage(VS.80).aspx">MailMessage クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/system.net.mail.mailaddress(VS.80).aspx">MailAddress クラス</a>
</li></ul>
