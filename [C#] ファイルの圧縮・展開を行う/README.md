...
<hr>
<p>ファイルの圧縮・展開を行う場合、直接ファイルを圧縮するのではなく、ファイルの内容をストリームとしてロードし、.NET Framework 2.0 から追加された
<a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.gzipstream(VS.80).aspx">
System.IO.Compression.GZipStream</a>、<a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.deflatestream(VS.80).aspx">System.IO.Compression.DeflateStream</a> クラスを利用しストリームでの圧縮・展開という形をとります。</p>
<p>なお、GZipStream、DeflateStream クラス双方とも圧縮を行うファイルについては、1 ファイルあたり上限が 4 GB に設定されています。次のサンプル コードでは、アプリケーション実行ディレクトリに約 100 MB のダミー ファイル &quot;DummyFile.data&quot; を作成し、そのファイルを圧縮して &quot;DummyFile.data.gz&quot; としてファイルに保存します。その後、&quot;DummyFile.data.gz&quot; を読み込み、&quot;DecompDummyFile.data&quot; の名前で展開を行っています。</p>
...
using <a class="libraryLink" href="http://msdn.microsoft.com/ja-JP/library/System.IO.Compression.aspx" target="_blank" title="Auto generated link to System.IO.Compression">System.IO.Compression</a>;

namespace CodeRecipe_Compress_CS
{
    class Program
    {
        static void Main(string[] args)
        {
            string SampleFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, &quot;DummyFile.data&quot;);
            string CompressedFilePath = Path.GetFileName(SampleFilePath) &#43; &quot;.gz&quot;;
            string DecompressedFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, &quot;DecompDummyFile.data&quot;);

            if (!File.Exists(SampleFilePath))
            {
                using (FileStream fs = new FileStream(SampleFilePath, FileMode.Create))
                {
                    try
                    {
                    ...
                    {
                        Console.WriteLine(&quot;ダミーファイル作i成中にエラーが発生しました。&quot;);
                        return;
                    }
                }
            }
            ...
            using (FileStream ifs = new FileStream(SampleFilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
            using (FileStream ofs = new FileStream(CompressedFilePath, FileMode.Create, FileAccess.ReadWrite))
            using (GZipStream gzs = new GZipStream(ofs, CompressionMode.Compress))
            {
                try
                {
                    Console.Write(&quot;圧縮開始...&quot;);
                    int readBytes = 0;
                    while ((readBytes = ifs.Read(buffer, 0, buffer.Length)) &gt; 0)
                    {
                        gzs.Write(buffer,0, readBytes);                        
                    }
                    Console.WriteLine(&quot;完了&quot;);
                }
                catch (Exception e)
                {
                    Console.WriteLine(&quot;圧縮中にエラーが発生しました。&quot;);
                    return;
                }
            }

            using (FileStream ifs = new FileStream(CompressedFilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
            using (FileStream ofs = new FileStream(DecompressedFilePath, FileMode.Create, FileAccess.ReadWrite))
            using (GZipStream gzs = new GZipStream(ifs, CompressionMode.Decompress))
            {
                try
                {
                    Console.Write(&quot;展開開始...&quot;);
                    int readBytes = 0;
                    while ((readBytes = gzs.Read(buffer, 0, buffer.Length)) &gt; 0)
                    {
                ...
                {
                    Console.WriteLine(&quot;展開中にエラーが発生しました。&quot;);
                    return;
                }
            }
...
<p>サンプル コードではまず、圧縮・展開共に利用するバッファ領域として 1024 バイトの長さを持つ byte 型配列を宣言します。次に、圧縮元ファイル、圧縮後ファイル用の FileStream、並びにストリームの圧縮を行う GZipStream を Using 句を使用し初期化しています。この際、GZipStream の初期化時の引数として、圧縮されたデータを保存しておくストリームを指定し、<a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.compressionmode(VS.80).aspx">CompressionMode</a>
 を Compress と指定しています。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">byte[] buffer = new byte[1024];

using (FileStream ifs = new FileStream(SampleFilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
using (FileStream ofs = new FileStream(CompressedFilePath, FileMode.Create, FileAccess.ReadWrite))
using (GZipStream gzs = new GZipStream(ofs, CompressionMode.Compress))</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>その後、<a href="http://msdn.microsoft.com/ja-jp/library/system.io.filestream.read(VS.80).aspx">FileStream.Read</a> メソッドを使用しバッファのサイズである、1024 バイトずつデータを読み取り、読み取ったデータを
<a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.gzipstream.write(VS.80).aspx">
GZipStream.Write</a> メソッドを使用し出力用の FileStream へ書き込みます。既に圧縮元のストリームをすべて読み込んでいる場合は、Read メソッドの返り値は 0 が返されます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">int readBytes = 0;
while ((readBytes = ifs.Read(buffer, 0, buffer.Length)) &gt; 0)
{
gzs.Write(buffer,0, readBytes);                        
}</code></pre>
...
<p>続いて、圧縮されたファイルを今度は読み込み元とし、展開を行っています。展開処理を行う場合は、GZipStream の初期化時に CompressionMode を Decompress と指定します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">using (FileStream ifs = new FileStream(CompressedFilePath, FileMode.Open, FileAccess.Read, FileShare.Read))
using (FileStream ofs = new FileStream(DecompressedFilePath, FileMode.Create, FileAccess.ReadWrite))
using (GZipStream gzs = new GZipStream(ifs, CompressionMode.Decompress)</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>展開されたデータを取得するには <a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.gzipstream.read(VS.80).aspx">
GZipStream.Read</a> メソッドを使用します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">while ((readBytes = gzs.Read(buffer, 0, buffer.Length)) &gt; 0)
{
ofs.Write(buffer, 0, readBytes);                        
}</code></pre>
</div>
...
<p>サンプル コードを実行すると &quot;DummyFile.data&quot;、&quot;DummyFile.data.zip&quot;、&quot;DecompDummyFile.data&quot; の 3 ファイルがアプリケーション実行フォルダーに作成されます。実際にファイルの差異を調べたい場合、コマンド プロンプトで &quot;fc &lt;比較元ファイル名&gt;, &lt;比較先ファイル名&gt;&quot; と入力することで、2 つのファイルの差異が確認できます。正しく圧縮・展開が行われていれば下記のように同じものという結果が出力されます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">&gt;fc DummyFile.data DecompDummyFile.data
ファイル DummyFile.data と DECOMPDUMMYFILE.DATA を比較しています
FC: 相違点は検出されませんでした</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
</div>
<p>デフレート アルゴリズムを使用する、DeflateStream クラスを使用する場合も GZipStream クラスを初期化する場合と同様の記述を行います。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<pre id="codePreview" class="csharp"><code class="csharp">// 圧縮
using (DeflateStream gzs = new DeflateStream(ifs, CompressionMode.Compress))
ream(ifs, CompressionMode.Compress)</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
...
<pre id="codePreview" class="csharp"><code class="csharp">// 展開
using (DeflateStream gzs = new DeflateStream(ifs, CompressionMode.Decompress))</code></pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
...
<h3>関連リンク</h3>
<ul>
<li><a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.gzipstream(VS.80).aspx">GZipStream クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/system.io.compression.deflatestream(VS.80).aspx">DeflateStream クラス</a>
</li><li><a href="http://msdn.microsoft.com/ja-jp/library/system.io.filestream(VS.80).aspx">FileStream クラス</a>
</li></ul>
