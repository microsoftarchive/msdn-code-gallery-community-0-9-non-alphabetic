# 連載! とことん C#: 第 3 回 さまざまな型変換 (キャストの話)
## Requires
- 
## License
...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 1 月 29 日</span></h4>
</div>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。ところで、今悩んでいることがあるのです」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「ほほう、それは何かね?」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「あるオブジェクトの型が分かっていますが、別の型に変換して伝達したいと思っています」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「そういう場合はキャストを使えば型の不一致は解決じゃな」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「でも、問題があるのです。別の型のオブジェクトを渡すと、コンパイル エラーにならずに実行時に例外になってびっくりしています」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「なるほど。そういう時は、キャストを付ける代わりに、変換方法をコンパイラに教えるべきじゃな」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="padding:10px">「えっ、そんなこと、できるんですか?」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">目に見えない変換</a> </li><li><a href="#02">暗黙の変換ができないとき</a> </li><li><a href="#03">キャストの出番だ</a> </li><li><a href="#04">キャストがデータを壊すとき</a> </li><li><a href="#05">参照型のキャスト</a> </li><li><a href="#06">as 演算子と is 演算子</a> </li><li><a href="#07">ボックス化とキャスト</a> </li><li><a href="#08">データ型変換のユーザー定義</a> </li><li><a href="#09">結末</a> </li></ol>
<hr>
<h2 id="01">1. 目に見えない変換</h2>
<p>C# には、表現可能な「精度」、つまり桁数が異なるデータ型がいろいろあります。例えば、「sbyte 型は 127 までの数値しか表現できませんが、short は 32767 まで表現できます」といった具合です。そのことから考えると、表現力が小さい変数の内容を表現力の大きい変数に移し替えるのは、なんら問題が無いように思えます。事実、プログラムを書いてみると、すんなりと、違うデータ型の変数に代入できます。</p>
<p>実際に試した例を以下に示します。</p>
...
<p>これは、123 と言う小さな整数を sbyte 型の変数に代入し、それから順次、より多くの桁数を表現できる変数に代入を続け、結果を一気に表示するというものです。</p>
<p>このような、誰が考えても納得できるような単純な変換は、いちいち何かの命令を書き込んで示す必要はありません。C# が暗黙のうちに変換してくれます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16702-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. 暗黙の変換ができないとき</h2>
<p>さて、上記サンプルは桁数が小さい変数から桁数が大きい変数に代入したから何の文句も出ないのですが、すべての場合がそうとは限りません。例えば、桁数が大きい変数から桁数が小さい変数に代入するようなプログラムを書いたら何が起こるでしょう?</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このソースコードは、実はコンパイル時にエラーになり実行できません。変換によって情報が欠落するため変換できないわけです。</p>
<p>このケースでは、実は 123 という数値の情報自体が欠落しないのですが、たいていの場合、それはコンパイラには分からず、ビット数が少ない型へ変換が入るとエラーになります。</p>
<p style="margin-top:20px"><a href="#top"><img src="16703-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. キャストの出番だ</h2>
<p>しかし、プログラマが何が起きるのか承知しているなら、桁数のより小さなデータ型への変換も可能です。そのためには、「キャスト」を使用します。キャストはデータ型の名前を括弧 &quot;()&quot; でくくったもので、変数や定数の先頭に付けることで記述したデータ型にデータを変換することを明示的に示します。キャストを使って、代入されるデータと、代入する変数のデータ型を一致させるように書き直したものが以下のソースです。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>これならコンパイルでき、実行もできます。</p>
<p style="margin-top:20px"><a href="#top"><img src="16704-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. キャストがデータを壊すとき</h2>
<p>上記の例は、扱う数値がどのデータ型にも収まるほど小さかったので問題なく処理できました。しかし、いつも数値が十分に小さいわけではありません。桁数が多いデータを桁数が小さいデータ型の変数に入れた結果、それが収まりきらない場合もあるでしょう。実際に、キャストを使えば、そうしたコードの記述は可能です。しかし、桁数が足りない以上、元の値を保つことは不可能になります。</p>
<p>では、実際に動きを試してみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div style="margin:20px 0px; padding:10px; background-color:#efefef; border:solid 1px">
<p>実行結果:</p>
<p>-103,-26215,-1717986919,1844674407370955161</p>
</div>
<p>結果は見ての通りです。同じ値をキャストしているだけで一切の計算は行っていませんが、値は変化してしまいます。表現するビット数が足りないからです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16705-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 参照型のキャスト</h2>
<p>クラスから new によって作成したインスタンスもキャストの対象になります。例えば下記のサンプル コードでは、ソース内で宣言したクラスである Class1 のインスタンスを object 型にキャストしています。object は、すべてのクラスの共通のベースとなるクラスなので、すべてのクラスのインスタンスは object 型にキャストできます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>しかし、このソース コードで問題になるのは (Class1)o でしょう。object 型を Class1 型に変換できるのでしょうか?</p>
<p>一般論としてはできません。なぜなら、object 型はすべてのクラスのインスタンスである可能性があり、変換できるとは限りません。そもそも、object 型には Class1 型が持っているはずの hello の定義を欠いています。ところが、このプログラムは動作します。なぜなら、object 型の変数 o に入っている実体は Class1 型のインスタンスへの参照だからです。</p>
<p>つまり、「変換できるか否かは中身次第」であり、プログラマ自身が「中身が何であるか」ということに責任を取る場合のみキャストが使用できます。</p>
<ul>
<li>参照型はキャストしても中身が変わるわけではない </li><li>無関係なクラスへのキャストは、コンパイル時にチェックをすり抜けても実行時にはエラーになる </li><li>基底クラスへのキャストは成功する (そもそもキャスト抜きでも成功する) </li><li>サブクラスへのキャストは、中身が一致していれば成功する </li></ul>
<p>&nbsp;</p>
<p style="margin-top:20px"><a href="#top"><img src="16706-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="06">6. as 演算子と is 演算子</h2>
<p>キャストに失敗すると例外を投げます。キャストの成功・失敗を判断したい場合に、この例外を拾って処理をおこなうように記述すると、実行速度のコストが大きな問題となるでしょう。そこで、事前に型をチェックして違っていたらキャストさせないというコードもよく使われます。</p>
<p>その判定に便利なのが is 演算子です。以下のコードは、キャストしたい型であるかを事前に調べた上でキャストを実行します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
class Class1
{
    static void Main(string[] args)
    {
        ...
        if (a is Class2)
        {
            Console.WriteLine(((Class2)a).Hello);
        }
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>しかし、実は、as 演算子を使用すると、「例外を投げない変換」を行うことができます。Main 関数は、この as 演算子を使って以下の通り書き換えることもできます。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden"><code class="csharp">object a = new Class2();
Class2 b = a as Class2;
if (b != null)
{
    Console.WriteLine(b.Hello);
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">object a = new Class2();
Class2 b = a as Class2;
if (b != null)
{
    Console.WriteLine(b.Hello);
...
<p>当然、プログラムでは、できれば例外は発生させたくありません。例外をキャッチする代わりに、上記のように is 演算子や as 演算子を使用して if 文で判定すると、それだけ高速化が実現できます。また、必ず変換に成功する場合でも、一般に、キャストより as 演算子の方がオーバー ヘッドが少なく済むため、これらの演算子は普段のプログラミングで憶えておいて損はないでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16707-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="07">7. ボックス化とキャスト</h2>
<p>すべての参照型は object 型の一種であり、ガベージ コレクトなどの管理下に置かれます。これに対し、int 型は値型であり、本質的に object 型とは異なります。しかし C# では、この相互の変換を簡単におこなうことができます。値型を object 化する処理は「ボックス化」 (boxing) と呼ばれ、その逆は「ボックス化解除」(unboxing) と呼ばれます。</p>
<p>このボックス化とキャストも関係があります。ボックス化・ボックス化解除は、上述の通りデータ変換の一種だからです。以下に簡単なボックス化とボックス化解除のサンプル コードを示します。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>このコードでは、あっさりと値型の整数を object 型に代入していますが、ここでボックス化が発生します。ボックス化で情報が欠落することはないので、明示的な変換は不要です。これに対して、ボックス化したオブジェクトを値型の整数に代入する時点で、ボックス化解除が行われますが、この場合には、&quot;(int)&quot; とキャストされていることから分かるとおり、キャストが必要とされます。ボックス化は暗黙的に行われるが、ボックス化解除は明示的に書かねばならないのです。これは as 演算子では代用できず、キャストを使わねばなりません。</p>
<p style="margin-top:20px"><a href="#top"><img src="16708-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="08">8. データ型変換のユーザー定義</h2>
<p>ある型のデータを別の型へ変換できることを知っている場合、その変換を行うメソッドを自分で記述することができます。この場合、暗黙の変換を行うか、キャストなどで明示した場合のみ変換するかを自由に選ぶことができます。</p>
<p>例えば、以下のコードをみてみましょう。</p>
<div class="CodeHighlighter">
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
{
    public int Length { get; set; }
    public static implicit operator MilliMetor(CentiMetor cm)
    {
...
class Class1
{
    static void Main(string[] args)
    {
        CentiMetor cm = new CentiMetor() { Length = 123 };
        MilliMetor mm = cm;
        Console.WriteLine(&quot;{0}cm,{1}mm&quot;, cm.Length, mm.Length);
    }
}</code></pre>
<pre id="codePreview" class="csharp"><code class="csharp">using System;
...
<p>このサンプル コードは、センチメートルを示すクラスと、ミリメートルを示すクラスがあります。定義には両者に関係がないため、そのままでは変換できません。しかし、両者は扱う単位が違うだけで、本来は変換できることが望まれます。そこで、センチメートルからミリメートルへ変換する機能を定義しています。</p>
<p>上記のコードの public static implicit operator MilliMetor(CentiMetor cm) という部分がその定義に相当します。ちなみに、implicit ではなく explicit キーワードを使うと、暗黙的な変換はできず、明示的な変換を要求されます。キーワード operator の次に変換する片方のデータ型名を書き、引数の型として変換するもう一方のデータ型名を記述します。この 2 つのデータ型名の一方は、その変換演算子を記述するデータ型名 (クラス名など) と一致していなければなりません。この例では、CentiMetor
 クラスの定義の中にあるので、引数の型は CentiMetor 型になっています。</p>
<p style="margin-top:20px"><a href="#top"><img src="16709-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="09">9. 結末</h2>
<table border="0" cellspacing="0" cellpadding="5" style="width:100%; margin-bottom:10px">
<tbody>
...
<td valign="top" style="padding:10px">「分かりました。キャストで強引に型を変えようとするから、実行時に例外が起きるわけですね!」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「うむ。キャストの出番はできるだけ減らすべきだろうな。キャストは本当に必要な時まで取っておくと良いじゃろう。何でもかんでもキャストを入れて強引にコンパイルを通しても、どうせ動作しないのじゃ」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>ジョシュア</strong></td>
