...
<div class="Clear"></div>
<div>
<h4><span style="color:#000000; font-family:Verdana,Arial,Helvetica,sans-serif; font-weight:normal; line-height:normal">更新日: 2010 年 5 月 28 日</span></h4>
</div>
...
<colgroup><col width="83"><col width="910"></colgroup>
<tbody>
<tr>
...
<td valign="top" style="padding:10px">「生徒のジョシュアです。実は、悩んでいることがあります。ラムダ式は便利ですが、使いすぎると上手く書けない機能がいろいろあるのです。例えば、繰り返しの打ち切りとか...」</td>
</tr>
<tr>
<td valign="top" style="padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="padding:10px">「ほほう...確かに、ラムダ式はメソッドの一部じゃが、別の面からいえばそれ自身が独立した 1 つのメソッドなのじゃ」</td>
</tr>
</tbody>
</table>
<hr>
<h2>目次</h2>
<ol>
<li><a href="#01">複数行のラムダ式と、その落とし穴</a> </li><li><a href="#02">ラムダ式の正体 (ラムダ式とメソッドの違い)</a> </li><li><a href="#03">書き換え例</a> </li><li><a href="#04">繰り返しメソッドの功罪</a> </li><li><a href="#05">結末</a> </li></ol>
<hr>
<h2 id="01">1. 複数行のラムダ式と、その落とし穴</h2>
<p>ラムダ式には大きく分けて、2 つの書き方があります。</p>
<p>1 つは、ラムダ式が単に単一の値を返すだけで良い場合です。この場合は結果となる式を書くだけで構いません。return も必要ありません。(下記の Func は、C# であらかじめ用意されている delegate の定義です。<a href="http://msdn.microsoft.com/ja-jp/ff698540.aspx">第 23 回</a>を参照してください。)</p>
...
<pre class="hidden">Func&lt;int&gt; a = () =&gt; 1 &#43; 2;</pre>
<pre id="codePreview" class="csharp">Func&lt;int&gt; a = () =&gt; 1 &#43; 2;</pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
<p>もう 1 つは、それ以上の機能を必要としていて、複数の文を書く必要がある場合です。この場合は中括弧で囲み、最後に retun 文で結果を返します。(下記は単一の処理しか実施していませんが、同様に、複数行記載できます。)</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden">Func&lt;int&gt; a = () =&gt; { return 1 &#43; 2; };</pre>
<pre id="codePreview" class="csharp">Func&lt;int&gt; a = () =&gt; { return 1 &#43; 2; };</pre>
</div>
</div>
<div class="endscriptcode">&nbsp;</div>
</div>
<p>さて、今、以下のようなプログラムがあったとします。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
...
<p>このプログラムは、以下のように、単一行のラムダ式として計算式を含めても動作します。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
...
&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">for</span>&nbsp;(;&nbsp;;&nbsp;)&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">string</span>&nbsp;s&nbsp;=&nbsp;Console.ReadLine();&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="cs__keyword">if</span>&nbsp;(s&nbsp;==&nbsp;<span class="cs__keyword">null</span>)&nbsp;<span class="cs__keyword">break</span>;&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Console.WriteLine(s);&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;
&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;
}</pre>
...
<p>ループ内のコードを上記のように複数行のラムダ式として記述すると、コンパイルが通りません。break 文でコンパイル エラーが発生します。なぜでしょうか?</p>
<p style="margin-top:20px"><a href="#top"><img src="16662-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="02">2. ラムダ式の正体 (ラムダ式とメソッドの違い)</h2>
<p>ラムダ式は、メソッド内で宣言されたローカルな変数にアクセスできます。最初の例では、実際に変数 x をラムダ式の中で利用していました。そのため、ラムダ式は一見して「メソッド内部の世界」(メソッドとして分かれていない 1 つの処理) のように思えるかもしれません。</p>
<p>しかし、実際はそうではありません。</p>
<p>ラムダ式とは、名前の無いメソッドがたまたま別のメソッド内に記述できるだけなのです。従って、本来、ラムダ式は自分自身の世界を持ち、&quot;外部の世界の常識&quot; を受け付けません。しかし、それでは不便なので、変数に対してはキャプチャという機能が用意され、上記の変数 x のように、ラムダ式の中からもアクセスできるようになっているのです。しかし、これは &quot;変数&quot; だけに用意された特例であり、制御構造には適用されません。</p>
<ul>
<li>break 文や continue 文は、ラムダ式の中に対応する繰り返しや switch 文が記述されている場合にのみ記述できます </li><li>return 文は常にラムダ式の実行を打ち切る効能を持っています (呼び出し元のメソッドを打ち切る効能を持っていません) </li></ul>
<p>つまり、ラムダ式は、元のメソッドのローカル変数にはアクセスできますが、繰り返しを打ち切ったり終了させることはできないのです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16663-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="03">3. 書き換え例</h2>
<p>解決策をいくつか見てみましょう。<br>
まず、for 文と break 文を両方ともラムダ式に入れた例です。</p>
<p>書き換え例その 1:</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>あるいは、for 文と break 文を両方ともラムダ式から出すとういう方法もあり得ます。</p>
<p>書き換え例その 2:</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>また、キャプチャされた変数 s にアクセスできるため、以下のような書き方も可能です。</p>
<p>書き換え例その 3:</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
...
<p>この場合、変数 s を宣言する際に初期化を行わない場合、&quot;変数 s が初期化されずに参照された&quot; という警告が出るかもしれませんが、それは上記のように初期化を行えば問題ありません。この場合、このコードは Console.ReadLine メソッドで確実に変数 s に値を入れてから参照していますが、コンパイラはそこまで把握しておらず警告を出してしまうのです。つまり、いかにコンパイラの進化が著しくとも、ラムダ式は本来「別のメソッド」と見なすべきであるため、コンパイラはメソッド内のことは把握できても、メソッド間の関係まで把握できていないわけです。</p>
<p style="margin-top:20px"><a href="#top"><img src="16664-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="04">4. 繰り返しメソッドの功罪</h2>
<p>この問題は、実は以下の知識を活用する場合に大きな障害となります。</p>
<ul>
<li>foreach 文よりも、コレクションのメンバー (メソッド) である ForEach メソッドの方が高速である </li></ul>
<p>例を見てみましょう。今、以下のソース コードがあります。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
...
<p>ここで、foreach 文を以下の ForEach メソッドに書き換えることができます。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden">        Array.ForEach(a, (n) =&gt;
        {
            Console.WriteLine(n);
        });</pre>
<pre id="codePreview" class="csharp">        Array.ForEach(a, (n) =&gt;
        {
            Console.WriteLine(n);
        });</pre>
</div>
...
<p>.NET では、このような書き換えを行った方が速くなります。ところが、以下のような繰り返し処理の場合は、こうした書き換えが難しくなくなります。</p>
<p>まず、以下のようなコードがあったとします。(これは記述できます。)</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
<div class="pluginEditHolderLink">Edit Script</div>
<span class="hidden">csharp</span>
<pre class="hidden">        foreach (var n in a)
        {
           if (n == 0) break;
            Console.WriteLine(n);
        }</pre>
<pre id="codePreview" class="csharp">        foreach (var n in a)
        {
            if (n == 0) break;
            Console.WriteLine(n);
        }</pre>
...
<p>これは以下のように書き換えることはできません。なぜなら、下記のラムダ式に書かれた break 文は foreach 文を打ち切る効能はあっても、ForEach メソッドを打ち切る効能を持っていないからです。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
...
<p>では同等の結果を得るコードが書けないのかといえば、そうではありません。指定条件で繰り返しを打ち切るメソッド (配列のメンバー) の Find を使えば、以下の通り記述できます。これで、0 が出てきた時点でデータが残っていても、この繰り返し処理を打ち切ることができます。</p>
<div style="clear:both">
<div class="scriptcode">
<div class="pluginEditHolder" pluginCommand="mceScriptCode">
<div class="title">C#</div>
...
<p>しかし、本来データを探す Find メソッドを繰り返しの制御に使う発想の転換が必要であり、元のソース コードとかなり違う &quot;意味&quot; になっており、これは、&quot;気軽に書き換えられる内容&quot; とは言えないかも知れません。あくまでも、上記のような制御文に伴う問題を正しく把握しておくことが重要であり、必要に応じ、状況に応じた解決策を検討すべきでしょう。</p>
<p style="margin-top:20px"><a href="#top"><img src="16665-image.png" border="0" alt=""> ページのトップへ</a></p>
<hr>
<h2 id="05">5. 結末</h2>
...
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「分かりました。キャプチャされた変数にアクセスできるだけで、ラムダ式は、本来、別のメソッドと見なすべきなのですね」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>センセーション</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「うむ。じゃから、必要な情報を受け渡したいと思ったら、それは明示的に受け渡す必要があるのじゃ」</td>
</tr>
<tr>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px"><strong>ジョシュア</strong></td>
<td valign="top" style="border-bottom:#ffffff dotted 1px; padding:10px">「はい。いくら簡単に見えても、ラムダ式の中では違うルールが支配しているとキモに命じます!」</td>
</tr>
</tbody>
</table>
